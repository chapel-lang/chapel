...
def _wrap_method(
    ...
    methodbody.append("")
    return _section(
        f"def {name}(self{argstr}) -> {rettype}:", _section(methodbody), indent=0
    )
...
def get_ErrorManager_header() -> str:
    s = _section(
        "class ErrorManager:",
        _section(_wrap_method("__enter__", rettype="List[Error]")),
        _section(_wrap_method("__exit__", args=[("*args", None)])),
        indent=0,
    )
    return "\n".join(s)
