...
if __name__ == "__main__":
    ...
    However, this breaks down if an explicit path is provided, such as
    in Makefiles generated by CMake. For cases like this, allow the tool
    itself (i.e. CMake) to execute the wrapper script, bypassing the driver,
    by setting the CHPL_SHIM_REAL_COMPILER_PATH environment variable.
    This enables tools that explicitly set the path to 'chpl' to still use the
    wrapper script if needed.
    """

    has_real_compiler_path = "CHPL_SHIM_REAL_COMPILER_PATH" in os.environ

    if sys.argv[0].endswith("chpl-shim.py") and not has_real_compiler_path:
        run_toplevel()
    else:
        run_chpl_shim()
