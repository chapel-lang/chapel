        ...
        for formal, _ in chapel.each_matching(root, Formal):
            ...
            if isinstance(parent, Function):
                ...
                if parent.name() == "deserialize" and formal.name() in (
                    ...
                    continue
                if parent.name() == "init" and formal.name() in ("reader", "deserializer"):
                    # all of the formals should be type intent, followed
                    # by reader and deserializer
                    parentFormals = [(f.name(), f.intent()) for f in parent.formals() if isinstance(f, Formal) and not f.is_this()]
                    hasDeserializer = parentFormals[-1] == ("deserializer", "ref")
                    hasReader = parentFormals[-2][0] == "reader"
                    allTypes = all(intent in ("type", "param") for _, intent in parentFormals[:-2])
