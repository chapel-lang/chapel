...
forall (elem, i) in zip(a, a.domain) with (ref b) {
  b[10-i] = i;
}
writeln("End Loop 1");
...
forall (elem, i) in zip(a, a.domain) with (ref dummy, ref b) {
  b[10-i] = i;
  dummy = b[10-i];
}
writeln("End Loop 2");
...
forall (elem, idx) in zip(a, dom) with (ref b) {
...
forall (aElem, domElem) in zip(a, a.domain) with (ref otherArr) {
