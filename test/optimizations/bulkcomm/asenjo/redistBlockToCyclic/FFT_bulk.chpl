...
proc BlockArr.copyBtoC(B)
{
  coforall loc in Locales do on loc
  {
    ...
    var numLocales: int(32)=dom.dist.targetLocDom.dim(0).length:int(32);
    var n:int(32)=dom.dist.boundingBox.dim(0).length:int(32);
...
proc  BlockArr.copyCtoB(B)
{
  ...
  coforall loc in Locales do on loc
  {
    ...
    var count: [1..#(stridelevels+1)] size_t;
    var lid=loc.id;
    var numLocales: int=dom.dist.targetLocDom.dim(0).length;
    var n:int(32)=dom.dist.boundingBox.dim(0).length:int(32);
