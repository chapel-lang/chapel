...
var a : [0..<8] uint(8),
    r = openReader("./readBinaryArrayInput.bin", locking=false),
    f = r.readAll();
...
var b : [0..<4] uint(8);
writeln(openReader("./readBinaryArrayInput.bin", locking=false).readBinary(b));
writeln(b);

// read the whole file, should return 'true':
var c : [0..<8] uint(8);
writeln(openReader("./readBinaryArrayInput.bin", locking=false).readBinary(c));
writeln(c);

// ask for more than 8 bytes, should throw:
var d : [0..<10] uint(8);
try {
    openReader("./readBinaryArrayInput.bin", locking=false).readBinary(d);
