...
// strictly speaking I want createTuple calls to have rank-1. But
...
iter DefaultRectangularArr.dsiPartialThese(param onlyDim,
    otherIdx,
    param tag: iterKind) where tag == iterKind.standalone &&
      __primitive("resolves", dom, "dsiPartialThese",
...
proc DefaultSparseDom.__private_findRowRange(r) {
  ...
  begin with (ref end) {
    var found: bool;
    (found, end) = binarySearch(_indices, ((...r),endDummy), hi=_nnz-1);
    done.write(true);
  }
  var found: bool;
  (found, start) = binarySearch(_indices, ((...r),startDummy), hi=_nnz-1);
  done.waitFor(true);
  return start..min(_nnz-1,end-1);
}
...
iter DefaultSparseDom.dsiPartialThese(param onlyDim: int, otherIdx,
  ...
  coforall t in 0..#numTasks {
    ...
    yield (myChunk[0]..min(_nnz-1, myChunk[1]),);
  }
}
...
iter DefaultSparseDom.dsiPartialThese(param onlyDim: int, otherIdx,
  ...
  const h = if onlyDim!=rank-1 then _nnz-1 else rowRange.high;
  const numElems = h-l+1;
  ...
  if onlyDim != rank-1 {
    coforall t in 0..#numTasks {
      ...
      for i in myChunk[0]..min(_nnz-1,myChunk[1]) do
        if _indices[i].withoutIdx(onlyDim) == otherIdxTup then
          yield _indices[i][onlyDim];
    }
  }
...
proc BlockDom.dsiPartialDomain(param exceptDim) {
  ...
  var space = {(...ranges)};
  var ret = space dmapped blockDist(space, targetLocales =
...
iter LocBlockArr.dsiPartialThese(param onlyDim, otherIdx,
    param tag: iterKind) where tag == iterKind.standalone &&
      __primitive("resolves", myElems._value, "dsiPartialThese",
                                                    onlyDim, otherIdx, tag) {
...
proc CyclicDom.dsiPartialDomain(param exceptDim) {
  ...
  var ret = space dmapped
    cyclicDist(startIdx=this.dist.startIdx.withoutIdx(exceptDim),
...
iter LocCyclicDom.dsiPartialThese(param onlyDim, otherIdx, param tag)
    where tag==iterKind.standalone &&
      __primitive("resolves", myBlock._value, "dsiPartialThese",
                                          onlyDim, otherIdx, tag) {
...
iter LocCyclicArr.dsiPartialThese(param onlyDim, otherIdx, param tag)
    where tag==iterKind.standalone &&
          __primitive("resolves", locDom, "dsiPartialThese",
                                              onlyDim, otherIdx, tag) {
