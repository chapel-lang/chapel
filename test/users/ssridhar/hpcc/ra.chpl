...
def main() {
  ...
  if (printArrays) then writeln("Before updates, T is: ", T, "\n");
  ...
  forall ( , r) in (Updates, RAStream()) {
    on T.domain.dist.ind2loc(r & indexMask) {
      const myR = r;
      local {
	T(r & indexMask) ^= r;
      }
  ...
  const verifyTime = getCurrentTime() - startTime;   // capture elapsed time

  printResults(validAnswer, updateTime, verifyTime); // print the results
}
...
def verifyResults() {
  ...
  const checkTime = getCurrentTime() - startTime;
  ...
  const reduceTime = getCurrentTime() - startTime;

  if (printStats) {
    writeln("Number of errors is: ", numErrors, "\n");
    writeln("Verify Execution Time = ", checkTime);
    writeln("Reduce Execution Time = ", reduceTime);
  }
...
//
def printResults(successful, updateTime, verifyTime) {
  writeln("Validation: ", if successful then "SUCCESS" else "FAILURE");
  if (printStats) {
    writeln("Total Execution time = ", updateTime + verifyTime);
    writeln("Performance (GUPS) = ", (N_U / updateTime) * 1e-9);
  }
}
