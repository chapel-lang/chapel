...
proc populate(dataArray, inputVertices, ref vertexProps, dataTypeMap, colId) {
  ...
  forall (v,j) in zip(inputVertices,inputVertices.domain) {
    ...
    ref currentProperty: unmanaged Property = vertexProps[v,etypeInd];
...
forall (v,d) in vertexProps.domain {
  ...
  if datatype == "int(64)" {
    ...
    vertexProps[v,d] = new unmanaged Property(d, int, newDom, newArr);
  }
  if datatype == "bool" {
    var newDom: domain(int);
    var newArr: [newDom] bool;
    vertexProps[v,d] = new unmanaged Property(d, bool, newDom, newArr);
  }
}
