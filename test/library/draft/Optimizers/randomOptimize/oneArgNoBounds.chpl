...
proc main() {
  ...
  optimizableArgs.pushBack(new optimizableArg("x", 16));
  ...
  // With setting the seed via our COMPOPTS file, we should be guaranteed to
  // see the same number each time, though.
  writeln(bestX);
}
