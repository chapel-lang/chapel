module Build{
  ...
  proc main(): int{
...

    {
      ...
      if (!success) {
        g_print(c_ptrToConst_helper("failed to append: %s\n"):c_string: c_ptr(gchar), error.message);
      ...
      if (isNull(array)) {
        g_print(c_ptrToConst_helper("failed to finish: %s\n"):c_string: c_ptr(gchar), error.message);
        g_error_free(c_ptrTo(error));
        g_object_unref(builder);
        return EXIT_FAILURE;
      }
...

    {
      ...
      g_print(c_ptrToConst_helper("length: %lli\n"):c_string : c_ptr(gchar), n);
      for i in 0..#n {
        var value: gint32;

        value = garrow_int32_array_get_value(GARROW_INT32_ARRAY(array), i);
        g_print(c_ptrToConst_helper("array[%lli] = %d\n"):c_string: c_ptr(gchar),
                i, value);
      }
    }
