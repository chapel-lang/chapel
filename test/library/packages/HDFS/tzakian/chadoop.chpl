...
sync coforall loc in Locales { // look at this more
  ...
  on loc {
    ...
    var hdfsFS: c_void_ptr = HDFS.hdfsConnect(namenode.localize():c_ptrConst(c_char):c_string,
                                              connectNumber);
    writeln("done");
    var fileInfo = HDFS.chadoopGetFileInfo(hdfsFS, dataFile.localize():c_ptrConst(c_char):c_string);
    //      writeln("Info for file " + dataFile + ":");
    //      writeln(fileInfo);
    var blockHosts = HDFS.hdfsGetHosts(hdfsFS, dataFile.localize():c_ptrConst(c_char):c_string, 0, fileInfo.mSize);
    // ==== Get the blocks
    var blockCount = HDFS.chadoopGetBlockCount(blockHosts);
    // END
    ...
    var dataFileLocal = HDFS.hdfsOpenFile(hdfsFS, dataFile.localize():c_ptrConst(c_char):c_string,
                                          O_RDONLY, 0, 0, 0);
