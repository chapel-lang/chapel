...
iter HDFSmap(dataFile: string, namenode: string = "default", port: int(32) = 0) {
  ...
  const hdfsFS: c_void_ptr = HDFS.hdfsConnect(namenode.localize():c_ptrConst(c_char):c_string, port);
  const fileInfo = HDFS.chadoopGetFileInfo(hdfsFS, dataFile.localize():c_ptrConst(c_char):c_string);
  const blockHosts = HDFS.hdfsGetHosts(hdfsFS, dataFile.localize():c_ptrConst(c_char):c_string, 0, fileInfo.mSize); // incr 0?
  const blockCount = HDFS.chadoopGetBlockCount(blockHosts);
  const dataFileLocal = HDFS.hdfsOpenFile(hdfsFS, dataFile.localize():c_ptrConst(c_char):c_string, O_RDONLY, 0, 0, 0);
  ...
  where tag == iterKind.leader {
    ...
    coforall loc in Locales {
      on loc {
        ...
        //======================== File connection =========================
        var hdfsFS: c_void_ptr = HDFS.hdfsConnect(namenode.localize():c_ptrConst(c_char):c_string, port);
        assert(HDFS.IS_NULL(hdfsFS) == HDFS.IS_NULL_FALSE, "Failed to connect to HDFS");

        var fileInfo      = HDFS.chadoopGetFileInfo(hdfsFS, dataFile.localize():c_ptrConst(c_char):c_string);
        var dataFileLocal = HDFS.hdfsOpenFile(hdfsFS, dataFile.localize():c_ptrConst(c_char):c_string, O_RDONLY, 0, 0, 0);
    ...
    if (!onceOnly.testAndSet()) { // we might be able to get rid of the atomic here
      ...
      var blockHosts = HDFS.hdfsGetHosts(hdfsFS, dataFile.localize():c_ptrConst(c_char):c_string, 0, fileInfo.mSize); // Investigate
