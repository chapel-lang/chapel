...
iter HDFSmap(dataFile: string, namenode: string = "default", port: int(32) = 0) {
  ...
  const hdfsFS: c_void_ptr = HDFS.hdfsConnect(c_ptrToConst_helper(namenode.localize()):c_string, port);
  ...
  where tag == iterKind.leader {
    ...
    coforall loc in Locales {
      on loc {
        ...
        //======================== File connection =========================
        var hdfsFS: c_void_ptr = HDFS.hdfsConnect(c_ptrToConst_helper(namenode.localize()):c_string, port);
