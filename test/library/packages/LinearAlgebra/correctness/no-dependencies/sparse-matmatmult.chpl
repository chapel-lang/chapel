...
// Combination of 0-based, 1-based, and other-based parent domains.
{
    ...
    var ADom: sparse subdomain(AparentDom) dmapped new dmap(new CS()); // CSR domain
    var A: [ADom] real;
    ADom += [(1,2),(0,1)];
    A(0,1) = 0.1;
    A(1,2) = 1.2;
    ...
    var BDom: sparse subdomain(BparentDom) dmapped new dmap(new CS()); // CSR domain
    var B: [BDom] real;
    BDom += [(1,3),(1,5)];
    B(1,3) = 1;
    B(1,5) = 1;
...
// Usage similar to that in correctness-Sparse-import.chpl
{
    ...
    var xDom: sparse subdomain({0..2, 0..2}) dmapped new dmap(new CS(sortedIndices=true));
    xDom += [(0,0),(0,1)];
