            ...
            for or in [true, false] {
              ...
              for ow in [true, false] {
                ...
                for ox in [true, false] {
                  ...
                  err = chmod(c_ptrToConst_helper("blah"), perm: mode_t);
                  if err != 0 then halt("error in chmod");
                  err = stat(c_ptrToConst_helper("blah"), c_ptrTo(structStat));
...
permission = 448;
var perm = permission:c_int;
err = chmod(c_ptrToConst_helper("blah"), perm: mode_t); // reset blah's permissions to only include user permissions
if err != 0 then halt("error in chmod");
