...
var err = stat(dirname:c_ptrConst(c_char):c_string, c_ptrTo(structStat));
if err != 0 then halt("error with stat");

const original = structStat.st_mode;

err = chmod(dirname:c_ptrConst(c_char):c_string, permissions: mode_t);
if err != 0 then halt("error with chmod");

// if succeeds then set it back to what it was
err = chmod(dirname:c_ptrConst(c_char):c_string, original);
