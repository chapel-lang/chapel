...
proc testit(seed: int, n: int) {
  ...
  B = 0;
  rng.skipToNth(0);
  for (i,x) in zip(A.domain, rng.sample(A.domain, uint, 0, max(uint))) {
    B[i] = x;
  }
  checkMatch(A, B);
  afterB = rng.getNext(uint);
  ...
  B = 0;
  rng.skipToNth(0);
  forall (i,x) in zip(A.domain, rng.sample(A.domain, uint, 0, max(uint))) with (ref B) {
    B[i] = x;
  }
  checkMatch(A, B);
  afterB = rng.getNext(uint);
  ...
  B = 0;
  rng.skipToNth(0);
  for (x,i) in zip(rng.sample(A.domain, uint, 0, max(uint)), A.domain) {
    B[i] = x;
  }
  checkMatch(A, B);
  afterB = rng.getNext(uint);
  ...
  B = 0;
  rng.skipToNth(0);
  forall (x,i) in zip(rng.sample(A.domain, uint, 0, max(uint)), A.domain) with (ref B) {
    B[i] = x;
  }
  checkMatch(A, B);
  afterB = rng.getNext(uint);
...
proc testitb(seed: int, n: int, min: uint, max: uint) {
  ...
  B = 0;
  rng.skipToNth(0);
  for (i,x) in zip(A.domain, rng.sample(A.domain, uint, min, max)) {
    B[i] = x;
  }
  checkMatch(A, B);
  ...
  B = 0;
  rng.skipToNth(0);
  forall (i,x) in zip(A.domain, rng.sample(A.domain, uint, min, max)) with (ref B) {
    B[i] = x;
  }
  checkMatch(A, B);
}
