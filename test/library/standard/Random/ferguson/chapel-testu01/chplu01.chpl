...
proc getRNG() {
  if useNPB {
    ...
    if testRealBits == 64 then
      return NPBRandomStream.create(seed, eltType=real(64), parSafe=false);
    ...
    if testReal && realTruncateBits == 0 {
      ...
      if testRealBits == 64 then
        return PCGRandomStream.create(seed, eltType=real(64), parSafe=false);
      else if testRealBits == 32 then
        return PCGRandomStream.create(seed, eltType=real(32), parSafe=false);
    } else {
      // Make an integer RNG
      if testUintBits == 64 then
        return PCGRandomStream.create(seed, eltType=uint(64), parSafe=false);
      else if testUintBits == 32 then
        return PCGRandomStream.create(seed, eltType=uint(32), parSafe=false);
    }
  }
}
