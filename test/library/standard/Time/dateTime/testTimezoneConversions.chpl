...
class USTimeZone: Timezone {
  ...
  override proc dst(dt: dateTime) {
    ...
    assert(start.getDate().weekday():int == 7 && start.month == 4 && start.day <= 7);

    // Find last Sunday in October.
    var end = first_sunday_on_or_after(DSTEND.replace(year=dt.year, tz=DSTEND.timezone));
    assert(end.getDate().weekday():int == 7 && end.month == 10 && end.day >= 25);
...
proc first_sunday_on_or_after(in dt) {
  var days_to_go = 7 - dt.getDate().weekday():int;
  if days_to_go > 0 {
