...
proc getMode(filename: string) throws {
  ...
  var err = stat(filename.encode(policy=encodePolicy.unescape):c_ptrConst(c_char):c_string, c_ptrTo(structStat));
...
var err = chmod(filename2.encode(policy=encodePolicy.unescape):c_ptrConst(c_char):c_string,
                newMode:mode_t);
if err != 0 then halt("Error in chmod call: ", strerror(errno));
writeln("chmod works: ", getMode(filename2) == newMode);
err = chmod(filename2.encode(policy=encodePolicy.unescape):c_ptrConst(c_char):c_string,
