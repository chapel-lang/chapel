...
proc test_tzinfo_fromtimestamp() {
  ...
  assert(expected == got.replace(tzinfo=shared.create(nil:unmanaged TZInfo?)));
}
...
proc test_replace() {
  ...
  var base2 = base.replace(tzinfo=shared.create(nil: unmanaged TZInfo?));
...
proc test_mixed_compare() {
  ...
  assert(t1 == t2);
  t2 = t2.replace(tzinfo=shared.create(nil: unmanaged TZInfo?));
  assert(t1 == t2);
