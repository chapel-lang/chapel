...
chpl_bool readFE(_syncvar_chpl_bool _this_663219, int32_t _ln, _string _fn) {
  chpl_bool T2;
  chpl_bool T1;
  chpl_sync_wait_full_and_lock(&((_this_663219)->sync_aux), _ln, _fn);
  T1 = (_this_663219->value);
  chpl_sync_mark_and_signal_empty(&((_this_663219)->sync_aux));
  T2 = T1;
  return T2;
}
...
void _writeEF_672733(_syncvar_uint64_t _this_672738, uint64_t val, int32_t _ln, _string _fn) {
  chpl_sync_wait_empty_and_lock(&((_this_672738)->sync_aux), _ln, _fn);
  _this_672738->value = val;
  chpl_sync_mark_and_signal_full(&((_this_672738)->sync_aux));
  return;
}
...
_syncvar_uint64_t __EQUAL__672665(_syncvar_uint64_t sv, uint64_t val, int32_t _ln, _string _fn) {
  _syncvar_uint64_t T1 = NULL;
  _writeEF_672733(sv, val, _ln, _fn);
  T1 = sv;
  return T1;
}
...
chpl_bool isFull(_syncvar_uint64_t _this_670469) {
  chpl_bool T2;
  chpl_bool T1;
  T1 = chpl_sync_is_full(&((_this_670469)->value), &((_this_670469)->sync_aux), false);
  T2 = T1;
  return T2;
}
