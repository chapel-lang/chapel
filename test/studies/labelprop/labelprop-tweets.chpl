...
proc main(args:[] string) {
  ...
  if distributed {
    var Pairs: domain( (int, int), parSafe=true) dmapped hashedDist(idxType=(int, int));
    run(todo, Pairs);
  } else {
    var Pairs: domain( (int, int), parSafe=true);
    run(todo, Pairs);
  }
}
  ...
  while go.read(memoryOrder.relaxed) && i < maxiter {
    ...
    */
    ...
    //for vid in G.vertices {
      ...
      // label -> count
      var foundLabels:domain(int(32), parSafe=true);
