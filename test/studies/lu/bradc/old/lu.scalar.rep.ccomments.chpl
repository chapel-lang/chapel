...
const MatDom: domain(2) = [1..numRows, 1..numCols] dmapped new cyclicDist;
...
for d in 1..numDiags {
  ...
  const RepRow: domain(Reduced) = [*, d..numCols];
  const RepCol: domain(Reduced) = [d..numRows, *];
  const MatRow: [RepRow] real = Mat(TopRow);
  const MatCol: [RepCol] real = Mat(LeftCol);
  

  // eliminate pivot column by subtracting rows
  forall (i,j) in Reduced {
    Mat(i,j) += MatCol(i,d) * MatRow(d,j);
  }
}
