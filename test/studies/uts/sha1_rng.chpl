...
_extern proc rng_init(inout state: uint(8), seed: int);
...
_extern proc rng_spawn(inout mystate: uint(8), inout newstate: uint(8), spawnNumber: int);


  // Return the current random 32 bit number from an RNG_state
_extern proc rng_rand(inout mystate: uint(8)): int;
