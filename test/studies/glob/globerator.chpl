...
iter glob(pattern:string, flags:int, expand:bool = false, recursive:bool = false, extension:string = ""):string {
    ...
    if expand { // use wordexp
        var glb:wordexp_t;
        err = chpl_wordexp((extension + pattern):c_ptrConst(c_char):c_string, flags:c_int, glb);
        ...
        var glb:glob_t;
        err = chpl_study_glob((extension + pattern):c_ptrConst(c_char):c_string, flags:c_int, glb);
...
where tag == iterKind.leader {
    ...
    if expand { // use wordexp
        var glb:wordexp_t;
        err = chpl_wordexp(pattern:c_ptrConst(c_char):c_string, flags:c_int, glb);
        ...
        var glb:glob_t;
        // Make this spawn a task if we encounter a dir, else yield in parallel
        err = chpl_study_glob(pattern:c_ptrConst(c_char):c_string, flags:c_int, glb);
...
where tag == iterKind.follower {
  if recursive {
    if chpl_isdir(followThis:c_ptrConst(c_char):c_string) == 1 {
