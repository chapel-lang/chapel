...
// Serial trapezoid iterator
iter trapezoidIterator( ): 3*int {

  // if buildSize() was not called beforehand, call it now.
  ...
  for t0 in 1..T by timeBand{   
     ...
     for x0 in tiles_A_start .. upperBound by betweenTiles {
     ...
     // for all starting points of x0 do a tile
     for x0 in tiles_B_start .. upperBound by betweenTiles {
...
// Parallel Trapezoid leader-iterator
iter trapezoidIterator(param tag: iterKind ): Tile
where tag == iterKind.leader {


  var tiles_A_start = lowerBound - timeBand + 1; // starting point for doing A-tile loops
  var tiles_B_start = tiles_A_start + width_max; // starting point for doing B-tile loop
  var betweenTiles = width_min + width_max; // distance to next point in tile set

  // x1 is not used, Im sure it could be sync'ed, but i don't want to deal
  ...
  for t0 in 1..T by timeBand{   
     ...
     // for all starting points of x0 yield a tile
     forall x0 in tiles_A_start .. upperBound by betweenTiles {
        ...
        if x0 <= lowerBound then
           yield (lowerBound, 0, x1, dx1, t0, t1);
     
        // check if right edge tile
        else if x1 >= upperBound then
           yield (x0, dx0, upperBound, 0, t0, t1);

        // otherwise, regular ol' tile
        else
           yield (x0, dx0, x1, dx1, t0, t1);
...
iter trapezoidIterator(param tag: iterKind, followThis: Tile ): 3*int 
where tag == iterKind.follower {
  // tile: (x0, dx0, x1, dx1, t0, t1);
  var x0 = followThis[1];
  var dx0 = followThis[2];
  var x1 = followThis[3];
  var dx1 = followThis[4];
  var t0 = followThis[5];
  var t1 = followThis[6];
...
proc Test_1( ){
   ...
   timer.start();
   // forall values yielded by trapezoidIterator()
   forall (read, write, x) in trapezoidIterator() {
      stencil( read, write, x ); // do stencil computation
   }
   timer.stop();
