...
proc initBoundaryConditions() {
  ...
  // Eventually, these should be able to be localAccesses once XSym,
  // YSym, and ZSym are guaranteed to be distributed
  ...
  // This should eventually be able to get a localAccess too
...
proc CalcElemNodeNormals(ref pfx: 8*real, ref pfy: 8*real, ref pfz: 8*real, 
  ...
  (pfx, pfy, pfz) = ElemFaceNormal(0,1,2,3) + ElemFaceNormal(0,4,5,1) +
...
inline proc CalcElemVolumeDerivative(x: 8*real, y: 8*real, z: 8*real) {
  ...
  (dvdx[0], dvdy[0], dvdz[0]) = VoluDer(1,2,3,4,5,7);
  (dvdx[3], dvdy[3], dvdz[3]) = VoluDer(0,1,2,7,4,6);
  (dvdx[2], dvdy[2], dvdz[2]) = VoluDer(3,0,1,6,7,5);
  (dvdx[1], dvdy[1], dvdz[1]) = VoluDer(2,3,0,5,6,4);
  (dvdx[4], dvdy[4], dvdz[4]) = VoluDer(7,6,5,0,3,1);
  (dvdx[5], dvdy[5], dvdz[5]) = VoluDer(4,7,6,1,0,2);
  (dvdx[6], dvdy[6], dvdz[6]) = VoluDer(5,4,7,2,1,3);
  (dvdx[7], dvdy[7], dvdz[7]) = VoluDer(6,5,4,3,2,0);
...
proc CalcElemCharacteristicLength(x, y, z, volume) {
  ...
  const charLength = max(AreaFace(0, 1, 2, 3),
                         AreaFace(4, 5, 6, 7),
                         AreaFace(0, 1, 5, 4),
                         AreaFace(1, 2, 6, 5),
                         AreaFace(2, 3, 7, 6),
...
proc CalcElemVelocityGradient(xvel, yvel, zvel, pfx,  pfy, pfz,
  ...
  const dyddx  = inv_detJ * ( pfx[0] * (yvel[0]-yvel[6])
                            + pfx[1] * (yvel[1]-yvel[7])
                            + pfx[2] * (yvel[2]-yvel[4])
                            + pfx[3] * (yvel[3]-yvel[5]) ),
...
proc CalcForceForNodes() {
  //zero out all forces
  forall n in Nodes with (ref fx, ref fy, ref fz) do local {
    fx[n].poke(0);
    fy[n].poke(0);
    fz[n].poke(0);
  }
...
const gammaCoef: 4*(8*real) = // WAS: [0..3, 0..7] real = 
...
proc CalcMonotonicQGradientsForElems(ref delv_xi, ref delv_eta, ref delv_zeta, 
  ...
  forall eli in Elems with (ref delv_eta, ref delv_xi, ref delv_zeta, ref delx_eta, ref delx_xi, ref delx_zeta) {
    ...
    local {
      ...
      dxv = 0.25*((xvl[4]+xvl[5]+xvl[6]+xvl[7])-(xvl[0]+xvl[1]+xvl[2]+xvl[3]));
      ...
      dxv = 0.25*((xvl[1]+xvl[2]+xvl[6]+xvl[5])-(xvl[0]+xvl[3]+xvl[7]+xvl[4]));
      ...
      dxv= -0.25*((xvl[0]+xvl[1]+xvl[5]+xvl[4])-(xvl[3]+xvl[2]+xvl[6]+xvl[7]));
...
// TODO: I think more local blocks could be applied to forall
// loops... like maybe anything that doesn't iterate over neighbors?
