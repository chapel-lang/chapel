...
/* Declare number of elements/nodes per edge of the cube */

config const edgeElems = 45; 
       const edgeNodes = edgeElems + 1;

if debug then writeln("Using ", edgeElems, "**3 elements, and ", edgeNodes, "**3 edges");
...
inline proc lxim((x,y,z): index(Elems)): index(Elems) {
...
inline proc lxip((x,y,z): index(Elems)): index(Elems) {
  var i = x*edgeElems*edgeElems + y*edgeElems + z + 1;
  if i == edgeElems**3 then return ElemSpace.high;
  return (i/(edgeElems*edgeElems), i%(edgeElems*edgeElems)/edgeElems, i % edgeElems);
}
inline proc letam((x,y,z): index(Elems)): index(Elems) {
  if x == 0 && y == 0 then return (x,             y, z);
  else if y == 0 then return      (x-1, edgeElems-1, z);
  else return                     (x,           y-1, z);
}
inline proc letap((x,y,z): index(Elems)): index(Elems) {
  if x == edgeElems-1 && y == edgeElems-1 then return (x,     y, z);
  else if y == edgeElems-1 then return                (x+1,   0, z);
  else return                                         (x,   y+1, z);
}
inline proc lzetam((x,y,z): index(Elems)): index(Elems) {
  if x == 0 then return (x,   y, z);
  else return           (x-1, y, z);
}
inline proc lzetap((x,y,z): index(Elems)): index(Elems) {
  if x == edgeElems-1 then return (x,   y, z);
  else return                     (x+1, y, z);
}

iter XSym { for (i,j) in NodeFace do yield (i, j, 0); }
iter YSym { for (i,j) in NodeFace do yield (i, 0, j); }
iter ZSym { for (i,j) in NodeFace do yield (0, i, j); }
...
proc main() {
  if debug then writeln("Lulesh -- Problem Size = ", edgeElems, "**3");
...
proc setupBoundaryConditions() {
  elemBC[..,..,ElemDim.low ] |= XI_M_SYMM;
  elemBC[..,..,ElemDim.high] |= XI_P_FREE;
  elemBC[..,ElemDim.low ,..] |= ETA_M_SYMM;
  elemBC[..,ElemDim.high,..] |= ETA_P_FREE;
  elemBC[ElemDim.low ,..,..] |= ZETA_M_SYMM;
  elemBC[ElemDim.high,..,..] |= ZETA_P_FREE;

  return ElemSpace.low;
}
...
proc ApplyAccelerationBoundaryConditionsForNodes() {
  ...
  for    x in XSym do xdd[x] = 0.0;
  for    y in YSym do ydd[y] = 0.0;
  for    z in ZSym do zdd[z] = 0.0;
}
