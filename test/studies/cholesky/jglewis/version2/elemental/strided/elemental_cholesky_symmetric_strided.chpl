module elemental_cholesky_symmetric_strided {
  ...
  use elemental_schur_complement,
      ...
      local_reduced_matrix_cyclic_partition_strided,
  ...
  proc elemental_cholesky_strided ( A : [] )
	...
	const ( my_rows, my_cols, my_L21_rows_to_compute ) = 
	  my_local_cyclic_data_strided ( A.domain, A_grid_domain, processor );
	...
	for ( A11_cols, A22_cols ) in
	  local_reduced_matrix_cyclic_partition_strided ( A_idx_range ) 
	  do {
	    ...
	    // local {
	    pos_def = scalar_inner_product_cholesky_strided (A11);
	    ...
	    //	    writeln ( processor, "  my_A1x_rows:", my_A1x_rows, "\n" +
	    //		      "     my_Ax1_cols: ", my_Ax1_cols,  "\n" +
	    //		      "     I_compute_L21_rows: ", I_compute_L21_rows );
