module locality_info_strided {
  ...
  proc my_local_cyclic_data_strided 
    ...
    const (r, c)           = A_grid_domain.highBound + (1, 1);
    ...
    const my_rows = A_domain.dim(0).lowBound + my_processor_row*row_stride .. 
                    A_domain.dim(0).highBound  by  r*row_stride;
    const my_cols = A_domain.dim(1).lowBound + my_processor_col*col_stride .. 
                    A_domain.dim(1).highBound  by  c*col_stride;

    // rows it computes of L21

    const my_L21_rows_to_compute =
      A_domain.dim(0).lowBound  +  
             my_processor_row*row_stride  +  r*row_stride*my_processor_col .. 
      A_domain.dim(0).highBound  by  r*c*row_stride;
