...
proc bucketSort(taskID : int, trial: int, time = false, verify = false) {
  ...
  var myBucketedKeys = bucketizeLocalKeys(taskID, myKeys, sendOffsets);
  ...
  var myLocalKeyCounts = countLocalKeys(taskID, keysInMyBucket);
...
proc countLocalKeys(taskID, myBucketSize) {
