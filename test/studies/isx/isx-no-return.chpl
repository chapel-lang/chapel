...
// variant of the isx-bucket-spmd.chpl version.  The goal of this version
// is to avoid returning local arrays, instead passing them in from the
// callsite.
//
...
proc bucketSort(bucketID, trial: int, time = false, verify = false) {
  ...
  countLocalBucketSizes(bucketSizes, myKeys);
  ...
  countLocalKeys(myLocalKeyCounts, bucketID, keysInMyBucket, myMinKeyVal);
