...
class FTree {
  type NodeIdx = 2*int;
  var  dom: domain(NodeIdx);
  var  s: [dom] Coeff;

class Function {
  var k:      int;              // use first k Legendre polynomials as the basis in each box
  var thresh: real;             // truncation threshold for small wavelet coefficients
  var f:      AnalyticFunction; // analytic function f(x) to be projected into the numerical represntation

  var initial_level: int;
  var max_level:     int = 30;  // maximum level of refinement mostly as a sanity check
  var autorefine  = true;       // Refine during mul

  var rm, r0, rp;               // blocks of the block tridiagonal derivative operator
