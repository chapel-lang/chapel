...
proc twinprimes_ssoz() {
  ...
  //  
  forall i in blockDist.createDomain(0..<pairscnt)
   with (+ reduce twinscnt, max reduce last_twin,
         const locPrimes = localize(primes),
         const locInvrs = localize(resinvrs)) {
    const restwin = restwins[i];
    local {
      const (lt, tc) = twins_sieve(restwin, kmin, kmax, ks, start_num,
                                   end_num, modpg, locPrimes, locInvrs);
