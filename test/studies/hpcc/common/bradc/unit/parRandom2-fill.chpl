use Random;

use BlockDist;

config var n = 10000: int(64);

const ProblemDist = distributionValue(new Block(rank=1, idxType=int(64), bbox=[1..n], targetLocales=Locales));
const ProblemSpace: domain(1, int(64)) distributed(ProblemDist) = [1..n];

var A: [ProblemSpace] real;
var B: [ProblemSpace] real;

var randStr1 = new RandomStream(314159265);
var randStr2 = new RandomStream(314159265);

randStr1.fillRandom(A);

for b in B do
  b = randStr2.getNext();

for (i,a,b) in (ProblemSpace,A,B) {
  if (a != b) then
    writeln("mismatch at #", format("#####", i), ": ", a, " != ", b);
  else
    writeln("#", format("#####", i), " = ", a);
}

delete randStr1;
delete randStr2;
