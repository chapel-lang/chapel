...
record HPLparams {
  ...
  def initialize() {
     ...
     if numProbSizes < 1 then writeln("HPL ERROR: Number of values of N is less than 1");
     NDom = [1..numProbSizes];
     for n in N { 
       infile.read(n);
       if n < 0 then writeln("HPL ERROR: Value of N less than 0");
     }
     infile.readln();

     infile.readln(numBlockSizes);
     if numBlockSizes < 1 then writeln("HPL ERROR: Number of values of NB is less than 1");
     NBDom = [1..numBlockSizes];
     for nb in NB { 
       infile.read(nb);
       if nb < 1 then writeln("HPL ERROR: Value of NB less than 1");
     }
     infile.readln();
     ...
     if numProcessGrids < 1 then writeln("HPL ERROR: Number of values of grids is less than 1");
     GridDom = [1..numProcessGrids];
     for p in P {
       infile.read(p);
       if p < 1 then writeln("HPL ERROR: Value of P less than 1");
     }  
     infile.readln();
     for q in Q {
       infile.read(q);
       if q < 1 then writeln("HPL ERROR: Value of Q less than 1");
     }
     infile.readln();
     ...
     if numPanelFacts < 1 then writeln("HPL ERROR: Number of values of PFACT is less than 1");
     PFactDom = [1..numPanelFacts];
     for pf in PFact do pf = infile.read(int):factortype;
     infile.readln();

     infile.readln(numRecurStops);
     if numRecurStops < 1 then writeln("HPL ERROR: Number of values of NBMIN is less than 1");
     NBMinDom = [1..numRecurStops];
     for nbmin in NBMin {
       infile.read(nbmin);
       if nbmin < 1 then writeln("HPL ERROR: Value of NBMIN less than 1");
     }
     infile.readln();

     infile.readln(numPanels);
     if numPanels < 1 then writeln("HPL ERROR: Number of values of NDIV is less than 1");
     NDivDom = [1..numPanels];
     for nd in NDiv {
       infile.read(nd);
       if nd < 2 then writeln("HPL ERROR: Value of NDIV less than 2");
     }
     infile.readln();

     infile.readln(numRecurPanelFacts);
     if numRecurPanelFacts < 1 then writeln("HPL ERROR: Number of values of RFACT is less than 1");
     RFactDom = [1..numRecurPanelFacts];
     for rf in RFact do rf = infile.read(int):factortype; 
     infile.readln();

     infile.readln(numBCasts);
     if numBCasts < 1 then writeln("HPL ERROR: Number of values of BCAST is less than 1");
     BCastDom = [1..numBCasts];
     for bc in BCast do bc = infile.read(int):bcasttype;
     infile.readln();
         
     infile.readln(numDepths);
     if numDepths < 1 then writeln("HPL ERROR: Number of values of DEPTH is less than 1");
     DepthDom = [1..numDepths];
     for d in Depth {
       d = infile.read(int);
       if d < 0 then writeln("HPL ERROR: Value of DEPTH less than 0");
     }      
     infile.readln();
