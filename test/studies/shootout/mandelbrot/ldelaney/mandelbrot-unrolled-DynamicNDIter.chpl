...
proc main() {
  ...
  forall xy in dynamic({xdim, ydim}, chunkSize, 0, 2){
      //NOTE that parDim is 2. This is because the ydim was originally the dim
      //dynamically iterated through, while x was not. 
    var x = xy(1);
    var y = xy(2);
    var xm = x*8;
    var res : uint(8);
    for i in 0..7 by 2 {
      ...
      for j in 1..maxIter {
        ...
        if Zr1*Zr1 + Zi1*Zi1 > limit {  // this breaks now
          b |= 2;
          if b == 3 then break;
        }
        if Zr2*Zr2 + Zi2*Zi2 > limit {  //this probably also breaks now
          b |= 1;
          if b == 3 then break;
        }
      }
