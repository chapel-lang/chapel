...
module meteor {
	...
	proc calcCellIndices(cell: [0..4] int(8), piece: int, indx: int(8) ) {
		cell[0] = indx;
	...
	proc cellsFitOnBoard(cell: [0..4] int(8), piece: int) : bool {
	...
	* solve function.
	*/
	proc firstEmptyCell(cell: [0..4] int(8), minimum: int(8)) {
	 ...
	 */
	proc bitmaskFromCells(cell: [0..4] int(8)) : uint(64) {
	 ...
	 */
	proc fillContiguousSpace(board: [0..49] int(8), indx: int(8)) {
	...
	*/
	...
	var solutionCount: sync int = 0;
	var maxSolutions = 2100;

	proc recordSolution(solNums: [0..9] uint(8), solMasks: [0..9] uint) {
		var solMask: uint;
		var mySolCount = solutionCount;
		solutionCount = mySolCount + 2;
		for solNo in 0..9 {
	...
	proc solve_linear(in depth: int, in cell: int, in board: uint,
			in avail: uint(16), solNums: [0..9] uint(8), solMasks: [0..9] uint) {
		var pieceNoMask: uint(16);
		var maxRots: int;
		var pieceMask: uint;

		if solutionCount.readFF() >= maxSolutions then
			return;
	...
	proc printLargestSmallest() {
		var sIndx = 0;
		var lIndx = 0;
		for i in 1..solutionCount.readFF()-1 {
	...
	proc main(args: [] string) {
		...
		calcRows();
		solve();
		writeln(solutionCount.readFF(), " solutions found\n");
