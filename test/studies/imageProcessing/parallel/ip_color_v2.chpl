...
/*****
      ...
      transformations.  Not a stand-alone program.  'use ip_color_v2'

      This version adds the clrimage(clrimage) constructor.
      ...
      The public bits:
         ...
         Data Structures
           ...
           clrimage - image after color conversion (real data)
               new clrimage(ncol, nrow) - create new image of given size
  ...
  ***/
  ...
  /***
    clrimage:  "Copy" an existing image.  This instance will share the same
               domain as the original.  The image data will not be shared.
...
/***
    ...
    args:           r, g, b - source pixel to convert
                    x, y, z - result
    modifies:  x, y, z
    NOTE:      This would normally be a private procedure, but we use it
...
proc rgb_to_hsv(rgb : rgbimage, ref hsv : clrimage) {
  ...
  hsv = new clrimage(rgb.ncol, rgb.nrow);
...
proc rgb_to_yuv(rgb : rgbimage, ref yuv : clrimage) {
  ...
  yuv = new clrimage(rgb.ncol, rgb.nrow);
...
proc rgb_to_rgb(rgb_c : rgbimage, ref rgb_chpl : clrimage) {
  ...
  rgb_chpl = new clrimage(rgb_c.ncol, rgb_c.nrow);
