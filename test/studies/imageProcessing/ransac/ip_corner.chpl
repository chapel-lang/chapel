...
/*****
      ...
      The public bits:
        Data Structures
          circumference - ring iterator
            new circumference(radius, xc, yc[, closed])  stand-alone, one shot
                                                         iterate via 'these'
            new circumference(radius)                    reusable
                inst.set_radius(radius)                  change radius
                inst(xc, yc [, closed])                  iterator
          fastspec - parameters for analysis
          corner - description of corner point
          thrdir - background relationship to corner point rel. threshold
        Procedures
          find_corners - build a list of all corners in an image
          mark_corners - copy a greyscale image and set FAST corners in red
          is_corner  - return true if FAST criteria passed
          is_corner_with_detail - return true if FAST passes, details via arg
...
record fastspec {
  ...
  var suppsep : real;                   /* x or y diff to suppress corner */
}

record corner {
  var center : 2 * int;                 /* center point = corner */
...
/* A simplified version of an ArrayList that over-allocates the array size
   ...
   Public methods:
     this - with no arguments, returns array with data
            with argument, returns a copy of the stored elements
...
proc suppress_corners(rawcnr : unmanaged chunkarray(corner),
  ...
  proc DefaultComparator.compare(c1 : corner, c2 : corner) : int {
    var (xc1, yc1) = c1.center;         /* center of corner 1 */
    var (xc2, yc2) = c2.center;         /* center of corner 2 */
    if (xc1 == xc2) then return (yc1 - yc2);
    else return (xc1 - xc2);
  }
  ...
  for c1 in cnr.domain {
    for c2 in cnr.domain[c1+1..] {
      ...
      if ((xc1 + spec.suppsep) < xc2) then break;
      if (abs(yc1 - yc2) <= spec.suppsep) {
