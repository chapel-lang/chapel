...
/*****
      ...
      we should expect the matching algorithm to behave with a rotate/shift/
      translate transform.
...
config const theta = 0.0;               /* angle of rotation */
...
proc main() {
  ...
  testmap.theta = pi * theta / 180.0;
  testmap.sxx =  sx * cos(testmap.theta);
  testmap.sxy =  sx * sin(testmap.theta);
  testmap.syx = -sy * sin(testmap.theta);
  testmap.syy =  sy * cos(testmap.theta);
  testmap.dx = dx;
  testmap.dy = dy;
  testmap.sx = sx;
  testmap.sy = sy;
