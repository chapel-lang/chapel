...

{
  ...
  proc debugmsg(args...) {} // stand-in for debugging printouts
  ...
  config const noiseDet = false;  // track noise computations in detail?
  ...
  proc Gen_RMAT_graph ( a : real, 
    ...
    { use Random;
      ...
      debugmsg("Gen_RMAT_graph  scale ", SCALE, "  locs ", numLocales,
 ...
 RandomPrivate_iterate(real, edge_domain, seed, start=rndPos+4*delta)) {
      ...
      debugmsg("array decls");

      var bit = 1 << numDep;
      var   skip : real;

      for depth in 1..numDep do {
          if noiseDet then debugmsg("starting depth ", depth);
	  bit >>= 1;
	  ...
	  Noisy_d = d * (0.95 + 0.1 * Unif_Random);
          if noiseDet then debugmsg("  randomizing");
	  ...
	  Noisy_c *= norm;
	  Noisy_d *= norm;
          if noiseDet then debugmsg("  normalizing");

	  skip = Rand_Gen.getNext ();
	  Rand_Gen.fillRandom (Unif_Random);
          if noiseDet then debugmsg("  preparing Unif_Random");

	  Edges.assign_quadrant ( Unif_Random, Noisy_a, Noisy_b, 
				     Noisy_c, Noisy_d, bit );
          if noiseDet then debugmsg("  assign_quadrant");
          else debugmsg("depth ", depth);
	...
	for e in edge_range do
	  writeln ("edge (", e, ") : (", Edges(e), ", ", 
		   Edge_Weight (e), ")" );
        debugmsg("DEBUG_WEIGHT_GENERATOR");
      }
      ...
      if edgeChecks then debugmsg("consistency checking");
    ...
    if parGC {
      ...
      debugmsg("neighbor lists - creating");
	...
	for u in G.vertices do {
	  ...
	  writeln (); 
       ...
       debugmsg("DEBUG_GRAPH_GENERATOR");
      }
	...
	writeln ( count, "  ", edge_count (count) );
      debugmsg("DEBUG_NEIGHBOR_HISTOGRAM");
     }
