...
// # atoms per bin
var CountSpace : domain(3) = DistSpace;
var binCount : [CountSpace] int(32);

// locale's chunk of 'bins' (consists of whole bins)
class Chunk {
  var Dom : domain(3);                            // real and ghost bins
  var Real : domain(3);                           // real bins
  var NeighDom : domain(rank=3, stridable=true);  // nearest neighbors
  var Arr : [Dom] [perBinSpace] atom;             // bin and atom storage
  var NSlice, WSlice : [NeighDom] domain(3);      // local/remote ghost slices
...
// poor man's barrier. most situations are taken care of by coforall's join
  ...
  coforall (i,j,k) in LocaleGridDom {
    on LocaleGrid(i,j,k) {
      ...
      for xyz in ND {
        ...
        if debug 
          then writeln(WSlice[xyz], " is the remote neighbor of ", NSlice[xyz]);
      }
    } // end of on statement
  } // end of coforall
}
...
proc create_velocity(treq : real) {
  ...
  coforall (ijk) in LocaleGridDom {
    on LocaleGrid[ijk] {
      ...
      forall (bin,c) in zip(Bins[ijk].Arr[Real], binCount[Real]) {
        for a in bin[1..c] {
          a.v -= vtot;
        }
      }
      ...
      for neigh in Bins[ijk].NeighDom {
        ...
        forall (N, W) in zip(NSlice[neigh], WSlice[neigh]) {
          ...
          for (loc, rem) in zip(Data[N][1..c], Bins[Neighs[neigh]].Arr[W][1..c]) {
            // if bin indices are the same, we need not adjust atom positions
...
proc bindist(i, j, k : int) {
  ...
  // can't think of a cleaner way to do this right now
  // would be nice if we could map >,==,< to 1, 0 -1
    ...
    on LocaleGrid[ijk] {
      ...
      forall (b,r,c) in zip(Data[Real],Real, binCount[Real]) {
        for (a,i) in zip(b[1..c],1..c) {
          ...
          for s in stencil[1..nstencil-1] {
            const o = r + s;
            for (n,x) in zip(Data[o][1..binCount[o]],1..binCount[o]) {
...
// resize if necessary (thanks to chapel, other stuff stays in place)
...
proc binatoms() {
  ...
  coforall ijk in LocaleGridDom {
    on LocaleGrid[ijk] {
      ...
      Moved[ijk] = M;
      var Real = Bins[ijk].Real;
      for (b,r,c) in zip(Bins[ijk].Arr[Real] ,Real ,binCount[Real]) {
        var cur = 1;

        // for each atom, check if moved
        // because we move from the end, allows us to examine 
        // the recently moved atom
        while(cur <= c) {
...
proc coord2bin(x : v3){
  ...
  var mask : v3int;
  mask(1) = if x(1) >= dim(1) then 1 else 0;
  mask(2) = if x(2) >= dim(2) then 1 else 0;
  mask(3) = if x(3) >= dim(3) then 1 else 0;

  const temp = ((x - dim*mask) * bininv + nbin*mask - mbinlo);
