...
if debugranges then {
  ...
  writeln("Working with Serial Dist. Adaptative Splitting v1");
  // The initial range
...
{   
  ...
  coforall tid in 0..#MaxThreadsPerLocale do { 
	...
	if undone(victim) then {
	  ...
	  current=splitRange(remain(victim), splitFactorSteal, undone(victim));
	...
	if numberofStealintents >= MaxThreadsPerLocale-1 then
	  ...
	  if victim==tid then victim=(victim+1) % MaxThreadsPerLocale;
...
var grainsize:string; // "fine", "coarse", "nested"
use Random; 
