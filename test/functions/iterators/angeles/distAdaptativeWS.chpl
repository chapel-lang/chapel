...
{   
  ...
  coforall tid in 0..#MaxThreadsPerLocale do { 
    ...
    //if tid==MaxThreadsPerLocale-1 then 
    //  remain(tid)=c#(sizechunk*(MaxThreadsPerLocale-1)-totlen); 
    ...
    if dFlag(tid) then {
      writeln("After the barrier at tid ", tid);
    }
...
proc CheckCorrectness(mWorking:bool, mStealing:bool, grainsize:string)
{
  ...
  select grainsize {
	...
	writeln();
	writeln("Grain size: ", grainsize,". Working with Adaptative ", mW," and Stealing with ", mS);
	writeln();
	t.start();
