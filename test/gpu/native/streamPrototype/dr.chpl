...
on here.gpus[0] {
  ...
  // one array
  forall  i in 0..n with (ref a) { a[i] += i+10;       } writeln(a);
  forall  i in 0..n with (ref a) { a[i] += i+value;    } writeln(a);
  foreach i in 0..n { a[i] += i+10;       } writeln(a);
  foreach i in 0..n { a[i] += i+value;    } writeln(a);

  // two arrays
  forall  i in 0..n with (ref b) { b[i] += a[i]*10;    } writeln(b);
  forall  i in 0..n with (ref b) { b[i] += a[i]*value; } writeln(b);
