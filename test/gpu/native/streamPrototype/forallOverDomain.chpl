...
on here.gpus[0] {
  ...
  // one array
  forall  i in a.domain with (ref a) { a[i] += i+10;       } writeln(a);
  forall  i in a.domain with (ref a) { a[i] += i+value;    } writeln(a);
  foreach i in a.domain { a[i] += i+10;       } writeln(a);
  foreach i in a.domain { a[i] += i+value;    } writeln(a);

  // two arrays
  forall  i in a.domain with (ref b) { b[i] += a[i]*10;    } writeln(b);
  forall  i in a.domain with (ref b) { b[i] += a[i]*value; } writeln(b);
