...
def initatoms(inout atoms, count, volsize, gridspacing)
{
  use Random;
...
def copyatoms(host_atominfo, atominfo, inout atoms, arr_index, count, zplane)
{
...
def writeenergy(filename, energy, volsize)
{
...
def main()
{
  ...
  const GPUBlockDist = new dmap(new GPUExplicitDist(rank=2, tbSizeX=BLOCKSIZEX, tbSizeY=BLOCKSIZEY, gridSizeX=4, gridSizeY=64));
  ...
  const atomspace : domain(1) dmapped GPUCCBlockDist = 0..MAXATOMS-1;
