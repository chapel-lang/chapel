...
proc copyatoms(host_atominfo, atominfo, inout atoms, arr_index, count, zplane)
{
  ...
  for i in 0..#count do {
    var step = i * 4;
    host_atominfo(i).x = atoms(step:int + arr_index:int);
    host_atominfo(i).y = atoms(step:int+1+arr_index:int);
    var dz = zplane - atoms(step:int+2+arr_index:int);
    host_atominfo(i).z = (dz * dz) : real(32);
    host_atominfo(i).w = atoms(step:int+3+arr_index:int);
  }
  atominfo = host_atominfo;
}
...
proc main()
{
  ...
  const GPUBlockDist = new dmap(new GPUExplicitDist(rank=2, tbSizeX=BLOCKSIZEX, tbSizeY=BLOCKSIZEY));
  const GPUBlockDist_atom = new dmap(new GPUExplicitDist(rank=1));
  ...
  var host_atominfo : [atom_dom] float4;
  ...
  for atomstart in 0..#atomcount by MAXATOMS:int {
    ...
    copyatoms(host_atominfo, atominfo, atoms, 4*atomstart, runatoms, 0 * gridspacing);
    //writeln("test = ", host_atominfo(0), " and = ", host_atominfo(1));
...
writeln("totaltime = ", totaltime);
  if (outfile == true) then {
    energy = energygrid;
    writeenergy("cp_checksum.out", energy, volsize); 
  }
}
