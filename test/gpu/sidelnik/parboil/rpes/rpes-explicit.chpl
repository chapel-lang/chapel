use GPUExplicitDist, GPUSharedMemDist;
...
def main() {
  ...
  var Wghts : [0..TABLESIZE-1] real(32);
  // prepare host data
  ...
  var Block_Work : [0..(totNumBlocks-1)] uint4;
  DistributeBlockWork(totNumShells, ComputeShell, Block_Work);

  var ReductionSum_Range = 0..(totReductionElements:int(64)-1);
  var NumBlocks_Range = 0..(totNumBlocks:int(64)-1);
  var NumAtoms_Range = 0..(totNumAtoms:int(64)-1);
  var BasisShells_Range = 0..(totBasisShells:int(64)-1);
  var TableSize_Range = 0..(TABLESIZE:int(64)-1);
  var BlockSize_Range = 0..(BLOCK_SIZE:int(64)-1);

  var ReductionSum : [ReductionSum_Range] real(32);

  // prepare device data
  const GPUBlockDist = new dmap(new GPUExplicitDist(rank=1, tbSizeX=BLOCK_SIZE));
  const GPUSharedDist = new dmap(new GPUSharedMemDist(rank=1));
  const reduction_space : domain(1, int(64)) dmapped GPUBlockDist = ReductionSum_Range;
  const NumBlocks_space : domain(1, int(64)) dmapped GPUBlockDist = NumBlocks_Range;
  const NumAtoms_space : domain(1, int(64)) dmapped GPUBlockDist = NumAtoms_Range;
  const BasisShells_space : domain(1, int(64)) dmapped GPUBlockDist = BasisShells_Range;
  const TableSize_space : domain(1, int(64)) dmapped GPUBlockDist = TableSize_Range;
  var BlockSize_space : domain(1, int(64)) dmapped GPUSharedDist = BlockSize_Range;

  //var reduce_range = [0..5:int(64)];
  //var sub_reduce_space : domain(1, int(64)) dmapped GPUBlockDist = reduce_range;

  var d_ReductionSum : [reduction_space] real(32);
  var d_FinalReduce : [reduction_space] uint2;
  var d_Output : [NumBlocks_space] real(32);
  var d_Block_Work : [NumBlocks_space] uint4;
  var d_Coors : [NumAtoms_space] float4;
  var d_Sprms : [BasisShells_space] float2;
  var d_Wghts : [TableSize_space] real(32);

  d_Block_Work = Block_Work;
  d_FinalReduce = FinalReduce;
  d_Coors = Coors;
  d_Sprms = Sprms;
  d_Wghts = Wghts;

  writeln("maxblocks = ", MaxBlocks);
  RunKernel(numIntegrals, totNumBlocks, d_Block_Work, d_Coors, d_Sprms, 
      BlockSize_space, d_Output, d_FinalReduce, 
      d_ReductionSum, MaxBlocks, totReductionElements, GPUBlockDist);

  ReductionSum = d_ReductionSum;
  //var Output : [NumBlocks_Range] real(32);
  //Output = d_Output;
  ...
  //for i in NumBlocks_Range do {
  //  writeln("test: ", i, " ", Output(i));
  //}

}


def RunKernel(numIntegrals, totNumBlocks, in d_Block_Work, in d_Coors, 
    in d_Sprms, BlockSize_space, out d_Output, 
    in d_FinalReduce, out d_ReductionSum, MaxBlocks, totReductionElements, 
    in GPUBlockDist) {
  var runs = (ceil(1.0 * totNumBlocks / GRID_SIZE)) : int;
  ...
  var gpuSpace : domain(1, int(64)) dmapped GPUBlockDist;
  for run in 0..#runs do {
    ...
    gpuSpace = 0..(numBlocks*BLOCK_SIZE):int(64)-1;
    ...
    forall myIndex in gpuSpace do {
      ...
      const myWork = d_Block_Work(Offset + blid);
      ...
      if (localthid < Maxthid) then {
        ...
        const (Atom1, Atom2, Atom3, Atom4) = (d_Coors(atom1), d_Coors(atom2), d_Coors(atom3), d_Coors(atom4));
        const (Param1, Param2, Param3, Param4) = (d_Sprms(n1), d_Sprms(n2), d_Sprms(n3), d_Sprms(n4));
      ...
      for param s in 0..5 do {
        const i = 1 << (-s  + LOG_BLOCK_SIZE - 1);
        if thid < i then
  ...
  runs = (ceil(1.0 * totReductionElements / GRID_SIZE)) : int;
  ...
  for run in 0..#runs do {
    ...
    // reduction kernel
    on __primitive("chpl_on_gpu", gridX, gridY, sizeX, sizeY, sizeZ, shared_size) do {

      const (blid, thid) = (getBlockID_x(), getThreadID_x());
      const reduce_data = d_FinalReduce(Offset+blid);
      ...
      for param s in 0..5 do {
        param i = 1 << (s * -1 + LOG_BLOCK_SIZE - 1);
        if thid < i then
          Data(thid) += Data(thid+i);
        thread_barrier();
      }
      ...
      if thid == 0 then
        d_ReductionSum(Offset + blid) = Data(0);
    }
    ...
    for shell2 in shell1..(totNumShells-1) do {
      for shell3 in shell2..(totNumShells-1) do {
        for shell4 in shell3..(totNumShells-1) do {
          ...
          var blocks = (ceil(1.0 * integrals / BLOCK_SIZE)) : int;
...
def PopulateShells(filename, in BasisAtom, out ComputeAtom, out ComputeShell, numBasisAtoms) {
  ...
  for atom in 0..(numAtoms-1) do {
    var (atomType, X, Y, Z) = infile.readln(string, real, real, real);
  ...
  for basisatom in BasisAtom do {
    ...
    for shell in 0..(numShells-1) do {
      ...
      for prim in 0..(numPrimitives - 1) do {
        var (temp, temp1, temp2, alpha, coeff) = infile.readln(string, string, string, real, real);
