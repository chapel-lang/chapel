...
if timing then timer.start();
    forall xIndex in gpu_numX_space do {
      ...
      if numK % 2 then {
        const expArg = PIx2: real(32) * (kx_d(kIndex) * sX + ky_d(kIndex) * sY + kz_d(kIndex) * sZ);
      ...
      while ((kIndex < kGlobalIndex + KERNEL_Q_K_ELEMS_PER_GRID) && (iIndex < numK)) {
        const expArg = PIx2: real(32) * (kx_d(kIndex) * sX + ky_d(kIndex) * sY + kz_d(kIndex) * sZ);
        const phiMag_scalar = phiMag_d(kIndex);
        ...
        const expArg1 = PIx2: real(32) * (kx_d(kIndex1) * sX + ky_d(kIndex1) * sY + kz_d(kIndex1) * sZ);
        const phiMag_scalar1 = phiMag_d(kIndex1);
