// Tests the behavior when listing a containing type in the except list of a
// used module that defines a tertiary method on a forwarded type.
module definesType {
  record Container {
...
module User {
  ...
  use definesTertiary except Container;

  proc main() {
    var x = new Container(new OtherType(2));
    x.tertiaryMethod(); // Thus, this should resolve
  }
}
