...
proc main() {
  ...
  printBlockLocaleAssignments( reshape(rootLocale.getLocales(), {1..numLocales, 1..1}) );

  writeln( "Block by column:" );
  printBlockLocaleAssignments( reshape(rootLocale.getLocales(), {1..1, 1..numLocales}) );
  ...
  const L : [D] locale = [(i,j) in D] rootLocale.getLocale((i * colcount + j) % numLocales);
  ...
  const cyclicLocales : [0..#oversubscribed] locale =
    [loc in 0..#oversubscribed] rootLocale.getLocale(loc % numLocales);
  ...
  var weirdLocales : [weirdDomain] locale =
    [(i,j) in weirdDomain] rootLocale.getLocale((i * colcount + j) % numLocales );
  // then update center to point elsewhere
  [ij in {2..5, 2..5} ] weirdLocales(ij) = rootLocale.getLocale(0); // change center
