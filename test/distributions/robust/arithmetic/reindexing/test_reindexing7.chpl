...
writeln("TA1D: ", foo({(...Dom1D.dims())}, TA1D.reindex(Dom1D), TA1D), " errors");

const TD2D: domain(2) = Space2.translate(-o5, -o5);
var TA2D: [TD2D] int;
for e in TA2D do e = next();
writeln("TA2D: ", foo({(...Dom2D.dims())}, TA2D.reindex(Dom2D), TA2D), " errors");

const TD3D: domain(3) = Space3.translate(-o5, -o5, -o5);
var TA3D: [TD3D] int;
for e in TA3D do e = next();
writeln("TA3D: ", foo({(...Dom3D.dims())}, TA3D.reindex(Dom3D), TA3D), " errors");

const TD4D: domain(4) = Space4.translate(-o5, -o5, -o5, -o5);
var TA4D: [TD4D] int;
for e in TA4D do e = next();
writeln("TA4D: ", foo({(...Dom4D.dims())}, TA4D.reindex(Dom4D), TA4D), " errors");
...
writeln("TA2D32: ", foo({(...Dom2D32.dims())}, TA2D32.reindex(Dom2D32), TA2D32), " errors");
