// streamQS.chpl (qthreads, sublocales)
//
// One task per core, but with a variable number of shepherds.
//
...
proc main() {
  ...
  coforall loc in rootLocale.getLocales() do on loc {
    ...
    const nchunks = chpl_numCoresOnThisLocale() * tasksPerCore;
    ...
    local {
      ...
      for trial in 1..numTrials {                        // loop over the trials
        ...
        coforall i in 0..#nchunks do
          on here.getChild(1 + i / chunks_per_child) do {
            var chunk = getChunk(i, nchunks, m);
    ...
    //
    nodeTimes[here.id] = min reduce trialTime;
...
{
  var start = (nChunk * problemSize) / chunkCount + 1;
  var end = ((nChunk+1) * problemSize) / chunkCount;
  return start..end;
}
