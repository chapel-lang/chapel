...
extern proc chpl_task_getSubloc(): chpl_sublocID_t;
{
  ...
  for loc in Locales {
    ...
    forall a in A do
      if beNoisy then writeln((a, chpl_task_getSubloc()));
  }
}
...
config const n = 4;
{
  ...
  var R = 1..n;
  forall r in R do
    if beNoisy then writeln((r, chpl_task_getSubloc()));
}

{
  ...
  forall (r0,r1) in zip(R0,R1) do
    if beNoisy then writeln((r0, r1, chpl_task_getSubloc()));
}

{
  ...
  var D = {1..n};
  forall d in D do
    if beNoisy then writeln((d, chpl_task_getSubloc()));
}

{
  ...
  forall (d0,d1) in zip(D0,D1) do
    if beNoisy then writeln((d0, d1, chpl_task_getSubloc()));
}
...

{
  ...
  forall (a,b) in zip(A,B) do
    if beNoisy then writeln((a, b, chpl_task_getSubloc()));
}
