...
proc main() {
  ...
  var ownCc = new owned ConcreteClass(10);
  var cc = ownCc.borrow();
  promote(cc);

  var ownGcTwo = new owned GenericClass(int, 2);
  var gcTwo = ownGcTwo.borrow();
  promote(gcTwo);

  var ownGcOne = new owned GenericClass(int, 1);
  var gcOne = ownGcOne.borrow();
  promote(gcOne);
}
