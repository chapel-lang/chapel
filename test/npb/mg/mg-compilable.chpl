...
const checksum: [S..O] float  = (/0.0000530770700573,
                                  ...
                                  0.0/);
implement these as functions for the time being: */

function probSize(c: classVals) {
  select (c) {
    when S do return 32;
    when W do return 64;
...
function iterations(c: classVals) {
  select (c) {
    when S do return 4;
    when W do return 40;
    when A do return 4;
