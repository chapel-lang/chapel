...
class ForceEAM : Force  {
  ...
  proc compute(store : bool) {
    ...
    virial = 0.0;
    forall (b,c) in zip(Bins, Count) {
      for a in b[1..c] {
        a.f = (0.0,0.0,0.0);
      }
    }

    for (b,p,c,r) in zip(Bins,Pos, Count, binSpace) {
      const exist = 1..c;
    ...
    forall (b,p,c,rbin) in zip(Bins, Pos, Count, binSpace) {
      const exist = 1..c;
      for (a,x,i) in zip(b[exist],p[exist],exist) {
        var fx, fy, fz : real;
...
class ForceLJ : Force {
  ...
  proc compute(store : bool) : void {
    ...
    forall (b,p,c,r) in zip(Bins, Pos, Count,binSpace) {
