...
/*
  ...
  One pattern that can be particularly surprising is:
...
/*
  ...
  Though the `Locales` array represents the set of distributed locales
  on which the program is running, it is represented as a local array
  on each locale.  As a result, the parallelism generated by this loop
  structure will once again be based on the number of local cores,
  implying that if ``numLocales >> here.maxTaskPar``, you will not end
  up executing on all the locales simultaneously.  The fix is to use
  a coforall-loop:
