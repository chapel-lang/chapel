...
/* Often, it is desirable for the contents of a module to be accessed from
   ...
   First, if only one symbol defined in the module is desired, and it is desired
   only once, the symbol can be referenced explicitly - this is done using the
...
{
  /* If many of the module's symbols are desired, or the same symbol is desired
     multiple times, then it is best to utilize what is known as a "use
     statement".
  */
  use modToUse;
  ...
  */
  ...
  /* A use statement brings all of the visible symbols of a module into the
     scope which defines the use statement.  These symbols may then be accessed
...
/* Once the scope defining a use statement is left, however, the symbols must be
   accessed with a prefix.  The following line would cause an error, because the
   previous use of the module 'modToUse' occurred within the curly braces above:
*/
// var twiceFoo = 2 * foo;
...
var bar = false;
{
  /* The symbols provided by a use statement are considered at an inner scope to
     the symbols defined outside of the scope where the use statement applies.
     Thus, if another bar were defined outside of these curly braces, and an
     access was attempted, the compiler would find the bar from modToUse, rather
     than the outer bar.
  */
...

{
   ...
   */
  use modToUse, Conflict only other;
...
{
  /* If every symbol other than the one which causes a conflict is desired, you
     can specify the symbols to exclude via an 'except' list.
   */
  use Conflict, modToUse except bar;
...

{
  /* If both symbols which conflict are desired, or if the use causes symbols
     to be shadowed which are necessary, you can choose to rename a symbol when
     including it.
   */
...
//   - where the file is named the same as the module and it is in the same
//     directory
//   - where the file is not named the same as the module
//   - where the file is not in the same directory
//   - where the file is a standard module
//   - if the file also defines a main method, or is also mostly top level
