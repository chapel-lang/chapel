...
def get():
    ...
    if val is None:
        ...
        if not system_library_available():
            error(
                "CHPL_UNWIND=system specified but libunwind not found on system.",
                ValueError,
            )

    if osx and val == "bundled":
        error(
            "Using CHPL_UNWIND=bundled is not supported on Mac OS X."
...
def get_link_args():
    ...
    if unwind_val == "bundled":
        ...
        args = third_party_utils.pkgconfig_get_system_link_args(
            "libunwind", static=True
        )
