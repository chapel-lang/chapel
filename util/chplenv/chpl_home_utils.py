...
def get_chpl_runtime_incl():
    ...
    if prefix is None:
        ...
        default = os.path.join(
            prefix, "lib", "chapel", chpl_vers, "runtime", "include"
        )
...
def get_chpl_runtime_lib():
    ...
    if prefix is None:
        ...
        assert chpl_vers is not None
        default = os.path.join(
            prefix, "lib", "chapel", chpl_vers, "runtime", "lib"
        )
...
def get_chpl_third_party():
    ...
    if prefix is None:
        ...
        assert chpl_vers is not None
        default = os.path.join(
            prefix, "lib", "chapel", chpl_vers, "third-party"
        )
...
def get_prefix_install_prefix():
    ...
    has_frontend = os.path.exists(frontend_path) and os.path.isdir(
        frontend_path
    )
...
def _main():
    ...
    parser.add_option(
        "--home", action="store_const", dest="func", const=get_chpl_home
    )
    parser.add_option(
        "--third-party",
        action="store_const",
        dest="func",
        const=get_chpl_third_party,
    )
    parser.add_option(
        "--chpldeps", action="store_const", dest="func", const=get_chpldeps
    )
    parser.add_option(
        "--chpldeps-chapel-py",
        action="store_const",
    ...
    parser.add_option(
        ...
        action="store_const",
        dest="func",
        const=using_chapel_module,
    )
    parser.add_option(
        "--configured-install-lib-prefix",
        action="store_const",
        dest="func",
        const=get_chpl_configured_install_lib_prefix,
    )
    parser.add_option(
        "--runtime-lib",
        action="store_const",
        dest="func",
        const=get_chpl_runtime_lib,
    )
    parser.add_option(
        "--runtime-incl",
        action="store_const",
        dest="func",
        const=get_chpl_runtime_incl,
    )
