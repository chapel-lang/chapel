...
def get_bundled_llvm_dir():
    ...
    llvm_subdir = os.path.join(
        chpl_third_party, "llvm", "install", llvm_target_dir
    )
    return llvm_subdir
...
def get_bundled_llvm_support_only_dir():
    ...
    llvm_subdir = os.path.join(
        chpl_third_party, "llvm", "install", "support-only-" + llvm_target_dir
    )
    return llvm_subdir
...
def get_clang_version(clang_command, short=False):
    ...
    if clang_command != "none" and clang_command != None:
        ...
        exists, returncode, got_out, _ = try_run_command(
...
def check_llvm_config(llvm_config):
    ...
    if not version_ok:
        s = "LLVM version {0} is not one of the supported versions: {1}".format(
            got_version, llvm_versions_string()
        )
...
def check_llvm_packages(llvm_config):
    ...
    if os.path.isdir(include_dir):
        llvm_header = os.path.join(
            include_dir, "llvm", "Config", "llvm-config.h"
        )
        ...
        if not clang_include_ok:
            ...
            clang_header2 = os.path.join(
                usr_include, "clang", "Basic", "Version.h"
            )
...
def find_system_llvm_config():
    ...
    for vers in llvm_versions():
        ...
        if homebrew_prefix:
            ...
            paths.append(
                homebrew_prefix + "/opt/llvm@" + vers + ".0/bin/llvm-config"
            )
            paths.append(
                homebrew_prefix + "/opt/llvm@" + vers + "/bin/llvm-config"
            )
...
def get_llvm_version():
    if os.environ.get("CHPL_LLVM_VERSION") is not None:
        warning(
            "Setting CHPL_LLVM_VERSION has no effect. Unset CHPL_LLVM_VERSION and set CHPL_LLVM_CONFIG to the desired 'llvm-config' path instead."
        )
...
def validate_llvm_config():
    ...
    if llvm_val == "system":
        if llvm_config == "" or llvm_config == "none":
            error(
                "CHPL_LLVM=system but could not find an installed LLVM"
    ...
    if llvm_support_val == "system":
        if llvm_config == "" or llvm_config == "none":
            error(
                "CHPL_LLVM_SUPPORT=system but could not find an installed "
    ...
    if llvm_val == "system":
        ...
        if not bindir.startswith("/nix/store"):
            if not (bindir and os.path.isdir(bindir)):
                error(
                    "llvm-config command {0} provides missing bin dir {1}".format(
                        llvm_config, bindir
                    )
                )
        ...
        if clang_c == "":
            error(
                "Could not find clang with the same version as "
                "CHPL_LLVM_CONFIG={}. Please try setting CHPL_TARGET_CC.".format(
                    llvm_config
                )
            )
        if clang_cxx == "":
            error(
                "Could not find clang++ with the same version as "
                "CHPL_LLVM_CONFIG={}. Please try setting CHPL_TARGET_CXX.".format(
                    llvm_config
                )
            )
...
def get_overridden_llvm_clang(lang):
    ...
    if res is not None:
        ...
        exists, returncode, out, _ = try_run_command(
            [res[0], "-E", "-x", "c", "-"], macro
        )
        if exists and returncode == 0 and out:
            ...
            if macro in out:
                error(
                    "'{}' is not clang and cannot be used with LLVM".format(
                        res[0]
                    )
                )
        else:
...
def has_compatible_installed_llvm():
    ...
    if llvm_config:
        ...
        if ok:
            ...
            if is_system_clang_version_ok(
                clang_c_command
            ) and is_system_clang_version_ok(clang_cxx_command):
                return True
...
def get():
    ...
    if not compatible_platform_for_llvm():
        if llvm_val != "none" and llvm_val != "unset":
            warning(
                "CHPL_LLVM={0} is not compatible with this "
...
def get_gcc_install_dir():
    ...
    if gcc_dir:
        if not flag_supported:
            llvm_version = get_llvm_version()
            warning(
                "This LLVM / clang version {0} is too old to use "
...
def _clang_verbose_on_sys_basic(clang="clang"):
    ...
    out, err = run_command([clang, "-###", cfile], stdout=True, stderr=True)
    out += err
    return out
...
def get_sysroot_linux_args():
    ...
    if stdout:
        ...
        if found:
            ...
            if not (
                dyn_linker.startswith("/lib/")
                or dyn_linker.startswith("/lib64/")
            ):
...
def get_system_llvm_built_sdkroot():
    ...
    if llvm_val == "system" and host_platform == "darwin":
        ...
        include_dir = run_command([llvm_config, "--includedir"]).strip()
        if os.path.isdir(include_dir):
            clang_config = os.path.join(
                include_dir, "clang", "Config", "config.h"
            )
...
def _determine_gcc_flag_to_use():
    ...
    flag_to_use = None
    if (
        overrides.get("CHPL_LLVM_GCC_INSTALL_DIR") is not None
        and overrides.get("CHPL_LLVM_GCC_PREFIX") is not None
    ):
        # the user set both, warn
        base_msg = (
            "Both CHPL_LLVM_GCC_INSTALL_DIR and CHPL_LLVM_GCC_PREFIX are set."
        )
        if is_gcc_install_dir_supported():
            flag_to_use = "CHPL_LLVM_GCC_INSTALL_DIR"
            warning(
                "{} CHPL_LLVM_GCC_INSTALL_DIR will be used.".format(base_msg)
            )
        else:
...
def get_clang_basic_args(clang_command):
    ...
    if flag_to_use:
        ...
        any(arg.startswith("--sysroot") for arg in clang_cfg_args)
        or any(arg.startswith("--gcc-install-dir") for arg in clang_cfg_args)
        or any(arg.startswith("--gcc-toolchain") for arg in clang_cfg_args)
    ):
...
def get_clang_prgenv_args():
    ...
    if chpl_compiler.get_prgenv_compiler() != "none":
        ...
        # files available.
        os.environ["PE_PKGCONFIG_PRODUCTS"] = "PE_CHAPEL:" + os.environ.get(
...
def get_clang_gcc_system_include_directories(flag, lang):
    ...
    if stdout:
        ...
        ignoring_regex = re.compile(
            r"(?:ignoring nonexistent)|(?:as it is a non-system)"
        )
        collecting = False
        for line in lines:
...
def filter_llvm_config_flags(llvm_val, flags):
    ...
    for flag in flags:
        if (
            flag == "-DNDEBUG"
            or flag == "-fPIC"
            or flag == "-gsplit-dwarf"
            or flag.startswith("-O")
            or flag == "-pedantic"
            or flag == "-Wno-class-memaccess"
            or (darwin and gnu and flag.startswith("-stdlib="))
            or (cygwin and flag == "-std=c++17")
            or flag == "-std=c++14"
        ):
...
def get_host_compile_args():
    ...
    if llvm_support_val == "system":
        ...
        if is_included_llvm_built(llvm_val):
            ...
            cxxflags = run_command([llvm_config, "--cxxflags"])
            bundled.extend(
                filter_llvm_config_flags("bundled", cxxflags.split())
            )
...
def compute_host_link_settings():
    ...
    if llvm_support_val == "system":
        ...
        if is_included_llvm_built(llvm_val):
            ...
            ldflags = run_command(
                [llvm_config, "--ldflags", "--libs"] + llvm_components
            )

            bundled.extend(ldflags.split())

            ldflags = run_command(
                [llvm_config, "--system-libs"] + llvm_components
            )
