...
def get(flag="target"):
    if flag == "network":
        ...
        if not atomics_val:
            ...
            error(
                "CHPL_NETWORK_ATOMICS=%s is incompatible with CHPL_COMM=%s"
                % (atomics_val, comm_val)
            )
    elif flag == "target":
        ...
        if not atomics_val:
            ...
            is_intel = (
                compiler_val == "intel" or compiler_val == "cray-prgenv-intel"
            )
            if compiler_val in ["gnu", "cray-prgenv-gnu", "mpi-gnu"]:
                ...
                if version >= CompVersion("5.0"):
                    ...
                    "4.1"
                ) and not platform_val.endswith("32"):
                    atomics_val = "intrinsics"
            elif is_intel:
                atomics_val = "intrinsics"
        ...
        check_valid_var(
            "CHPL_ATOMICS", atomics_val, ["cstdlib", "intrinsics", "locks"]
        )
...
def _main():
    ...
    parser.add_option(
        "--network", dest="flag", action="store_const", const="network"
    )
