...
def gpu_compiler_basic_compile(compiler: str, lang: str):
    ...
    _, _, stdout, _ = try_run_command(
        [compiler, "-v", "-c", "-x", lang, "-", "-o", "/dev/null"],
...
def determine_gpu_type():
    typesFound = [
        val for (val, gpu_type) in GPU_TYPES.items() if which(gpu_type.compiler)
    ]
    if len(typesFound) == 1:
        return typesFound[0]
...
def get_arch():
    ...
    if arch:
        ...
        if len(arch.split(",")) > 1 and gpu_type != "nvidia":
            error(
                "Multi-target builds are only supported for the 'nvidia' GPU type."
            )
            arch = "error"
    ...
    if arch != "":
        ...
        error(
            "CHPL_GPU={} requires also setting CHPL_GPU_ARCH. "
...
def get_sdk_path(for_gpu):
    ...
    if chpl_sdk_path:
        if for_gpu == get() and not validate_path(chpl_sdk_path):
            _reportMissingGpuReq(
                "{}='{}' does not exist. Make sure the toolkit path is correct.".format(
                    gpu.sdk_path_env, chpl_sdk_path
                )
            )
            return "error"
    ...
    if sdk_path:
        if not validate_path(sdk_path):
            _reportMissingGpuReq(
                "Inferred {} toolkit is not valid. Try setting {}.".format(
                    gpu.runtime_impl, gpu.sdk_path_env
                )
            )
            return "error"
        return sdk_path
        ...
        _reportMissingGpuReq(
            "Can't infer {} toolkit from '{}'. Try setting {}.".format(
                gpu.runtime_impl, gpu.compiler, gpu.sdk_path_env
            )
        )
        return "error"
    else:
...
def get_gpu_mem_strategy():
    ...
    if memtype:
        ...
        if memtype not in valid_options:
            error(
                "CHPL_GPU_MEM_STRATEGY must be set to one of: %s"
                % ", ".join(valid_options)
            )
        return memtype
...
def get_cuda_libdevice_path():
    if get() == "nvidia":
        ...
        if not libdevice:
            _reportMissingGpuReq(
                "Can't determine libdevice path from {}".format(compiler)
            )
            return "error"
        return libdevice
    return "none"
...
def get_rocm_llvm_path():
    if get() == "amd":
        ...
        if not llvm_path:
            _reportMissingGpuReq(
                "Can't determine AMD LLVM path from {}".format(compiler)
            )
            return "error"
...
def get_rocm_amdgcn_path():
    if get() == "amd":
        ...
        if not amdgcn_path:
            _reportMissingGpuReq(
                "Can't determine amdgcn path from {}".format(compiler)
            )
            return "error"
        return amdgcn_path
    return "none"
...
def _validate_rocm_llvm_version_impl(gpu: gpu_type):
    ...
    if major_version in ("5",) and chpl_llvm.get() == "bundled":
        ...
        if int(minor_version) < 3:
            ...
            if (
                chpl_llvm.get() != "bundled"
                and os.environ.get("CHPL_LLVM_65188_PATCH", "0") != "1"
            ):
                error(
                    "Cannot target AMD GPUs with ROCm 6.0-6.2 without CHPL_LLVM=bundled"
                )
        else:
...
def _validate_cuda_version_impl():
    ...
    if is_ver_in_range(cuda_version, "12", "13"):
        ...
        if llvm == "system":
            ...
            llvm_ver_str = chpl_llvm.get_llvm_config_version(
                llvm_config
            ).strip()
...
def get_sdk_version():
    ...
    if os.environ.get("CHPL_GPU_SDK_VERSION") is not None:
        ...
        if gpu.real_gpu:
            ...
            fixstr = " and set {} to the desired version-specific path instead.".format(
                var
            )
        warning(
            "Setting CHPL_GPU_SDK_VERSION has no effect. Unset CHPL_GPU_SDK_VERSION"
            + fixstr
        )
...
def validate(chplLocaleModel):
    ...
    if gpu_val == "none":
        error(
            "CHPL_GPU cannot be set to 'none' when using the 'gpu' locale model."
        )
    ...
    if chpl_compiler.get("target") != "llvm":
        error(
            "The 'gpu' locale model can only be used with "
