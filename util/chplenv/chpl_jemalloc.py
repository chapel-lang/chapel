...
def get(flag="target"):
    ...
    if flag == "target":
        if chpl_jemalloc:
            warning(
                "CHPL_JEMALLOC is deprecated, please unset it and use CHPL_TARGET_JEMALLOC"
            )
        if chpl_target_jemalloc:
            jemalloc_val = chpl_target_jemalloc
            if chpl_jemalloc:
                warning(
                    "CHPL_JEMALLOC and CHPL_TARGET_JEMALLOC are both set, "
    ...
    if mem_val == "jemalloc" and jemalloc_val == "none":
        error(
            "{0} must not be 'none' when {1} is jemalloc".format(
                var_name, mem_var_name
            )
        )
    elif mem_val != "jemalloc" and jemalloc_val != "none":
        error(
            "{0} must be 'none' when {1} is not jemalloc".format(
                var_name, mem_var_name
            )
        )
...
def get_jemalloc_config_file(flag):
    ...
    install_path = third_party_utils.get_bundled_install_path(
        "jemalloc", ucp=ucp
    )
...
def get_compile_args(flag):
    ...
    if jemalloc_val == "bundled":
        ...
        return third_party_utils.get_bundled_compile_args(
            "jemalloc", ucp=ucp_val
        )
