...
def get(flag="target"):
    ...
    if flag in ("target", "host"):
        ...
        if mimalloc is None:
            ...
            if mem == "mimalloc" and mimalloc == "none":
                ...
                error(
                    "{0} must not be 'none' when {1} is 'mimalloc'".format(
                        var_name(flag), memvar
                    )
                )
    ...
    if (
        mimalloc == "system"
        and not third_party_utils.pkgconfig_system_has_package("mimalloc")
    ):
        third_party_utils.could_not_find_pkgconfig_pkg(
            "mimalloc", var_name(flag)
        )
