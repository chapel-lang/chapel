...
NOPATH = set(
    ["nopath"]
)  # for variables to be skipped for --path or --only-path
DEFAULT = set(["default"])
...
def compute_all_values():
    ...
    # adjustments
    ENV_VALS["CHPL_RUNTIME_CPU"] = chpl_cpu.get(
        "target",
...
def compute_internal_values():
    ...
    ENV_VALS["CHPL_RUNTIME_SUBDIR"] = printchplenv(
        set(["runtime"]), print_format="path"
    ).rstrip("\n")
    ENV_VALS["CHPL_LAUNCHER_SUBDIR"] = printchplenv(
        set(["launcher"]), print_format="path"
    ).rstrip("\n")
    ENV_VALS["CHPL_COMPILER_SUBDIR"] = printchplenv(
        set(["compiler"]), print_format="path"
    ).rstrip("\n")
    ...
    ENV_VALS["  CHPL_HOST_JEMALLOC_UNIQ_CFG_PATH"] = (
        chpl_jemalloc.get_uniq_cfg_path("host")
    )
    ENV_VALS["  CHPL_TARGET_JEMALLOC_UNIQ_CFG_PATH"] = (
        chpl_jemalloc.get_uniq_cfg_path("target")
    )
    ENV_VALS["  CHPL_HOST_MIMALLOC_UNIQ_CFG_PATH"] = (
        chpl_mimalloc.get_uniq_cfg_path("host")
    )
    ENV_VALS["  CHPL_TARGET_MIMALLOC_UNIQ_CFG_PATH"] = (
        chpl_mimalloc.get_uniq_cfg_path("target")
    )
    ENV_VALS["  CHPL_LIBFABRIC_UNIQ_CFG_PATH"] = (
    ...
    ENV_VALS["  CHPL_PE_CHPL_PKGCONFIG_LIBS"] = (
        chpl_llvm.gather_pe_chpl_pkgconfig_libs()
    )
    ...
    ENV_VALS["  CHPL_TARGET_BUNDLED_RUNTIME_LINK_ARGS"] = " ".join(
        tgt_runtime_link[0]
    )
    ENV_VALS["  CHPL_TARGET_BUNDLED_PROGRAM_LINK_ARGS"] = " ".join(
        tgt_program_link[0]
    )
    ENV_VALS["  CHPL_TARGET_SYSTEM_RUNTIME_LINK_ARGS"] = " ".join(
        tgt_runtime_link[1]
    )
    ENV_VALS["  CHPL_TARGET_SYSTEM_PROGRAM_LINK_ARGS"] = " ".join(
        tgt_program_link[1]
    )

    static_args = compile_link_args_utils.compute_use_static_runtime_link_args(r_subdir)
    ENV_VALS['  CHPL_TARGET_USE_STATIC_RUNTIME_LINK_ARGS'] = " ".join(static_args)

    shared_args = compile_link_args_utils.compute_use_shared_runtime_link_args(r_subdir)
    ENV_VALS['  CHPL_TARGET_USE_SHARED_RUNTIME_LINK_ARGS'] = " ".join(shared_args)

    static_path = compile_link_args_utils.compute_static_runtime_lib_path(r_subdir)
    ENV_VALS['  CHPL_TARGET_STATIC_RUNTIME_LIB_PATH'] = static_path

    shared_path = compile_link_args_utils.compute_shared_runtime_lib_path(r_subdir)
...
def printchplenv(
    contents, print_filters=None, print_format="pretty", only=None, raw=False
):
    global CHPL_ENVS
    ...
    if not ENV_VALS.items:
        raise KeyError(
            "ENV_VALS must be populated with compute_*_values()"
...
def diagnose_missing_library(lib_type):
    ...
    if found_error:
        ...
        for var in variables:
            ...
            varname = var[0].strip()
            print(
                "There is no {} for '{}={}'".format(lib_type, varname, var[1])
            )
            others = os.listdir(current_path)
            options = [
                o if not shortname else o.removeprefix("{}-".format(shortname))
                for o in others
            ]
            # filter out . files
            ...
            if env_set(varname):
                ...
                if chplconfig_path:
                    print(
                        "This variable is set in '{}', consider removing it.".format(
                            chplconfig_path
                        )
                    )
                else:
                    print(
                        "This variable is set in a chplconfig, consider removing it."
                    )
            else:
                print(
                    "This variable was inferred from the current system and currently set variables:"
                )
                affecting_env = printchplenv(
                    set(
                        [
                            "runtime",
                            "launcher",
                    ...
                    print_filters=set(["overrides", "anonymize"]),
...
def parse_args():
    ...
    parser.add_option(
        "--compiler", action="append_const", dest="content", const="compiler"
    )
    parser.add_option(
        "--runtime", action="append_const", dest="content", const="runtime"
    )
    parser.add_option(
        "--launcher", action="append_const", dest="content", const="launcher"
    )
    parser.add_option(
        "--internal", action="append_const", dest="content", const="internal"
    )
    ...
    parser.add_option(
        "--anonymize", action="append_const", dest="filter", const="anonymize"
    )
    parser.add_option(
        "--overrides", action="append_const", dest="filter", const="overrides"
    )
    parser.add_option(
        "--only-path", action="append_const", dest="filter", const="only-path"
    )
    ...
    parser.add_option(
        "--pretty", action="store_const", dest="format", const="pretty"
    )
    parser.add_option(
        "--simple", action="store_const", dest="format", const="simple"
    )
    parser.add_option(
        "--make", action="store_const", dest="format", const="make"
    )
    parser.add_option(
        "--cmake", action="store_const", dest="format", const="cmake"
    )
    parser.add_option(
        "--path", action="store_const", dest="format", const="path"
    )
    parser.add_option(
        "--bash", action="store_const", dest="format", const="bash"
    )
    parser.add_option("--csh", action="store_const", dest="format", const="csh")
    parser.add_option(
        "--value", action="store_const", dest="format", const="value"
    )
    ...
    parser.add_option(
        "--verify-verbose", action="store_true", dest="verify_verbose"
    )
...
def main():
    ...
    if (
        "internal" in contents
        or options.verify
        or options.verify_verbose
        or options.diagnose_lib
    ):
    ...
    if options.verify or options.verify_verbose:
        ...
        success, reason = chplenv_verify.verify(
            ENV_VALS, verbose=options.verify_verbose
        )
        if not success:
