...
def get():
    ...
    if comm_val == "ugni":
        warning(
            "The 'ugni' communication layer is deprecated and will be removed in a future release"
        )
...
def _is_ibv():
    ...
    return (
        which("ibstat")
        and os.path.exists("/dev/infiniband/uverbs0")
        and any(dev.startswith("ib") for dev in os.listdir("/sys/class/net/"))
    )
...
def _is_cxi():
    ...
    return (
        which("cxi_stat")
        and any(dev.startswith("hsn") for dev in os.listdir("/sys/class/net/"))
        and os.path.exists("/sys/class/cxi/")
        and any(dev.startswith("cxi") for dev in os.listdir("/dev/"))
    )
