    ...
    if sanitizers_val != "none":
        ...
        if flag == "exe" and chpl_compiler.get(flag="target") == "llvm":
            ...
            if llvm_vers in ("14", "15"):
                error(
                    "LLVM {0} is not supported with sanitizers".format(
                        llvm_vers
                    )
                )
            if sanitizers_val != "address":
                error(
                    "CHPL_SANITIZE_EXE={0} is not supported with CHPL_TARGET_COMPILER=llvm".format(
                        sanitizers_val
                    )
                )
...
def _main():
    ...
    parser.add_option(
        "--exe", dest="flag", action="store_const", const="exe", default=""
    )
