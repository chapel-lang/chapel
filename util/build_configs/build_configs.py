...
class Dimension(object):
    ...
    def __init__(
        self, name, var_name, values=None, default=None, help_text=None
    ):
    ...
    def validate(self):
        ...
        if not self.name and self.name != self.name.lower():
            raise ValueError(
                "name must be a lower case string: {0}".format(self.name)
            )
        if not self.var_name or self.var_name != self.var_name.upper():
            raise ValueError(
                "var_name must an upper case string: {0}".format(self.var_name)
            )
        if not isinstance(self.help_text, basestring):
            raise ValueError(
                "help_text must be a string: {0}".format(self.help_text)
            )
...
class Config(object):
    ...
    def get_env(self, orig_env):
        ...
        for dim in Dimensions:
            ...
            if (
                not new_env_var
                or not isinstance(new_env_var, basestring)
                or new_env_var == "UNSET"
            ):
...
def main():
    ...
    config_count_str = "{0} configuration{1}".format(
        len(build_configs), "s" if len(build_configs) > 1 else ""
    )
    ...
    def list_config_names():
        ...
        for i, build_config in enumerate(build_configs):
            ...
            if not build_config_name:
                build_config_name = "None"
            names.append(
                "%3d / %3d  %s" % (i + 1, len(build_configs), build_config_name)
            )
        return names
    ...
    if make_logfile:
        print(
            "\n[BUILD_CONFIGS] CHPL_HOME={0}".format(chpl_home),
            file=make_logfile,
        )
        print(
            "\n[BUILD_CONFIGS] Building {0}".format(config_count_str),
            file=make_logfile,
        )
...
def get_chpl_misc(opts, args, build_env):
    ...
    if chpl_home_env:
        ...
        if chpl_home_env != chpl_home_opt:
            logging.warning(
                "Resetting build_env[CHPL_HOME],\nfrom\t{0}\nto\t{1}".format(
                    chpl_home_env, chpl_home_opt
                )
            )
    ...
    if result:
        logging.error(
            "From {0}\n{1}\n{2}\nexit code {3}".format(
                chplenv_cmd, error, output, result
            )
        )
        sys.exit(2)
    if error:
    ...
    if result or error:
        logging.error(
            "From {0}\n{1}\n{2}\nexit code {3}".format(
                chpl_make_py, error, output, result
            )
        )
        sys.exit(2)
    if output:
    ...
    if opts.setenv:
        ...
        dryrun_cmd = 'bash {0} -c "source {1} 2>&1 && {2}"'.format(
            x, setenv, dryrun_cmd
        )
    else:
        setenv = None
...
def get_configs(opts):
    ...
    for config_num, config_tuple in enumerate(config_strings):
        ...
        if (
            ...
            and (
                config.target_compiler == "cray"
                or config.host_compiler == "cray"
            )
    ...
    if libunwind_hack_configs:
        logging.warning(
            "Replacing unwind=bundled with unwind=none to work around bundled build failure on CCE aarch64. The following configs are affected: {0}".format(
                ", ".join(map(str, libunwind_hack_configs))
            )
        )
    ...
    if dry_run:
        ...
        with elapsed_time("Config number {0}".format(counter)):
            result, output, error = check_output(
                dryrun_cmd, chpl_home, build_env
            )
        if result or error:
            logging.warning(
                "Errors/Warnings from printchplenv, config {0}\n{1}\n{2}\nExit code {3}".format(
                    build_config_name, output, error, result
                )
            )
        elif verbose:
            logging.debug(
                "Results from printchplenv, config {0}\n{1}\n{2}\nExit code {3}".format(
                    build_config_name, output, error, result
                )
            )
        make_result = result
        ...
        if make_logfile:
            ...
            print(
                "[BUILD_CONFIGS] DRY-RUN: printchplenv command:\n\t{0}\n".format(
                    dryrun_cmd
                ),
                file=make_logfile,
            )
            result, output, error = check_output(
                dryrun_cmd, chpl_home, build_env, file=make_logfile
            )
    else:
        ...
        if make_logfile:
            print(
                "[BUILD_CONFIGS] Chapel make command:\n\t{0}\n".format(
                    make_cmd
                ),
                file=make_logfile,
            )
        with elapsed_time("Config number {0}".format(counter)):
            result, output, error = check_output(
                make_cmd, chpl_home, build_env, file=make_logfile
            )
        if result:
        ...
        if make_logfile:
            ...
            print(
                "[BUILD_CONFIGS] printchplenv command:\n\t{0}\n".format(
                    dryrun_cmd
                ),
                file=make_logfile,
            )
...
def elapsed_time(timer_name):
    ...
    logging.info("{0} finished in {1:.3f} seconds.".format(timer_name, elapsed))
...
def print_configs():
    ...
    for dim in Dimensions:
        print(
            "{name}\t{help_text}\n\t{var_name}".format(
                name=dim.name, var_name=dim.var_name, help_text=dim.help_text
            )
        )
        if dim.values:
            print("\t\tValues:")
...
def parse_config_value_callback(option, opt, value, parser, choices, **kwargs):
    ...
    for v in parsed_vals:
        if choices and v not in choices:
            parser.error(
                "option {0}: invalid choice: {1} (choose from {2})".format(
                    opt, v, choices
                )
            )
...
comm=gasnet either of these will work:
  ...
  %prog --comm=none --comm=gasnet"""
    ...
    parser.add_option(
        "--chpl-home", help="CHPL_HOME setting. (default: %default)"
    )
    parser.add_option(
        "-p",
        "--parallel",
    ...
    for dim in Dimensions:
        config_group.add_option(
            ...
            action="callback",
            type="string",
            callback=parse_config_value_callback,
            callback_args=(dim.values,),
            help=dim.help_text,
        )
