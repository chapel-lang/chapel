...
c_func_decl_regex = re.compile(
    r"(?P<returns>\w+)\s+(?P<name>\w+)\s*\(\s*(?P<arguments>(?:\w+\s+)?\w+(?:\s*\*\s*|\s+)\w+(?:\s*,\s*(?:\w+\s+)?\w+(?:\s*\*\s*|\s+)\w+)*)?\s*\)\s*;"
)
...
c_args_regex = re.compile(
    r"(?:(?P<modifier>const)\s+)?(?P<type>\w+)(?P<refdepth>(?:\s*\*+\s*)|\s+)(?P<name>\w+)"
)
...
doc_func_regex = re.compile(
    r"(?:(?P<type>(?:[\w\*]+)(?: +\w+)*)\s+)?(?:(?:SUBROUTINE)|(?:FUNCTION))\s+(?P<name>\w+)\(\s*(?P<arguments>(?:\w+\s*(?:,\s*\w+\s*)*)?)\s*\)"
)
...
doc_scalarargs_regex = re.compile(
    r"Scalar Arguments\s+\.\.\s+(?P<body>(?:.|\n)*?)\s+\.\."
)
doc_scalarargs_decls_regex = re.compile(
    r"(?P<type>(?:[\w\*]+)(?: +\w+)*) +(?P<names>\w+(?:\s*,(?:\s*\$\s*)?\s*\w+)*)"
)
...
doc_arrayargs_regex = re.compile(
    r"Array Arguments\s+\.\.\s+(?P<body>(?:.|\n)*?)\s+\.\."
)
doc_arrayargs_decls_regex = re.compile(
    r"(?P<type>(?:[\w\*]+)(?: +\w+)*) +(?P<names>\w+(?:\([\s\S]*?\))?(?:\s*,(?:\s*\$\s*)?\s*\w+(?:\([\s\S]*?\))?)*)"
)
doc_arrayargs_decls_names_dims_regex = re.compile(
    r"(?P<name>\w+)(?:\((?P<dimensions>.*?)\))?,"
)

doc_functionargs_regex = re.compile(
    r"Function Arguments\s+\.\.\s+(?P<body>(?:.|\n)*?)\s+\.\."
)
...
doc_args_regex = re.compile(
    r"\s+\\param\[(?P<intent>\w+(?:,\w+)*)\]\s+(?P<name>\w+)\s+\\verbatim\s+(?P<body>(?:[\s\S])+?)\s*\\endverbatim"
)
...
doc_args_typeinfo_regex = re.compile(
    r"(?P<name>\w+)\s+is\s+(?P<type>\w+)(?: (?P<array>array), dimension\s+(?P<dimensions>.*))?"
)
...
doc_args_dimensions_regex = re.compile(
    r"(\w+)[- ]by[- ](\w+)(?: coefficient)? matrix"
)
...
scalar_matrix_relation_regex = re.compile(
    r"(?:number\s+of\s+)?(?P<what>\w+)\s+(?:(?:of)|(?:in))\s+(?:the\s+)?(?:input\s+)?(?:(?:matrix)|(?:matrices)|(?:submatrix))?(?:\s+)?(?P<who>(?:(?:(?:\w+\( \w+ \))|(?:\w+))\s*)+)"
)
...
doc_purpose_regex = re.compile(
    r"\\par\s+Purpose:\s+=+\s+\\verbatim\s+(?P<body>(?:[\s\S]+?))\s+\\endverbatim"
)
...
func_name_group_regex = re.compile(
    r"^(?P<type>(?:(?:ds)|(?:zc)|[dszc]))(?P<config>\w\w)(?P<function>\w\w\w*)"
)
...
class CreateTreePass(Pass):
    ...
    def apply(xml_tree):
        ...
        print(
            ...
            selfname,
            ":",
            "Completed" if selfname.complete else "FAILED",
            "]\n",
        )
...
class LAPACKFunctionDefinePass(Pass):
    ...
    def apply(xml_tree):
        ...
        for file_node in file_nodes:
            ...
            sys.stdout.write(
                "%s ( %d : %d )                               \r"
                % (file_node.get("name"), file_count, len(file_nodes))
            )
            sys.stdout.flush()
...
class FuncPurposeDocPass(Pass):
    ...
    def apply(xml_tree):
        ...
        for proc_node in procs_node.findall("./procedure"):
            ...
            doc_node = text_node.find(
                "./file/[@name='" + proc_file_name + "']/documentation"
            )

            purpose_match = doc_purpose_regex.search(doc_node.text)
            purpose = (
                purpose_match.group("body")
    ...
    def apply(xml_tree):
        ...
        for proc_node in procs_node.findall("./procedure"):
            ...
            for arg_match in doc_args_regex.finditer(doc_node.text):
                ...
                arg_node = proc_node.find(
                    "./arguments-list/argument/[@name='" + arg_name + "']"
                )

                arg_node.set("intent", arg_match.group("intent"))
                dim_match = doc_args_dimensions_regex.search(
                    arg_match.group("body")
                )
                if dim_match != None:
                    arg_node.set(
                        "matrix-size",
                        dim_match.group(1) + "," + dim_match.group(2),
                    )

                SubElement(arg_node, "documentation").text = arg_match.group(
                    "body"
                )
...
class FuncArgsTypePass(Pass):
    ...
    def apply(xml_tree):
        ...
        for proc_node in procs_node.findall("./procedure"):
            ...
            if scalars != None:
                for line in doc_scalarargs_decls_regex.finditer(
                    scalars.group("body")
                ):
                    names_list = re.sub(
                        r"[\s$]", "", line.group("names")
                    ).split(",")
            ...
            if arrays != None:
                for line in doc_arrayargs_decls_regex.finditer(
                    arrays.group("body")
                ):
                    ...
                    for (
                        name_match
                    ) in doc_arrayargs_decls_names_dims_regex.finditer(
                        name_list
                    ):
                        ...
                        dimensions = (
                            name_match.group("dimensions")
                            if name_match.group("dimensions") != None
                            else ""
                        )
            ...
            if functions != None:
                for line in doc_functionargs_decls_regex.finditer(
                    functions.group("body")
                ):
                    names_list = re.sub(
                        r"[\s$]", "", line.group("names")
                    ).split(",")
...
class AssociateArgsToArrayPass(Pass):
    ...
    def apply(xml_tree):
      ...
      """
            ...
            arg_names = [
                arg.get("name")
                for arg in proc_node.findall("./arguments-list/argument")
            ]
            ...
            for arg_node in proc_node.findall("./arguments-list/argument"):
                ...
                if (
                    doc_node == None
                    or arg_node.get("semantic") != "scalar"
                    or arg_node.get("type").lower() != "integer"
                ):
                    continue
                ...
                for m in scalar_matrix_relation_regex.finditer(doc_node.text):
                    ...
                    if " and " in names:
                        names_list = [
                            name.strip() for name in names.split("and")
                        ]
                    else:
                        names_list = [names]
...
class LAPACKEFunctionDefinePass(Pass):
    dependencies = [
        CreateTreePass
    ]  # TODO include BaseLAPACKPass when the two need to meet
    complete = False
    ...
    def apply(xml_tree):
        ...
        for func_decl in c_func_decl_regex.finditer(lapacke_header):
            ...
            if (
                procs_node.find(
                    "./procedure/[@name='" + func_decl.group("name") + "']"
                )
                != None
            ):
...
class AssociateFunctionsLAPACKtoLAPACKEPass(Pass):
    ...
    def apply(xml_tree):
        ...
        for lapack_c_proc in lapack_c_procs.findall("./procedure"):
            ...
            lapack_f_proc = lapack_f_procs.find(
                "./procedure/[@name='" + base_name + "']"
            )
            ...
            SubElement(
                SubElementUnique(lapack_c_proc, "analogues"), "analogue"
            ).text = (
                "./LAPACK/procedures/procedure/[@name='"
                + lapack_f_proc.get("name")
                + "']"
            )
            SubElement(
                SubElementUnique(lapack_f_proc, "analogues"), "analogue"
            ).text = (
                "./LAPACKE/procedures/procedure/[@name='"
                + lapack_c_proc.get("name")
                + "']"
            )
    ...
    def apply(xml_tree):
        ...
        for c_proc in c_procs.findall("./procedure"):
            ...
            for f_arg in f_proc.findall("./arguments-list/argument"):
                ...
                # Note that it is totally possible to create the path string from the two atributes of the tag.
                # easier to create once here, instead of manytimes everywhere else.
                c_ana_node = SubElement(
                    SubElementUnique(c_arg, "analogues"), "analogue"
                )
                c_ana_node.text = (
                    "./LAPACK/procedures/procedure/[@name='"
                    + f_proc.get("name")
                ...
                f_ana_node = SubElement(
                    SubElementUnique(f_arg, "analogues"), "analogue"
                )
                f_ana_node.text = (
                    "./LAPACKE/procedures/procedure/[@name='"
                    + c_proc.get("name")
...
class ImportArgumentAssociationsPass(Pass):
    ...
    def apply(xml_tree):
        ...
        c_procs = xml_tree.find("./LAPACKE/procedures")
        pass_input = Pass.input_xml.find(
            "./pass/[@name='ImportArgumentAnaloguesPass']"
        )

        for in_proc in pass_input.findall("./procedure"):
            c_proc = c_procs.find(
                "./procedure/[@name='" + in_proc.get("name") + "']"
            )
            ...
            for in_arg in in_proc.findall("./argument"):
                ...
                f_arg = f_proc.find(
                    "./arguments-list/argument/[@name='"
                    + in_arg.get("substitution")
                    + "']"
                )
                # prettyprintxml( c_arg )
                if c_arg == None or f_arg == None:
                    raise GeneralPassFailure(
                        "Argument speficied in input not found in tree."
                        + c_proc.get("name")
                        + ":"
                        + c_arg.get("name")
                    )
...
class ImportLAPACKESemanticsIntentsPass(Pass):
    ...
    def apply(xml_tree):
        ...
        pass_input = Pass.input_xml.find(
            "./pass/[@name='ImportLAPACKESemanticsIntentsPass']"
        )
        for assign in pass_input.findall("./assign"):
            for arg in xml_tree.findall(assign.get("path")):
                ...
                if semantic == None and intent == None:
                    raise GeneralPassFailure(
                        "assignment contains no semantic or intent attributes"
                        + assign.get("path")
                    )
...
class TypeSubstitutionPass(Pass):
    ...
    def apply(xml_tree):
        ...
        pass_input = Pass.input_xml.find(
            "./pass/[@name='TypeSubstitutionPass']"
        )
...
class BaseTransformLAPACKEPass(Pass):
    dependencies = [
        BaseLAPACKEPass,
        TypeSubstitutionPass,
        FoldLAPACKSemanticsIntentsToLAPACKEPass,
        ImportLAPACKESemanticsIntentsPass,
        TypeSubstitutionPass,
        ImportArgumentTypePass,
    ]
    complete = False
    ...
    def apply(xml_tree):
        ...
        for proc in lapacke_procs.findall("./procedure"):
            ...
            for arg in proc.findall("./arguments-list/argument"):
                ...
                arg_semantic = (
                    arg.get("semantic") if arg.get("semantic") != None else ""
                )
                arg_intent = (
                    arg.get("intent") if arg.get("intent") != None else ""
                )
                ...
                if arg_refs == 0:
                    if arg_semantic == "array":
                        raise GeneralPassFailure(
                            "Attempted array semantic with 0 refdepth "
                            + proc_name
                            + " "
                            + arg.get("name")
                        )
...
class TranslateChapelKeywordsPass(Pass):
    ...
    def apply(xml_tree):
        ...
        pass_input = Pass.input_xml.find(
            "./pass/[@name='TranslateChapelKeywordsPass']"
        )
        ...
        global_pre = (
            ""
            if global_info == None or global_info.get("prefix") == None
            else global_info.get("prefix")
        )

        global_suf = (
            ""
            if global_info == None or global_info.get("suffix") == None
        ...
        for keyword in pass_input.findall("./keyword"):
            ...
            replacement = (
                ""
                if keyword.get("replacement") == None
                else keyword.get("replacement")
            )

            if replacement == "" and global_pre == "" and global_suf == "":
                raise GeneralPassFailure(
                    "If no global prefix or suffix is defined, a replacement for a symbol must be defined. ("
                    + symbol
                    + ")"
                )
            ...
            """
                if arg.get("matrix-size") != None:
                    ...
                    if removeRows != None and removeCols != None:
                        pass_through[rows] = (
                            "(if matrix_order == lapack_memory_order.row_major then "
                            + arg.get("name")
                            + ".domain.dim(1).size else "
                            + arg.get("name")
                            + ".domain.dim(2).size) : c_int"
                        )
                        pass_through[cols] = (
                            "(if matrix_order == lapack_memory_order.row_major then "
                            + arg.get("name")
                            + ".domain.dim(2).size else "
                            + arg.get("name")
                            + ".domain.dim(1).size) : c_int"
                        )
          ...
          """
            for arg in args_list:
                ...
                if arg.get("type") == "c_char":
                    pass_through[arg.get("name")] = (
                        arg.get("name") + ".byte(1) : c_char"
                    )
                ...
                if associate_array_str != None:
                    ...
                    if field == "rows":
                        ...
                        pass_through[arg.get("name")] = (
                            "(" + array + ".domain.dim(1).size) : c_int"
                        )
                    else:
                        raise GeneralPassFailure(
                            field
                            + " is not a recognized array association field"
                        )
...
class BaseChapelizePass(Pass):
    dependencies = [
        CreateChapelModuleTreePass,
        ChapelizeLAPACKEFunctionsPass,
        TranslateChapelKeywordsPass,
        ChapelerrificLAPACKEFunctionsPass,
    ]
    complete = False
...
class ImportTypeDefinesPass(Pass):
    ...
    def apply(xml_tree):
        ...
        pass_input = Pass.input_xml.find(
            "./pass/[@name='ImportTypeDefinesPass']"
        )
...
class ImportExternConstDefinesPass(Pass):
    ...
    def apply(xml_tree):
        ...
        pass_input = Pass.input_xml.find(
            "./pass/[@name='ImportExternConstDefinesPass']"
        )
...
class ImportEnumeratedTypeDefinesPass(Pass):
    ...
    def apply(xml_tree):
        ...
        pass_input = Pass.input_xml.find(
            "./pass/[@name='ImportEnumeratedTypeDefinesPass']"
        )
...
class BaseImportPass(Pass):
    dependencies = [
        ImportTypeDefinesPass,
        ImportExternConstDefinesPass,
        ImportEnumeratedTypeDefinesPass,
    ]
    complete = False
...
class ChapelModuleExternProcPass(Pass):
    ...
    def apply(xml_tree):
        ...
        for proc in module_procs.findall("./procedure"):
            ...
            proc_name = proc.get("name")
            basename = (
                proc_name.replace("LAPACK_", "").replace("LAPACKE_", "").upper()
            )

            lapack_node = xml_tree.find(
                "./LAPACK/procedures/procedure/[@name='" + basename + "']"
            )
    ...
    def apply(xml_tree):
        ...
        for proc in module_procs.findall("./procedure"):
            ...
            for pass_arg in ordered_args:
                call_args_producer.append(
                    SegmentProducer(
                        (
                            pass_arg.get("name")
                            if pass_arg.get("type") != "c_char"
                            else pass_arg.get("name") + ".byte(1) : c_char"
                        )
                    )
                )
            func_body.append(
                SegmentProducer(
                    ("return " if proc.get("return-type") != "void" else "")
                    + proc.get("name")
                )
                + call_args_producer
                + LineProducer(";")
...
class ChapelModuleChapelerrificProcPass(Pass):
    ...
    def apply(xml_tree):
        ...
        module_root = xml_tree.find("./chapel-module")
        module_procs = module_root.find("./procedures")
        pass_info = Pass.input_xml.find(
            "./pass/[@name='ChapelModuleChapelerrificProcPass']"
        )
        ...
        for proc in module_procs.findall("./procedure"):
            ...
            if (
                type == "ds" or type == "zc"
            ) and not config + func in iterative_functions:
                temp_type = type[0]
            ...
            for name_category in [
                (config + func, "untyped chapelerrific")
            ]:  # (type+config+func, "chapelerrific")
        ...
        """
                    ...
                    if arg.get("pass-up") == "true":
                        args_producer.append(
                            SegmentProducer(
                                ...
                                + (
                                    arg.get("type")
                                    if not arg.get("type") in typeMap
                                    else typeMap[arg.get("type")]
                                )
                                + (
                                    " = " + arg.text
                                    if arg.text != None
                        ...
                        if (
                            lapack_node == None
                            or arg.get("name") == "matrix_order"
                        ):
                            continue

                        # print "./arguments-list/argument/[@name='" + arg.get("name").upper() + "']"
                        lapack_arg_node = lapack_node.find(
                            "./arguments-list/argument/[@name='"
                            + arg.get("name").upper()
                            + "']"
                        )
            ...
            """
                ...
                code.prepend(
                    CommentProducer(
                        "\n"
                        + "Wrapped procedure of "
                        + proc.get("name")
                        + " for the type "
                        + typeToTypeString[type]
                        + ".\n"
                    )
                )
                ...
                call_args_producer = ListProducer(", ", "(", ")")
                for pass_arg in proc.findall(
                    "./pass-through-arguments-list/argument"
                ):
...
class ChapelModuleExternTypeDefinesPass(Pass):
    ...
    def apply(xml_tree):
        ...
        for define in module_defs.findall("./define"):
            def_str = (
                "/*" + define.find("./description").text + "*/\n"
                if define.find("./description") != None
                else ""
            )

            if (
                define.get("external") != None
                and define.get("external").lower() == "yes"
            ):
                def_str += "extern "
    ...
    def apply(xml_tree):
        ...
        for define in module_defs.findall("./enumeration"):
            ...
            for value in define.findall("./value"):
                values.append(
                    SegmentProducer(
                        ...
                        + (
                            " = " + value.text
                            if value.text != None and value.text.strip() != ""
                            else ""
                        )
                    )
                )
            ...
            if description_node != None:
                SubElement(define, "code").text = (
                    CommentProducer(description_node.text).generate()
                    + (
                        SegmentProducer("enum " + define.get("name"))
                        + values
                        + LineProducer(";")
                    ).generate()
                )
...
class BaseCodeGenerationPass(Pass):
    dependencies = [
        ChapelModuleExternProcPass,
        ChapelModuleStringToCharWraperProcPass,
        ChapelModuleChapelerrificProcPass,
        ChapelModuleExternTypeDefinesPass,
        ChapelModuleExternConstDefinesPass,
        ChapelModuleEnumDefinesPass,
    ]
    complete = False
...
class DumpCodePass(Pass):
    ...
    def apply(xml_tree):
        ...
        nodoc_helper_procs = (
            helper_info.get("no-doc") == "internals"
            or helper_info.get("no-doc") == "procedures"
            or helper_info.get("no-doc") == "all"
        )
...
class BucketLAPACKFunctionGroupsPass(Pass):
    ...
    def apply(xml_tree):
        ...
        for proc in module_procs.findall("./procedure"):
            ...
            config_node = group_node.find(
                "./matrix-configuration/[@name='" + config + "']"
            )
            if config_node == None:
            ...
            if config_node.find("./types/type/[@name='" + type + "']") != None:
                print(
                    "Double declaration of abstract LAPACK function",
                    type,
                    config,
                    func,
                    base_name,
            ...
            type_node.set(
                "analogue",
                "./chapel-module/procedures/procedure/[@name='"
                + proc_name
                + "']",
            )
...
class ImportAbstractLAPACKFunctionsPass(Pass):
    ...
    def apply(xml_tree):
        ...
        group_input = Pass.input_xml.find(
            "./pass/[@name='ImportAbstractLAPACKFunctionsPass']"
        )
        ...
        for group in group_input.findall("./group"):
            for config in group.findall("./matrix-configuration"):
                ...
                for arg in config.findall("./method-arguments/argument"):
                    args_producer.append(
                        SegmentProducer(
                            ...
                            + (
                                " = " + arg.text
                                if arg.text != None and arg.text.strip() != ""
                                else ""
                            )
                        )
                    )
                ...
                for type in config.findall("./types/type"):
                    where_producer.append(
                        SegmentProducer("T == " + type.get("type"))
                    )

                code.append(where_producer)

                info_var = (
                    config.get("name") + group.get("name") + "_return_info"
                )

                func_body = ScopeProducer()
                func_body.append(
                    LineProducer("var " + info_var + " : lapack_int;")
                )
                ...
                for arg in config.findall("./analogue-arguments-list/argument"):
                    arg_name = arg.get("name")
                    arg_relates[arg_name] = config.find(
                        "./arguments-relationships/argument/[@name='"
                        + arg_name
                        + "']"
                    )
                    ana_args.append(arg)
                ...
                for type in config.findall("./types/type"):
                    ...
                    call_equals = SegmentProducer(
                        info_var + " = " + chpl_ana.get("name")
                    )
...
class CommonArgumentCollectionPass(Pass):
    ...
    def apply(xml_tree):
        ...
        """
                all_args = (
                    {}
                )  # dictionary to set [arg_name]=>set( function_names )
                ...
                for type_func in config_node.findall("./types/type"):
                    ...
                    if chapel_func == None:
                        raise GeneralPassFailure(
                            type_name
                            + config_node.get("name")
                            + group_node.get("name")
                            + " does not have chapel analogue"
                        )

                    for arg in chapel_func.findall("./arguments-list/argument"):
                        func_args_type[type_name][arg.get("name")] = arg.get(
                            "type"
                        )
                ...
                for arg_name in all_args:
                    if all_args[arg_name] != all_funcs:
                        ...
                        for type_func_name in all_args[arg_name]:
                            ...
                            type_func = config_node.find(
                                "./types/type/[@name='" + type_func_name + "']"
                            )

                            args_list = SubElementUnique(
                                type_func, "arguments-list"
                            )
                            args_list.append(arg)
...
class BaseAbstractLAPACKPass(Pass):
    dependencies = [
        BucketLAPACKFunctionGroupsPass,
        CommonArgumentCollectionPass,
    ]
    complete = False
...
class DropAttemptedAssociations(Pass):
    ...
    def apply(xml_tree):
        ...
        for chpl_proc in xml_tree.findall(
            "./chapel-module/procedures/procedure"
        ):
            ...
            for arg in args_list:
                    ...
                    for i in range(len(dimensions)):
                        ...
                        if removeVar != None:
                            ...
                            print(
                                (
                                    dimension
                                    + " is not described in the arguments of "
                                    + proc.get("name")
                                )
                            )
                ...
                if arg.get("matrix-size") != None:
                    ...
                    if removeRows != None and removeCols != None:
                        pass_through[rows] = (
                            "if matrix_order == LAPACK_ROW_MAJOR then "
                            + arg.get("name")
                            + ".domain.dim(1).size else "
                            + arg.get("name")
                            + ".domain.dim(2).size "
                        ...
                        print(
                            (
                                ...
                                + " and "
                                + cols
                                + " are not described in the arguments of "
                                + proc.get("name")
                            )
                        )
...
class AbstractDropAttemptedAssociations(Pass):
    ...
    def apply(xml_tree):
        ...
        for group_node in abstract_lapack.findall("./group"):
            for config_node in group_node.findall("./matrix-configuration"):
                if config_node.findall("./types/type/arguments-list") != []:
                    print(
                        config_node.get("name") + group_node.get("name"),
                        " has typed functions with non common arguments. Skipping.",
                    )
                    continue

                full_func_name = config_node.get("name") + group_node.get(
                    "name"
                )
                all_args = []
                array_args = set()
                ...
                removed = {}  # str => bool
                for arg in config_node.findall(
                    "./analogue-arguments-list/argument"
                ):
                ...
                for arg in config_node.findall(
                    ...
                    if (
                        removed[arg.get("name")]
                        or arg.get("semantic") != "array"
                    ):
                        continue
                ...
                pass_through_node = SubElement(
                    config_node, "arguments-relationships"
                )
                for arg in config_node.findall(
                    "./analogue-arguments-list/argument"
                ):
        ...
        prettywritexml(
            abstract_lapack, "AbstractDropAttemptedAssociations_output.xml"
        )
        selfname.complete = True
        print(
            "[",
    ...
    def apply(xml_tree):
        ...
        for key in combos:
            print(
                key,
                "(",
                combos[key][0].get("name"),
                combos[key][1].get("name"),
                ")",
            )
    ...
    def apply(xml_tree):
        ...
        for proc in lapack_procs_root.findall("./procedure"):
            ...
            for arg in proc.findall("./arguments-list/argument"):
                ...
                if arg_name != "matrix_order" and (
                    (intent == None) ^ (semantic == None)
                ):
...
class FindPassByRefNonAnalouges(Pass):
    ...
    def apply(xml_tree):
        ...
        for proc in lapack_procs_root.findall("./procedure"):
            ...
            for arg in proc.findall("./arguments-list/argument"):
                if (
                    arg.find("./analogues") == None
                    and int(arg.get("refdepth")) > 0
                ):
                    if not printed:
                        printed = True
                    ...
                    print(
                        ...
                        arg.get("name"),
                        "of refdepth",
                        int(arg.get("refdepth")),
                        "has no fortran analogue",
                    )
            if printed:
                print("")
    ...
    def apply(xml_tree):
        ...
        for c_proc in lapack_c_procs.findall("./procedure"):
            ...
            for f_arg in f_proc.findall("./arguments-list/argument"):
                ...
                if (
                    f_ana_node.find(
                        "./analogue/[@function='" + c_proc.get("name") + "']"
                    )
                    == None
...
class DropFileOfCTreeUnmatchedArgsPass(Pass):
    ...
    def apply(xml_tree):
        ...
        for c_proc in lapack_c_procs.findall("./procedure"):
            ...
            prettywritexml(
                output_tree, "DropFileOfCTreeUnmatchedArgsPass_output.xml"
            )
                ...
                # we can try to match the strings from the
                    ...
                    while change:
                        ...
                        for c_arg in c_no_match:
                            ...
                            for f_arg in f_no_match:
                                score = score_string(
                                    c_arg.get("name").lower(),
                                    f_arg.get("name").lower(),
                                )
                                if score < min_score:
                                    min_score = score
                                    min_list = [f_arg]
...
class PrintArgDoc(Pass):
    ...
    def apply(xml_tree):
        ...
        arg = "A"
        for proc_node in procs_node.findall("./procedure"):
            for arg_node in proc_node.findall(
                "./arguments-list/argument/[@name='" + arg + "']"
            ):
                doc_node = arg_node.find("documentation")
                total += 1
                if doc_node == None:
                    print(
                        proc_node.get("name"), "/", arg, "has no documentation"
                    )
                    continue
        ...
        for group_node in abstract_lapack.findall("./group/[@name='sv']"):
            for config_node in group_node.findall("./matrix-configuration"):
                ...
                for type_node in config_node.findall("./type"):
                    ...
                    name_to_counts[type_node.get("name")] = len(
                        name_to_args[type_node.get("name")]
                    )
                ...
                for i in range(len(names) - 1):
                    all_same = all_same and (
                        name_to_counts[names[i]] == name_to_counts[names[i + 1]]
                    )
                print(all_same)
                all_count = name_to_counts[
                    names[1]
                ]  # grab arbitrary count if all the same
                ...
                for pos in range(all_count):
                    ...
                    for i in range(len(names) - 1):
                        is_same = (
                            is_same
                            and (
                                name_to_args[names[i]].get("name")
                                == name_to_args[names[i + 1]].get("name")
                            )
                            and (
                                name_to_args[names[i]].get("semantic")
                                == name_to_args[names[i + 1]].get("semantic")
                            )
                            and (
                                name_to_args[names[i]].get("intent")
                                == name_to_args[names[i + 1]].get("intent")
                            )
                        )
                    print(pos, is_same)
                    if not is_same:
    ...
    def apply(xml_tree):
        ...
        for group in group_input.findall("./group"):
            for config in group.findall("./matrix-configuration"):
                ...
                code.append(
                    SegmentProducer(
                        "proc " + config.get("name") + group.get("name")
                    )
                )
