...
def backtrace(debugger, command, result, internal_dict):
    ...
    while frame is not None:
        ...
        if name.startswith("chpl::resolution::resolveFnCall("):
            ...
            location = frame.EvaluateExpression(
                "parsing::locateAst({}, {})".format(
                    context.GetName(), ast.GetName()
                )
            )

            call_info = frame.variables[3]
            call_name = unique_string_to_str(
                get_child_by_name(call_info.deref, "name_")
            )
            trace.append(
                (call_name, *unpack_location(root_dir, location), frame.idx)
            )
    ...
    for i, (name, file, line, idx) in enumerate(trace):
        print(
            "{:2d}: {} at {}:{} (lldb frame {})".format(
                i, name, file, line, idx
            )
        )
