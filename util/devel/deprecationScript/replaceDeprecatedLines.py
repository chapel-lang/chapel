...
def replaceSingleInstance(line, from_string, to_string, files_to_update, regex):
    ...
    if files_to_update and file_name not in files_to_update:
        print(
            "Skipping ",
            resolved_file_path,
            " because it is not in the list of files to replace",
        )
        return
...
def replaceAllInstances(
    lines_file, from_string, to_string, files_to_update, regex
):
    ...
    with open(lines_file, "r") as f:
        ...
        # Replace
        for line in lines:
            replaceSingleInstance(
                line, from_string, to_string, files_to_update, regex
            )
