...
def extractLineNumbers(file_name, warning_message, output_file_name):
    ...
    for line in lines:
        ...
        if line.startswith(TEST_LINE_PREFIX):
            ...
            current_sub_test_path = (
                line.split(TEST_LINE_CLOSING)[0]
                .split(TEST_LINE_PREFIX)[1]
                .rsplit("/", 1)[0]
                + "/"
            )
            continue
...
def resolveToAbsPath(current_sub_test_path, line):
    ...
    # Else we append the current_sub_test_path to the line along with '$CHPL_HOME/test/'
        ...
        result = (
            DIR_ABS_PREFIX
            + current_sub_test_path
            + line.split(DIFF_INDICATOR, 1)[-1]
        )

        # replace "XX/somedir/../YY" with "XX/YY"
        subs = 1
        while subs > 0:
            result, subs = re.subn(r"/[^/]+/\.\./", "/", result)
...
def main():
    ...
    if len(sys.argv) < 3:
        print(
            "Usage: python3 extractLineNumbers.py <log_file> <search_string> [output_file_name]"
        )
