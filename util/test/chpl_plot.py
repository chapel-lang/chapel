...
class Plot:
  ...
  def __init__(self, table, title=None, xlabel=None, ylabel=None, kind=None, includeLegend=True, better=None, mpl_legend_args=None):
...
def _maxLen(l):
  ...
  return reduce(lambda x,y: max(x,len(str(y))), l, 0)
...
class Table:
  ...
  def md(self):
    def _compute_col_length(col_title):
      return max(len(col_title), _maxLen(self.yData[col_title]))
    ...
    for row_num in range(len(self.xData)):
      ...
      for col_num in range(len(self.yData)):
        ...
        res += f" %{col_label_len[col_num]}s |" % str(self.yData[cur_col][row_num])
  ...
  def relabel_col_w_tbl_title(self):
...
class TableCollection():
  ...
  def md(self):
    ...
    return "\n\n".join(map(lambda tbl: tbl.md(), self.tables.values()))
  ...
  def relabel_col_w_tbl_title(self):
    res = {}
    for entry in self.tables:
      res[entry] = self.tables[entry].relabel_col_w_tbl_title()
    return TableCollection(res)
