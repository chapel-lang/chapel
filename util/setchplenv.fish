...
set -x CHPL_HOST_PLATFORM (eval $CHPL_HOME/util/chplenv/platform)
echo "to $CHPL_HOST_PLATFORM"

echo -n "Updating PATH "
set -x PATH $PATH "$CHPL_HOME/bin/$CHPL_HOST_PLATFORM" "$CHPL_HOME/util"
echo "to include $CHPL_HOME/bin/$CHPL_HOST_PLATFORM"
echo "                     and $CHPL_HOME/util"

echo -n "Updating MANPATH "
set -x MANPATH "$MANPATH" "$CHPL_HOME"/man
