...
_chpl ()
{
  ...
  case "$cur" in
    -*)
      # The following lists were generated with the commands preceeding them.

      # single letter options
      # chpl --help | grep -v '^[A-Za-z]' | grep '^.*--' |              \
      # sed -e 's;^\(.*--[^ ]*\).*$;\1;' | grep ',' | sed -e 's;,.*$;;'
      single="-M -g -I -l -L -O -o -s -h -t -d -D"
