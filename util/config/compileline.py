...
def myrun(cmd, arg=None):
    ...
    lines = [
        line for line in output.splitlines(True) if not line.startswith("make")
    ]
    ret = "".join(lines)
...
def parseArguments():
    ...
    parser.add_option(
        ...
        dest="actions",
        action="append_const",
        help="print a C compiler invocation that can use " "the Chapel runtime",
    )
    parser.add_option(
        "--compile-c++",
        const="compilecxx",
    ...
    parser.add_option(
        ...
        dest="actions",
        action="append_const",
        help="print out -L and -l options to link with " "the Chapel runtime",
    )
    parser.add_option(
        "--linker",
        const="linker",
    ...
    parser.add_option(
        "--multilocale-lib-deps",
        const="multilocale-lib-deps",
        dest="actions",
        action="append_const",
        help="print out libraries required to link " "multi-locale libraries",
    )
    parser.add_option(
        "--host-c-compiler",
        const="host-c-compiler",
        dest="actions",
        action="append_const",
...
def main():
    ...
    if "CHPL_HOME" in os.environ:
        if os.path.abspath(os.environ["CHPL_HOME"]) != chpl_home_dir:
            ...
            if os.path.samefile(env_check, rel_check):
                ...
                sys.stderr.write(
                    "Warning: check {0} not found\n".format(check_path)
                )
                sys.stderr.write(
                    "Warning: Mismatched CHPL_HOME; got {0} but expected {1}\n".format(
                        os.path.abspath(os.environ["CHPL_HOME"]), chpl_home_dir
                    )
                )
    ...
    for a in actions:
        if a == "home":
            ...
            os.environ["CHPL_TARGET_COMPILER_PRGENV"] = (
                chpl_compiler.get_prgenv_compiler()
            )
            ...
            if llvm == "none":
                sys.stderr.write(
                    "Cannot get --llvm configuration with CHPL_LLVM=none\n"
                )
                sys.exit(1)
        elif a == "compilecc":
