...
# Note: newer LLVM versions (16+) do not require this
...
class ClangLibBuiltin(ClangBuiltin):
    ...
    def __init__(
        self, functionName, functionType, attrs, libHeader, supportedLangs
    ):
...
def parseLibBuiltin(b):
    ...
    if result:
        d = result.groupdict()
        return ClangLibBuiltin(
            d["name"], d["type"], d["attrs"], d["header"], d["langs"]
        )
    return None
...
def runLicenseScript(f):
    licenseScriptPath = (
        chplHome + "/util/buildRelease/add_license_to_sources.py"
    )
...
def main():
    ...
    for b in clangDelegatedBuiltins:
        ...
        if builtinCandidate and builtinCandidate.canGenerateWrapper():
            clangBuiltinsWrapperContent.append(
                builtinCandidate.generateWrapper()
            )
        else:
            print("Cannot generate wrapper for ", b)
            sys.exit(1)

    clangBuiltinsWrapperLocation = (
        chplHome + "/runtime/include/llvm/clang_builtins_wrapper.h"
    )
    ...
    clangBuiltinsWrappedSetContent = [
        ...
        "{",
        ",\n".join(
            [stringify(s) for s in rewrittenBuiltins + clangDelegatedBuiltins]
        ),
        "};\n",
        "#endif",
    ]

    clangBuiltinsWrappedSetLocation = (
        chplHome + "/compiler/include/clangBuiltinsWrappedSet.h"
    )
    ...
    print(
        "Wrapper header for compiler saved to "
        + clangBuiltinsWrappedSetLocation
    )
