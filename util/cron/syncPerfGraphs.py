...
def main():
    parser = argparse.ArgumentParser(
        description="Syncs chapel performance "
    ...
    parser.add_argument(
        "dirToSync",
        metavar="DIR",
        help="perf directory to " "sync that contains the SUCCESS file",
    )
    parser.add_argument(
        "destDir",
        metavar="DIR",
        help="directory on dreamhost to " "sync the graphs to.",
    )
    parser.add_argument(
        "--logFile",
        "-l",
    ...
    parser.add_argument(
        ...
        default=5,
        type=int,
        help="max number of retries for sync command",
    )
...
def syncToCrayWebhost(dirToSync, destDir, logFile, numRetries):
    logFile.write(
        "ChapelU Webhost sync log for: {0} \n\n".format(
            time.strftime("%m/%d/%Y")
        )
    )
    ...
    if not os.path.isfile(successFile):
        logFile.write(
            "SUCCESS file did not exist in {0}. Assuming genGraph was "
    ...
    rsyncRet = updateTarget(
        dirToSync, webHost, perfDir, rsyncDesc, logFile, numRetries
    )
    return rsyncRet
...
def updateTarget(dirToSync, webHost, perfDir, rsyncDesc, logFile, numRetries):
    ...
    delOldCommand = 'ssh {0} "find {1} -ctime +100 | xargs rm -rf "'.format(
        webHost, perfDir
    )
...
def runRsyncCommand(rsyncCommand, rsyncDesc, numRetries, logFile):
    ...
    for _ in range(numRetries):
        ...
        if rsyncRet == 0:
            ...
            delay = 30
            logFile.write(
                "Waiting {0} seconds before retrying sync...\n\n".format(delay)
            )
            time.sleep(delay)
    return rsyncRet
...
def executeCommand(command, commandDesc, logFile):
    logFile.write(
        "Attempting to {0} with `{1}`...\n".format(commandDesc, command)
    )
    logFile.flush()

    commandRet = subprocess.call(
        shlex.split(command), stdout=logFile, stderr=subprocess.STDOUT
    )
