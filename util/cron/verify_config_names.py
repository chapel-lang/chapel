...
for line in sys.stdin.readlines():
    ...
    pattern = re.compile(
        r'CHPL_NIGHTLY_TEST_CONFIG_NAME="(?P<config_name>[a-z0-9\-.]+)"',
        re.IGNORECASE,
    )
    ...
    if match is not None:
        ...
        print(
            "[ERROR] Could not find nightly test config name "
    ...
    if not filename_base.endswith(config_name):
        print(
            '[ERROR] test script name: "{0}" does not match its config name: "{1}"'.format(
