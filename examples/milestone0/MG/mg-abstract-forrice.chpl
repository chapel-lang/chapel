...
const probSize: [S..O] = (32, 64, 256, 256, 512, 1024, 256);
const numIters: [S..O] = (4, 40, 4, 20, 20, 50, 4);
const checksum: [S..O] = (0.0000530770700573,
...
config const numLevels = lg2(n);

config const nit = numIters(Class);
...
const Levels = (1..numLevels);
const Base: domain(3) distributed(block(3)) = [1..nx, 1..ny, 1..nz];
const Hier: [] domain(Base) = [lvl: Levels] Base by -2**(lvl-1);
...
function printResults(rnm2, inittime, runtime) {
  ...
  if (verifyValue != 0.0) {
    const epsilon = 0.00000001;  -- BLC: 1.e-8;
    const delta = rnm2 - verifyValue;
...
function psinv(U, R: [?DUR]) {
  static const c = initCValues();
  static const c3d = [(i,j,k): Stencil] c((i!=0) + (j!=0) + (k!=0));

  const Rstr = DUR.stride;

  U += [ijk:DUR] sum [off:Stencil] (c3d * R(ijk + Rstr*off));
}


function resid(R, V, U: [?DUR]) {
  static const a = (-8.0/3.0, 0.0, 1.0/6.0, 1.0/12.0);
  static const a3d = [(i,j,k):Stencil] a((i!=0) + (j!=0) + (k!=0));

  const Ustr = DUR.stride;

  R = V - [ijk:DUR] sum [off:Stencil] (a3d * U(ijk + Ustr*off));
}


function rprj3(S; R: [?DR]) {
  static const w = (0.5, 0.25, 0.125, 0.0625);
  static const w3d = [(i,j,k): Stencil] w((i!=0) + (j!=0) + (k!=0));
...
function interp(R: [?DR]; S: [?DS]) {
  static const IDom = [-1..0, -1..0, -1..0];
  static const IStn = [(i,j,k):IDom] [i..0, j..0, k..0];
  static const w = [ijk:IDom] 1.0 / IStn.size();

  const Rstr = DR.stride;
  const Sstr = DS.stride;
...
function longRandlc(n): {
  static const s = 314159265.0;
  static const arand = 5.0**13;
...
function randlc(x, a): {
  static const r23 = 0.5**23;
  static const t23 = 2.0**23;
  static const r46 = 0.5**46;
  static const t46 = 2.0**46;
...
function initCValues(const Class): coeff {
