...
config const class: character = 'S';
...
const Hier: [lvl: Levels] domain(3) distributed(block(3)) = Base / 2**(lvl-1);
...
function psinv(U, R) {
  ...
  U += [ijk:RD] sum [off:Stencil] (c3d * R(ijk + off));
}
...
function resid(R, V, U) {
  ...
  R = V - [ijk:UD] sum [off:Stencil] (a3d * U(ijk + off));
}
...
function rprj3(S, R) {
  ...
  S = [ijk: RD] sum [off:Stencil] (w3d * R(ijk + off));
}
...
function interp(R, S) {
  ...
  forall ioff in IDom {
    [ijk:SD] R(ijk + ioff) 
               += w(ioff) * sum [off: IStn(ioff)] S(ijk + off);
  }
}
