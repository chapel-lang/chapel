...
/*
   ...
   .. note::
      ...
      both NPBRandomStream and PCGRandomStream to be available.
...
module Random {
  ...
  */
  ...
  /*
     ...
     Besides a Chapel RandomStream interface, this module includes
     a number of low-level PCG random functions.
  ...
  */
  module PCGRandom {
    ...
    /*
      ...
      generating a value in a range, it creates a new random number generator
      with the same seed as the old one but a different increment. It steps this
      random number generator once and then uses the stream of numbers from it
      to compute a value in the particular bounds in a way that attempts to
      avoid bias. While we have tested this strategy with TestU01 1.2.3, it may
      have undesireable statistical properties.
      ...
      number. This strategy leaves out many possible real values, but is
      believed to have reasonable statistical properties. One side effect
      of this strategy is that the real number 1.0 can be generated because
      of rounding. The real number 0.0 can be generated because PCG can
      produce the value 0 as a random integer.
