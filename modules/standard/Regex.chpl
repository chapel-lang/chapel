...
record regex {
  ...
  proc init(pattern: ?t, posix=false, literal=false, noCapture=false,
    ...
    /*var ret: regex(t);*/
    qio_regex_create_compile(pattern.localize():c_ptrConst(c_char):c_string, pattern.numBytes, opts,
                             this._regex);
  ...
  proc _search_match(text: exprType, anchor: c_int, param has_captures, ref captures):regexMatch
  {
    ...
    var got:bool;
    got = qio_regex_match(localRegex, text.localize():c_ptrConst(c_char):c_string, text.numBytes,
...
private proc doReplaceAndCountFast(x: ?t, pattern: regex(t), replacement: t,
  ...
  var nreplaced: int = qio_regex_replace(localRegex, replacement.localize():c_ptrConst(c_char):c_string,
                                    replacement.numBytes, x.localize():c_ptrConst(c_char):c_string,
