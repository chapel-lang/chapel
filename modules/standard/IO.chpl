...
inline proc channel.readwrite(ref x) where !this.writing {
  ...
  /*
     ...
     :returns: ch
   */
  inline proc <~>(const ref ch: channel, x) throws const ref
  where ch.writing {
    ch.writeIt(x);
    return ch;
  }
  // documented in the writing version.
  pragma "no doc"
  inline proc <~>(const ref ch: channel, ref x) throws const ref
  where !ch.writing {
    ch.readIt(x);
    return ch;
  }
  ...
  /* Overload to support reading an :type:`IO.ioLiteral` without
     ...
     the ioLiteral.
   */
  inline proc <~>(const ref r: channel, lit:ioLiteral) throws const ref
  where !r.writing {
    var litCopy = lit;
    r.readwrite(litCopy);
    return r;
  ...
  /* Overload to support reading an :type:`IO.ioNewline` without
     ...
     the ioNewline.
   */
  inline proc <~>(const ref r: channel, nl:ioNewline) throws const ref
  where !r.writing {
    var nlCopy = nl;
    r.readwrite(nlCopy);
    return r;
