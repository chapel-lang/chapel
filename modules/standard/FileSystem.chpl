...
proc copy(src: string, dest: string, metadata: bool = false, permissions: bool = true) throws {
  ...
  proc copyMode(src: string, dest: string) throws {
    proc getMode(name: string): int throws {
      ...
      var ret:c_int;
      var err = chpl_fs_viewmode(ret, unescape(name):c_ptrConst(c_char):c_string);
...
/*
    ...
    const statRet = stat(src:c_ptrConst(c_char):c_string, c_ptrTo(statStruct));

    if statRet != 0 then try ioerror(statRet:errorCode, "in copy's stat");;
    const chmodRet = chmod(dest:c_ptrConst(c_char):c_string, statStruct.st_mode);
...
*/
  ...
  if (metadata) {
    ...
    var err = chpl_fs_copy_metadata(unescape(src):c_ptrConst(c_char):c_string,
                                    unescape(dest):c_ptrConst(c_char):c_string);
...
private proc copyTreeHelper(src: string, dest: string, copySymbolically: bool=false, metadata: bool=false) throws {
  ...
  // Create dest
  var oldMode:c_int;
  var err = chpl_fs_viewmode(oldMode, unescape(src):c_ptrConst(c_char):c_string);
...
proc exists(name: string): bool throws {
  ...
  var ret:c_int;
  var err = chpl_fs_exists(ret, unescape(name):c_ptrConst(c_char):c_string);
...
proc getGid(name: string): int throws {
  ...
  var result: c_int;
  var err = chpl_fs_get_gid(result, unescape(name):c_ptrConst(c_char):c_string);
...
proc getFileSize(name: string): int throws {
  ...
  var result: int;
  var err = chpl_fs_get_size(result, unescape(name):c_ptrConst(c_char):c_string);
...
proc getUid(name: string): int throws {
  ...
  var result: c_int;
  var err = chpl_fs_get_uid(result, unescape(name):c_ptrConst(c_char):c_string);
...
private module GlobWrappers {
  ...
  inline proc glob_w(pattern: string, ref ret_glob:glob_t): void {
    ...
    const err = chpl_glob(unescape(pattern):c_ptrConst(c_char):c_string, GLOB_NOFLAGS, ret_glob);
...
proc isDir(name:string):bool throws {
  ...
  var err = chpl_fs_is_dir(ret, unescape(name):c_ptrConst(c_char):c_string);
...
proc isFile(name:string):bool throws {
  ...
  var err = chpl_fs_is_file(ret, unescape(name):c_ptrConst(c_char):c_string);
...
proc isSymlink(name: string): bool throws {
  ...
  var ret:c_int;
  var err = chpl_fs_is_link(ret, unescape(name):c_ptrConst(c_char):c_string);
...
proc isMount(name: string): bool throws {
  ...
  var ret:c_int;
  var err = chpl_fs_is_mount(ret, unescape(name):c_ptrConst(c_char):c_string);
...
iter listDir(path: string = ".", hidden: bool = false, dirs: bool = true,
  ...
  var dir: DIRptr = opendir(unescape(path):c_ptrConst(c_char):c_string);
  if (!is_c_nil(dir)) {
    ...
    perror(("error in listDir(): " + path):c_ptrConst(c_char):c_string);
  }
}
...
proc mkdir(name: string, mode: int = 0o777, parents: bool=false) throws {
  ...
  var err = chpl_fs_mkdir(unescape(name):c_ptrConst(c_char):c_string, mode, parents);
...
proc rename(oldname: string, newname: string) throws {
  ...
  var err = chpl_fs_rename(unescape(oldname):c_ptrConst(c_char):c_string,
                           unescape(newname):c_ptrConst(c_char):c_string);
...
proc remove(name: string) throws {
  ...
  var err = chpl_fs_remove(unescape(name):c_ptrConst(c_char):c_string);
...
proc sameFile(file1: string, file2: string): bool throws {
  ...
  var ret:c_int;
  var err = chpl_fs_samefile_string(ret, unescape(file1):c_ptrConst(c_char):c_string,
                                         unescape(file2):c_ptrConst(c_char):c_string);
...
proc symlink(oldName: string, newName: string) throws {
  ...
  var err = chpl_fs_symlink(unescape(oldName):c_ptrConst(c_char):c_string,
                            unescape(newName):c_ptrConst(c_char):c_string);
