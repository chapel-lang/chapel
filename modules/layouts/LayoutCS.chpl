...
defaults to ``true`` if omitted. For example:
  ...
  .. code-block:: chapel
    ...
    var CSR_Domain: sparse subdomain(D) dmapped new CS(compressRows=true); // Default argument
    var CSC_Domain : sparse subdomain(D) dmapped new CS(compressRows=false);
