  ...
  */
  ...
  record HDF5_file {
    ...
    proc init(name: string, access: c_uint = C_HDF5.H5F_ACC_RDONLY) {
      filename = name;
      file_id = C_HDF5.H5Fopen(name:c_ptrConst(c_char):c_string, access, C_HDF5.H5P_DEFAULT);
    }
  ...
  */
  ...
  proc readAllNamedHDF5Files(locs: [] locale, filenames: [] string,
    ...
    forall (f, name) in zip(files, filenames) {
      ...
      var file_id = C_HDF5.H5Fopen(locName:c_ptrConst(c_char):c_string, C_HDF5.H5F_ACC_RDONLY, C_HDF5.H5P_DEFAULT);
      var dims: [0..#rank] C_HDF5.hsize_t;
      var dsetRank: c_int;

      C_HDF5.H5LTget_dataset_ndims(file_id, dsetName:c_ptrConst(c_char):c_string, dsetRank);
      if rank != dsetRank {
        halt("rank mismatch in file: " + name + " dataset: " + dsetName +
             rank:string + " != " + dsetRank:string);
      }
      C_HDF5.HDF5_WAR.H5LTget_dataset_info_WAR(file_id, dsetName:c_ptrConst(c_char):c_string,
  ...
  proc readHDF5Dataset(file_id, dsetName: string, data) {
    ...
    C_HDF5.H5LTread_dataset(file_id, dsetName:c_ptrConst(c_char):c_string,
  ...
  proc writeArraysToHDF5Files(dirName: string, dsetNames: [] string,
    ...
    forall (arr, dsetName, fname) in zip(data, dsetNames, filenames) {
      ...
      if mode == Hdf5OpenMode.Truncate || !exists(filename) {
        file_id = C_HDF5.H5Fcreate(filename:c_ptrConst(c_char):c_string, C_HDF5.H5F_ACC_TRUNC,
                                   C_HDF5.H5P_DEFAULT, C_HDF5.H5P_DEFAULT);
      } else {
        file_id = C_HDF5.H5Fopen(filename:c_ptrConst(c_char):c_string, C_HDF5.H5F_ACC_RDWR,
                                 C_HDF5.H5P_DEFAULT);
      }
      var dims: [0..#rank] C_HDF5.hsize_t;
      ...
      C_HDF5.HDF5_WAR.H5LTmake_dataset_WAR(file_id, dsetName:c_ptrConst(c_char):c_string, rank,
                                           c_ptrTo(dims),
                                           getHDF5Type(eltType),
                                           c_ptrTo(arr.A));
      C_HDF5.H5Fclose(file_id);
  ...
  iter hdf5ReadChunks(filename: string, dset: string,
    ...
    var file_id = C_HDF5.H5Fopen(filename:c_ptrConst(c_char):c_string,
                                 C_HDF5.H5F_ACC_RDONLY,
                                 C_HDF5.H5P_DEFAULT);
    var dataset = C_HDF5.H5Dopen(file_id, dset:c_ptrConst(c_char):c_string,
                                 C_HDF5.H5P_DEFAULT);
    var dataspace = C_HDF5.H5Dget_space(dataset);

    var dsetRank: c_int;

    C_HDF5.H5LTget_dataset_ndims(file_id, dset:c_ptrConst(c_char):c_string, dsetRank);

    var dims: [0..#dsetRank] C_HDF5.hsize_t;
    C_HDF5.HDF5_WAR.H5LTget_dataset_info_WAR(file_id, dset:c_ptrConst(c_char):c_string,
                                             c_ptrTo(dims), nil, nil);
  ...
  module IOusingMPI {
    ...
    proc hdf5WriteDistributedArray(A: [], filename: string, dsetName: string) {
      ...
      coforall loc in A.domain.targetLocales() do on loc {
        ...
        var fid = H5Fcreate(locFilename:c_ptrConst(c_char):c_string, H5F_ACC_TRUNC,
        ...
        var dataset = H5Dcreate1(fid, dsetName:c_ptrConst(c_char):c_string, hdf5Type,
                                 sid, H5P_DEFAULT);
    ...
    proc hdf5ReadDistributedArray(A, filename: string, dsetName: string) {
      ...
      coforall loc in A.targetLocales() do on loc {
        ...
        var file_id = C_HDF5.H5Fopen(filenameCopy:c_ptrConst(c_char):c_string,
                                     C_HDF5.H5F_ACC_RDONLY,
                                     C_HDF5.H5P_DEFAULT);
        var dataset = C_HDF5.H5Dopen(file_id, dsetName:c_ptrConst(c_char):c_string,
                                     C_HDF5.H5P_DEFAULT);
        var dataspace = C_HDF5.H5Dget_space(dataset);
        var dsetRank: c_int;
        ...
        var dims: [1..dsetRank] C_HDF5.hsize_t;
        {
          ...
          */
          use C_HDF5.HDF5_WAR;
          H5LTget_dataset_info_WAR(file_id, dsetName:c_ptrConst(c_char):c_string,
                                   c_ptrTo(dims), nil, nil);
        }
