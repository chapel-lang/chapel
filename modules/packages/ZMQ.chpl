  ...
  */
  ...
  record Socket {
    ...
    proc bind(endpoint: string) {
      on classRef.home {
        var tmp = endpoint;
        var ret = zmq_bind(classRef.socket, tmp:c_ptrConst(c_char):c_string);
        if ret == -1 {
          var errmsg: string;
          try! {
            errmsg = string.createCopyingBuffer(zmq_strerror(errno));
    ...
    proc connect(endpoint: string) {
      on classRef.home {
        var tmp = endpoint;
        var ret = zmq_connect(classRef.socket, tmp:c_ptrConst(c_char):c_string);
        if ret == -1 {
          var errmsg: string;
          try! {
            errmsg = string.createCopyingBuffer(zmq_strerror(errno));
    ...
    */
    ...
    proc setSubscribe(value: string) throws {
      on classRef.home {
        var ret = zmq_setsockopt(classRef.socket, ZMQ_SUBSCRIBE,
                                 value:c_ptrConst(c_char):c_string: c_void_ptr,
    ...
    */
    ...
    proc send(data: ?T, flags: int = 0) throws where isString(T) || isBytes(T) {
      on classRef.home {
        ...
        var msg: zmq_msg_t;
        if (0 != zmq_msg_init_data(msg, copy:c_ptrConst(c_char):c_string:c_void_ptr,
