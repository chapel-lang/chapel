...
private extern proc qio_channel_nbytes_available(threadsafe:c_int, ch:qio_channel_ptr_t, ref space:int(64));
private extern proc qio_channel_copy_to_available(threadsafe:c_int, ch:qio_channel_ptr_t, ptr:c_void_ptr, len:ssize_t):syserr;
private extern proc qio_channel_nbytes_write_behind(threadsafe:c_int, ch:qio_channel_ptr_t, ref space:int(64));
private extern proc qio_channel_copy_from_buffered(threadsafe:c_int, ch:qio_channel_ptr_t, ptr:c_void_ptr, len:ssize_t, ref n_written_out:ssize_t):syserr;
...
class CurlFile : QioPluginFile {
  ...
  override proc setup(out pluginChannel:QioPluginChannel,
                      start:int(64),
                      end:int(64),
...
proc read_data(ptr:c_void_ptr, size:size_t, nmemb:size_t, userp:c_void_ptr):size_t
{
  ...
  if (size == 0) || (nmemb == 0) || ((size*nmemb) < 1) then
    return 0; // asked us to write 0 bytes..

  if ret.curr >= ret.count {
    return 0; // reached the end of our buffer group
  }
...
private proc chpl_curl_write_received(contents: c_void_ptr, size:size_t, nmemb:size_t, userp: c_void_ptr):size_t {
  ...
  err = qio_channel_copy_to_available(0, cc.qio_ch, contents, amt);
...
private proc read_atleast(cc:CurlChannel, requestedAmount:int(64)):syserr {
  ...
  while cc.running_handles > 0 {
    ...
    space = 0;
    qio_channel_nbytes_available(0, ch, space);
    if space >= amt then
      break;
...
private proc write_amount(cc:CurlChannel, requestedAmount:int(64)):syserr {
  ...
  starting_space = 0;
  qio_channel_nbytes_write_behind(0, ch, starting_space);
