  ...
  */
    ...
    class Parser {
      ...
      proc parseValue(): shared Toml throws {
        ...
        try! {
          ...
          } else if Str.match(val) { // Strings (includes multi-line)
            var toStr: string;
            if val.startsWith('"""') {
              toStr += getToken(source).strip('"""', true, false);
              while !toStr.endsWith('"""') {
          ...
          } else if dt.match(val) { // DateTime
            var date =
              dateTime.strptime(getToken(source), "%Y-%m-%dT%H:%M:%SZ");
            return new shared Toml(date);
          } else if ld.match(val) { // Date
            var raw = getToken(source).split("-");
            var d = new date(raw[0]: int,
                            raw[1]: int,
                            raw[2]: int);
            return new shared Toml(d);
          } else if ti.match(val) { // Tme
            use IO;
            var raw = getToken(source).split(":");
          ...
          } else if val == '#' { // comments within arrays
            skipLine(source);
            return parseValue();
    ...
    */
    class Toml : writeSerializable {
      ...
      proc init(root: Toml) {
        ...
        this.dt = root.dt;
        this.s = root.s;
        for idx in root.A.keys() do
          this.A[idx] = new shared Toml(root.A[idx]!)?;
        this.tag = root.tag;
      }
      ...
      */
      proc get(
        tblpath: string,
        const in default: shared Toml? = nil: shared Toml?
      ): shared Toml? {
        const indx = tblpath.split('.', maxsplit=1);
      ...
      */
      ...
      proc set(
        tbl: string,
        arr: [?dom] shared Toml?
      ) throws where !dom.isAssociative() {
        ref t = getThis(tbl);
        if t == nil {
      ...
      iter keys() throws {
        if this.tag != fieldToml then
          throw new TomlError("Only tables have keys");
        for k in this.A.keys() {
          yield k;
        }
      }
      iter keys(param tag: iterKind) throws where tag == iterKind.standalone {
        if this.tag != fieldToml then
          throw new TomlError("Only tables have keys");
        foreach k in this.A.keys() do
          yield k;
      }
      ...
      */
      ...
      proc flatten(ref flat: map(string, shared Toml?, false),
                  rootKey = '') : flat.type {
      ...
      @chpldoc.nodoc
      proc printTables(ref flat: map(string, shared Toml?, false),
                       f: fileWriter(?)) {
        if flat.contains('root') {
          f.writeln('[root]');
      ...
      @chpldoc.nodoc
      /* Send values from table to toString for writing  */
      proc printValuesJSON(f: fileWriter(?),
                           v: borrowed Toml, in indent=0) throws {
        var keys = v.A.keysToArray();
        sort(keys);
  ...
  module TomlReader {
    ...
    */
    ...
    proc addToken(source, tokensToAdd: [] string) {
