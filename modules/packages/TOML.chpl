...
proc parseToml(input: string) : Toml {
  var D: domain(string);
  var table: [D] Toml;
  var rootTable = new raw Toml(table);
  const source = new raw Source(input);
  const parser = new raw Parser(source, rootTable);
...
*/
  class Toml {
    ...
    proc init(root: Toml) {
      ...
      this.re = root.re;
      this.dom = root.dom;
      for idx in root.dom do this.arr[idx] = new raw Toml(root.arr[idx]);
      this.dt = root.dt;
      this.s = root.s;
      this.D = root.D;
      for idx in root.D do this.A[idx] = new raw Toml(root.A[idx]);
      this.tag = root.tag;
    }
...
module TomlReader {
 ...
 config const debugTomlReader = false;
  ...
  proc skipLine(source) {
    ...
    var emptyCurrent = new raw Tokens(emptyArray);
  ...
  class Source {
    ...
    proc splitLine(line) {
      ...
      if !linetokens.isEmpty() {
        var tokens = new raw Tokens(linetokens);
