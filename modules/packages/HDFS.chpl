  ...
  */
  ...
  class HDFSFileSystem {
    ...
    proc init(nameNode: string="default", port:int=0) {
      ...
      this.port = port;
      this.hfs = hdfsConnect(this.nameNode:c_ptrConst(c_char):c_string, this.port.safeCast(uint(16)));
      this.complete();
      refCount.write(1);
    }
    @chpldoc.nodoc
    ...
    proc openHelper(path:string, mode:ioMode,
      ...
      var hfile = hdfsOpenFile(this.hfs, path.localize():c_ptrConst(c_char):c_string,
  ...
  class HDFSFile : QioPluginFile {
    ...
    override proc filelength(out length:int(64)):errorCode {
      ...
      var fInfoPtr = hdfsGetPathInfo(fs.hfs, path:c_ptrConst(c_char):c_string);
      if fInfoPtr == nil {
        return EINVAL;
      }
    ...
    override proc getpath(out path:c_string, out len:int(64)):errorCode {
      ...
      path = qio_strdup(this.path:c_ptrConst(c_char):c_string);
      len = this.path.size;
      if verbose then
