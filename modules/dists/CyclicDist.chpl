...
class Cyclic: BaseDist {
  ...
  proc init(startIdx,
    ...
    coforall locid in targetLocDom {
      on targetLocs(locid) {
        locDist(locid) = new raw LocCyclic(rank, idxType, locid, this);
      }
    }
  ...
  proc dsiClone() {
    return new raw Cyclic(startIdx, targetLocs,
...
proc Cyclic.dsiNewRectangularDom(param rank: int, type idxType, param stridable: bool, inds) {
  ...
  var dom = new raw CyclicDom(rank=rank, idxType=idxType, dist = this, stridable=stridable);
  ...
  if locDoms(dist.targetLocDom.low) == nil {
    coforall localeIdx in dist.targetLocDom {
      on dist.targetLocs(localeIdx) do
        locDoms(localeIdx) = new raw LocCyclicDom(rank, idxType, stridable, dist.getChunk(whole, localeIdx));
    }
  } else {
    coforall localeIdx in dist.targetLocDom {
...
proc CyclicDom.dsiBuildArray(type eltType) {
  var arr = new raw CyclicArr(eltType=eltType, rank=rank, idxType=idxType, stridable=stridable, dom=this);
  arr.setup();
  return arr;
}
...
proc CyclicDom.dsiPrivatize(privatizeData) {
  ...
  var c = new raw CyclicDom(rank=rank, idxType=idxType, stridable=stridable, dist=privdist);
  c.locDoms = locDoms;
  c.whole = whole;
  return c;
}
    ...
    for localeIdx in dom.dist.targetLocDom {
      on dom.dist.targetLocs(localeIdx) {
        ...
        if disableCyclicLazyRAD {
          ...
          myLocArr.locCyclicRAD = new raw LocCyclicRADCache(rank, idxType, dom.dist.startIdx, dom.dist.targetLocDom);
...
proc CyclicArr.setup() {
  coforall localeIdx in dom.dist.targetLocDom {
    on dom.dist.targetLocs(localeIdx) {
      locArr(localeIdx) = new raw LocCyclicArr(eltType, rank, idxType, stridable, dom.locDoms(localeIdx));
...
proc CyclicArr.dsiPrivatize(privatizeData) {
  ...
  var c = new raw CyclicArr(eltType=eltType, rank=rank, idxType=idxType, stridable=stridable, dom=privdom);
  c.locArr = locArr;
