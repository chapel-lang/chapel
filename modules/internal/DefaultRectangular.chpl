...
*/
    ...
    iter these(param tag: iterKind,
      ...
      if localeModelHasSublocales && numSublocs != 0 {
        ...
        if numChunks == 1 {
          if rank == 1 {
            yield (offset(0)..#ranges(0).length,);
          } else {
    ...
    proc dsiNumIndices {
      ...
      for param i in 0..rank-1 do
        sum *= ranges(i).length;
      return sum;
      // WANT: return * reduce (this(0..rank-1).length);
    }
  ...
  class DefaultRectangularArr: BaseRectangularArr {
    ...
    proc postinit() {
      ...
      const size = if storageOrder == ArrayStorageOrder.RMO
                   then blk(0) * dom.dsiDim(0).length
                   else blk(rank-1) * dom.dsiDim(rank-1).length;
    ...
    override proc dsiReallocate(allocBound: range(idxType,
      ...
      on this {
        ...
        forall i in arrayBound(dom.ranges(0)) do
