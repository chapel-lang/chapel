...
module DefaultSparse {
  ...
  class DefaultSparseDom: BaseSparseDomImpl {
    ...
    pragma "local field"
    var inds: [nnzDom] index(rank, idxType);
    ...
    proc find(ind) {
      ...
      if rank == 1 && isTuple(ind) && ind.size == 1 then
        return binarySearch(inds, ind(1), lo=1, hi=_nnz);
      else
        return binarySearch(inds, ind, lo=1, hi=_nnz);
    }
    ...
    override proc bulkAdd_help(inds: [?indsDom] index(rank, idxType),
      ...
      if _nnz == 0 {
        ...
        var indIdx = inds.domain.low;
        var prevIdx = parentDom.low - 1;

        if isUnique {
          inds[inds.domain.low..#inds.size]=inds;
          return inds.size;
        }
        else {
          for i in inds {
  ...
  proc DefaultSparseArr.dsiSerialWrite(f) throws {
    if (rank == 1) {
      ...
      if (dom._nnz >= 1) {
        var prevInd = dom.inds(1);
        f <~> data(1);
        for i in 2..dom._nnz {
          if (prevInd(1) != dom.inds(i)(1)) {
            f <~> "\n";
          } else {
            f <~> " ";
          }
