...
module NetworkAtomics {
  ...
  record RAtomicT : writeSerializable {
    ...
    inline proc fetchMin(val:valType, param order: memoryOrder = memoryOrder.seqCst): valType {
      pragma "insert line file info" extern externFunc("fetch_min", valType)
        proc atomic_fetch_min(ref op:valType, l:int(32), obj:c_ptr(void), ref result:valType, order:memory_order): void;

      var ret:valType;
      var v = val;
      atomic_fetch_min(v, _localeid(), _addr(), ret, c_memory_order(order));
      return ret;
    }

    inline proc min(val:valType, param order: memoryOrder = memoryOrder.seqCst): void {
      pragma "insert line file info" extern externFunc("min", valType)
        proc atomic_min(ref op:valType, l:int(32), obj:c_ptr(void), order:memory_order): void;

      var v = val;
      atomic_min(v, _localeid(), _addr(), c_memory_order(order));
    }

    inline proc fetchMax(val:valType, param order: memoryOrder = memoryOrder.seqCst): valType {
      pragma "insert line file info" extern externFunc("fetch_max", valType)
        proc atomic_fetch_max(ref op:valType, l:int(32), obj:c_ptr(void), ref result:valType, order:memory_order): void;

      var ret:valType;
      var v = val;
      atomic_fetch_max(v, _localeid(), _addr(), ret, c_memory_order(order));
      return ret;
    }

    inline proc max(val:valType, param order: memoryOrder = memoryOrder.seqCst): void {
      pragma "insert line file info" extern externFunc("max", valType)
        proc atomic_max(ref op:valType, l:int(32), obj:c_ptr(void), order:memory_order): void;

      var v = val;
      atomic_max(v, _localeid(), _addr(), c_memory_order(order));
    }
