...
module ChapelDynamicLoading {
  ...
  class chpl_BinaryInfo {
      ...
      on Locales[0] {
        ...
        if shouldCreateNewEntry {
          ...
          if numErrs.read() > 0 {
            ...
            manage store.handleToInfo as m {
    ...
    proc loadSymbol(sym: string, type t, out err: owned DynLoadError?) {
      ...
      on Locales[0] {
        ...
        const ptrOnThis = localDynLoadSymbolLookup(sym, handle, err);

        if err != nil || ptrOnThis == nil {
          ...
          if found {
            ...
            if idx != 0 {
              ...
              const data = (idx, sym);
              const added = _procPtrToDataLocale0.set(ptrOnThis, data);
              assert(added);
            }
          }
        }
  ...
  inline proc fetchDynamicIdxForStaticIdx(idx: int): int {
    ...
    on Locales[0] {
      manage chpl_localPtrCache.guard as m {
