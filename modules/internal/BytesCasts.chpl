...
module BytesCasts {
  ...
  operator :(x: bytes, type t:integral) throws {
    ...
    if isIntType(t) {
      select numBits(t) {
        when 8  do retVal = c_string_to_int8_t(localX:c_ptrConst(c_char):c_string, isErr);
        when 16 do retVal = c_string_to_int16_t(localX:c_ptrConst(c_char):c_string, isErr);
        when 32 do retVal = c_string_to_int32_t(localX:c_ptrConst(c_char):c_string, isErr);
        when 64 do retVal = c_string_to_int64_t(localX:c_ptrConst(c_char):c_string, isErr);
        otherwise compilerError("Unsupported bit width ", numBits(t),
                                " in cast from bytes to " + t:string);
      }
    } else {
      select numBits(t) {
        when 8  do retVal = c_string_to_uint8_t(localX:c_ptrConst(c_char):c_string, isErr);
        when 16 do retVal = c_string_to_uint16_t(localX:c_ptrConst(c_char):c_string, isErr);
        when 32 do retVal = c_string_to_uint32_t(localX:c_ptrConst(c_char):c_string, isErr);
        when 64 do retVal = c_string_to_uint64_t(localX:c_ptrConst(c_char):c_string, isErr);
        otherwise compilerError("Unsupported bit width ", numBits(t),
                                " in cast from bytes to " + t:string);
      }
    }
  ...
  operator :(x: bytes, type t:chpl_anyreal) throws {
    ...
    select numBits(t) {
      when 32 do retVal = c_string_to_real32(localX:c_ptrConst(c_char):c_string, isErr);
      when 64 do retVal = c_string_to_real64(localX:c_ptrConst(c_char):c_string, isErr);
      otherwise compilerError("Unsupported bit width ", numBits(t),
                              " in cast to bytes");
    }
  ...
  operator :(x: bytes, type t:chpl_anyimag) throws {
    ...
    select numBits(t) {
      when 32 do retVal = c_string_to_imag32(localX:c_ptrConst(c_char):c_string, isErr);
      when 64 do retVal = c_string_to_imag64(localX:c_ptrConst(c_char):c_string, isErr);
      otherwise compilerError("Unsupported bit width ", numBits(t),
                              " in cast to bytes");
    }
  ...
  operator :(x: bytes, type t:chpl_anycomplex) throws {
    ...
    select numBits(t) {
      when 64 do retVal = c_string_to_complex64(localX:c_ptrConst(c_char):c_string, isErr);
      when 128 do retVal = c_string_to_complex128(localX:c_ptrConst(c_char):c_string, isErr);
      otherwise compilerError("Unsupported bit width ", numBits(t),
                              " in cast to bytes");
    }
