...
module ArrayViewLocal {
  ...
  class ArrayViewLocalArr: AbsBaseArr {
    ...
    // and then returning a new ArrayViewLocalArr() instance referring
    // to them.
    //
    proc type chpl__deserialize(data) {
      ...
      return new unmanaged ArrayViewLocalArr(eltType=arr.eltType,
                                             _DomPid=data(0),
                                             dom = dom,
                                             _ArrPid=data(1),
                                             _ArrInstance=arr);
    ...
    iter these() ref {
      if shouldUseIndexCache() {
        ...
        if arr.dsiHasSingleLocalSubdomain() {
          for i in arr.dsiLocalSubdomain(here) do
            yield arr.dsiLocalAccess(i);
        } else {
          for locDom in arr.dsiLocalSubdomains(here) do
            for i in locDom do
              yield arr.dsiLocalAccess(i);
        }
      }
    ...
    iter these(param tag: iterKind) ref
      ...
      if arr.dsiHasSingleLocalSubdomain() {
        forall i in arr.dsiLocalSubdomain(here) do yield arr.dsiLocalAccess(i);
      } else {
        for locDom in arr.dsiLocalSubdomains(here) do
          forall i in locDom do yield arr.dsiLocalAccess(i);
      }
    }
    ...
    iter these(param tag: iterKind) where tag == iterKind.leader {
      //
      // TODO:
      // This iterator needs to yield a 'followThis' from privDom's
      // perspective. One possibility would be to require some kind of special
      // leader iterator if the distribution wishes to support local slices.
      //
      // We might be able to do something general-purpose if the distribution's
      // local subdomain is rectangular. It's less clear what should be done
      // for associative, sparse, or some unknown kind of distribution.
      compilerError("local slices may not lead zippered iteration");

      // Dummy yields to get past internal compiler errors
      for followThis in privDom.these(tag) do yield followThis;
    }

    iter these(param tag: iterKind, followThis) ref
      where tag == iterKind.follower {
      for i in privDom.these(tag, followThis) do yield arr.dsiLocalAccess(i);
    }
    ...
    proc dsiSerialWrite(f) throws {
      if arr.dsiHasSingleLocalSubdomain() then
        chpl_serialReadWriteRectangular(f, arr, arr.dsiLocalSubdomain(f.readWriteThisFromLocale()));
      else
        compilerError("local slice has multiple local subdomains and cannot be read or written");
    }
    ...
    proc dsiPrivatize(privatizeData) {
      return new unmanaged ArrayViewLocalArr(eltType=this.eltType,
