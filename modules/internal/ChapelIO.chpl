...
The :proc:`write` method an also be called on strings to write the output to a
...
If a none of these routines are provided, a default version of ``readThis`` and
``writeThis`` will be generated by the compiler. If ``readWriteThis`` is
defined, the compiler will generate ``readThis`` or ``writeThis`` - if they do
not already exist - which call ``readWriteThis``.

Objects of type class:`Reader` or class:`Writer` both support the following
fields and methods:

 * :attr:`Reader.writing` :attr:`Writer.writing` returns `true` if called on a class:``Writer`` and `false` if called on a class:``Reader``.
 * :attr:`Reader.binary` :attr:`Writer.binary` returns `true` if called on a Reader or Writer that is configured to perform binary I/O, and `false` otherwise.
 * :proc:`Reader.error` :proc:`Writer.error` returns a saved error code.
 * :proc:`Reader.setError` :proc:`Writer.setError` save an error code.
 * :proc:`Reader.clearError` :proc:`Writer.clearError` clear any saved error code.
 * :proc:`Reader.readwrite` :proc:`Writer.readwrite` read or write a value according to its readThis or writeThis method.
 * :proc:`Reader.readWriteLiteral` :proc:`Writer.readWriteLiteral` read or write a literal string value.
 * :proc:`Reader.readWriteNewline` :proc:`Writer.readWriteNewline` reads and discards input until a newline or writes a newline.
...
The :class:`Writer` class contains no arguments and serves as a base class to
...
that any Chapel primitive type as an argument.
...
* for a `domain` argument,  outputs the dimensions of the domain enclosed by
  ``[`` and ``]``.
* for a `range` argument, output the lower bound of the range followed by
  ``..`` followed by the upper bound of the range.  If the stride of the range
  is not one, the output is additionally followed by the word ``by`` followed
  by the stride of the range.
...
module ChapelIO {
  ...
  /* Overlead to support reading an ioLiteral without
  ...
  /* Overlead to support reading an ioNewline without
  ...
  /* Explicit call for reading a literal as an
     alternative to using ioLiteral.
   */
  ...
  /* Explicit call for reading a newline as an
     alternative to using ioNewline.
   */
