...
#ifdef HAVE_MEMALIGN
    qt_threadqueue_private_t * const ret = memalign(qthread_cacheline(), sizeof(qt_threadqueue_private_t));
#elif defined(HAVE_POSIX_MEMALIGN)
    qt_threadqueue_private_t *ret = NULL;
    posix_memalign(&(ret), qthread_cacheline(), sizeof(qt_threadqueue_private_t));
#elif defined(HAVE_WORKING_VALLOC)
    qt_threadqueue_private_t * const ret = valloc(sizeof(qt_threadqueue_private_t));
#elif defined(HAVE_PAGE_ALIGNED_MALLOC)
    qt_threadqueue_private_t * const ret = malloc(sizeof(qt_threadqueue_private_t));
#else
    qt_threadqueue_private_t * const ret = valloc(sizeof(qt_threadqueue_private_t));  /* cross your fingers */
#endif
    return ret;
}
