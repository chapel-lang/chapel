...
typedef void (*qt_dict_cleanup_f)(void *, void *);
 ...
 *
 * the signature of the key comparsion function is: int my_key_equals(void*
 * key1, void* key2) and it should return 1 ("true") if the keys are equal and 0
 * otherwise
 *
 * the signature of the qt_dict_hash_f function is: int my_hashcode(void*
 * string) and it can return any integer value.
 *
 ...
 *      d must be empty (keys and values must have been cleaned up already (or
 * else, leaks)
 *
 */
 ...
 *      returns void*:
 *                      ADDR - address of item found in the hashmap after the
 * current put NULL - if the insert failed because of an error
 *
 */
 ...
 *      returns void*:
 *                      ADDR - address of item found in the hashmap after the
 * current put (if "key" was present in the dictionary, the address of the old
 ...
 *                      NULL - if item was not present in the dictionary or
 * error
 *
 */
 ...
 *      returns:
 *                      addr - address of new iterator if creation was
 * successful ERROR - if an error occurred
 *
 */
 ...
 *      returns:
 *                      addr - address of the next entry in the hash, if one was
 * found NULL -  if there are no more elements in hash ERROR -  if an error
 * occurred (e.g. iterator was destroyed) Note: For handling concurrency, if an
 * item was deleted while using the iterator, the behavior is undefined (can
 * return an error, signal empty hash or correctly return the next entry)
 */
...
/*
 *      Creates a new iterator, pointing to the item past the end of the list
 * (which is NULL) returns: addr - address of the newly created iterator NULL -
 * if dictionary is null or not allocated properly
 */
qt_dictionary_iterator *qt_dictionary_end(qt_dictionary *dict);

/*
 *      Tests equality for two iterators
 *      requires that they are both walking the same dictionaries and are
 * located on the same positions
 */
