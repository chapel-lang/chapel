...
template <typename T>
typename T::acctype
qt_loopaccum_balance(size_t start, size_t stop, T const &obj) {
  typename T::acctype accumulate(T::identity);
  qt_loopaccum_balance(start,
                       stop,
                       sizeof(typename T::acctype),
                       &accumulate,
                       qloop_accum_cpp_wrapper<T>,
                       &(const_cast<T &>(obj)),
                       (qt_accum_f)(T::accumulate));
  return accumulate;
}
