...
#define STRUCT(_structname_, _rtype_)                                          \
  struct _structname_ {                                                        \
    const _rtype_ *array;                                                      \
...
#define INNER_LOOP(_fname_, _structtype_, _opmacro_)                           \
  static aligned_t _fname_(void *args_void) {                                  \
...
#define INNER_LOOP_FF(_fname_, _structtype_, _opmacro_)                        \
  static aligned_t _fname_(struct _structtype_ *args) {                        \
    size_t i;                                                                  \
...
#define OUTER_LOOP(                                                            \
  _fname_, _structtype_, _opmacro_, _rtype_, _innerfunc_, _innerfuncff_)       \
  _rtype_ API_FUNC _fname_(                                                    \
    const _rtype_ *array, size_t length, int checkfeb) {                       \
    size_t i, start = 0;                                                       \
    ...
    while (start + MT_LOOP_CHUNK < length) {                                   \
      /* spawn off an MT_LOOP_CHUNK-sized segment of the first part of the     \
       * array */                                                              \
    ...
    if (checkfeb) {                                                            \
      ...
      myret = array[start];                                                    \
      for (i = start + 1; i < length; i++) { _opmacro_(myret, array[i]); }     \
    }                                                                          \
    if (waitfor) {                                                             \
...
#define MAX_MACRO(max, contender)                                              \
  if (max < (contender)) max = (contender)
#define MIN_MACRO(max, contender)                                              \
  if (max > (contender)) max = (contender)
...
OUTER_LOOP(qutil_double_sum,
           qutil_ds_args,
           SUM_MACRO,
           double,
           qutil_double_sum_inner,
           qutil_double_FF_sum_inner)
...
OUTER_LOOP(qutil_uint_sum,
           qutil_uis_args,
           SUM_MACRO,
           aligned_t,
           qutil_uint_sum_inner,
           qutil_uint_FF_sum_inner)
...
typedef int (*cmp_f)(void const *a, void const *b);
...
void API_FUNC qutil_mergesort(double *array, size_t length) { /*{{{*/
  ...
  for (i = 0; i < numthreads; i++) { qthread_readFF(NULL, rets + i); }
  ...
  if (chunksize <= length) {
    ...
    while (chunksize <= length) {
      ...
      while (i < length - chunksize) {
        ...
        args[numthreads].second_stop = ((i + 2 * chunksize - 1) < (length - 1))
                                         ? (i + 2 * chunksize - 1)
                                         : (length - 1);
        qthread_fork((qthread_f)qutil_mergesort_inner,
                     args + numthreads,
                     rets + numthreads);
        i += 2 * chunksize;
        numthreads++;
...
static inline qutil_qsort_iprets_t qutil_qsort_inner_partitioner(
  ...
  size_t const megachunk_size = MT_CHUNKSIZE * numthreads;
...
static inline aligned_t qutil_qsort_inner(void *a_void) { /*{{{*/
  ...
  /* tri-median pivot selection */
  {
    size_t i;
    i = a->length / 2;
    if (array[0] > array[i]) { SWAP(double, array, 0, i); }
   ...
   * be partitioned correctly */
  {
    ...
    /* now, spawn the next two iterations */
    {
      ...
      if (rets[0] == 0) { qthread_readFF(NULL, rets); }
      if (rets[1] == 0) { qthread_readFF(NULL, rets + 1); }
    }
  }
  return 0;
} /*}}}*/
...
static inline aligned_t qutil_aligned_qsort_partition(void *args_void) { /*{{{*/
  ...
  aligned_t const pivot = args->pivot;
  size_t const length = args->length;
  size_t const jump = args->jump;
  size_t leftwall, rightwall;
...
static inline qutil_qsort_iprets_t qutil_aligned_qsort_inner_partitioner(
  ...
  for (i = 0; i < numthreads; i++) {
    ...
    qthread_fork_syncvar(
      (qthread_f)qutil_aligned_qsort_partition, args + i, rets + i);
  }
...
static inline aligned_t qutil_aligned_qsort_inner(void *a_void) { /*{{{*/
  ...
  /* tri-median pivot selection */
  {
    size_t i;
    i = a->length / 2;
    if (array[0] > array[i]) { SWAP(aligned_t, array, 0, i); }
    if (array[0] > array[a->length - 1]) {
  ...
  while (furthest.rightwall > furthest.leftwall &&
    ...
    furthest = qutil_aligned_qsort_inner_partitioner(
      a->array + furthest.leftwall,
      furthest.rightwall - furthest.leftwall + 1,
      pivot);
    furthest.leftwall += offset;
    furthest.rightwall += offset;
