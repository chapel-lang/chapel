...
void INTERNAL qt_topology_init(qthread_shepherd_id_t *nshepherds,
  ...
  num_workers =
    qt_internal_get_env_num("HWPAR", num_sheps * num_wps, num_sheps * num_wps);
  ...
  if (THREADQUEUE_POLICY_TRUE == qt_threadqueue_policy(SINGLE_WORKER)) {
    if (0 == num_wps) {
      ...
      print_warning("Disregarding request for %d workers per shepherd "
                    "specified, scheduler only supports 1.\n",
                    num_wps);
      num_wps = 1;
    }
  } else {
    if (0 < num_wps && 0 == num_sheps) {
       ...
       * WPS */
      print_warning("Number of shepherds not specified - number of workers may "
                    "be ignored\n");
    }
  }
  ...
  if (num_workers != 0) {
    if ((num_workers < num_sheps) || (num_workers == 1)) {
      ...
      if ((num_workers % num_sheps) != 0) { num_wps++; }
    } else {
      num_wps = num_workers / num_sheps;
      if (num_workers % num_sheps > 0) { num_wps++; }
    }
  ...
  if (THREADQUEUE_POLICY_TRUE == qt_threadqueue_policy(SINGLE_WORKER) &&
      1 != num_wps) {
    print_error("attempted to use %i workers per shepherd with scheduler that "
                "only supports 1 worker per shepherd.\n",
                (int)num_wps);
    exit(EXIT_FAILURE);
  }
