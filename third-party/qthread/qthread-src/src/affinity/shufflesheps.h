...
extern inline INTERNAL void sort_sheps(unsigned int *restrict dists,
  ...
  size_t s_max = num - 1;
  qthread_shepherd_id_t *tmp =
    qt_calloc(sizeof(qthread_shepherd_id_t), (num - 1));
  size_t tmp_base = 0;
  size_t tmp_max = 0;
  ...
  while (s_max > 0) {
    ...
    for (i = 0; i < s_max;
         ++i) { // find the minimum distance of the remaining sheps
      if (mindist > dists[s[i]]) { mindist = dists[s[i]]; }
    }
    for (i = 0; i < s_max; ++i) { // copy all minimum-distance sheps to tmp
      if (dists[s[i]] == mindist) { tmp[tmp_max++] = s[i]; }
    }
