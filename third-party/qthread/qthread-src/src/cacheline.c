...
static void cpuid(unsigned int const op,
...
static void figure_out_cacheline_size(void) {
...
#endif
  } else if ((ebx == 0x68747541) && (ecx == 0x444d4163) &&
             (edx == 0x69746e65)) {
    largest_std = eax;
    v = AMD;
#ifdef DEBUG_CPUID
  ...
  if (v == Intel) {
    ...
    if (largest_std >= 4) {
      ...
      if ((cache > 0) && (cacheline_bytes > 0)) { return; }
    }
  }
