...
qarray_internal_shepof_segidx(qarray const *a, size_t const seg) {
  switch (a->dist_type) {
    ...
    case FIXED_FIELDS: {
      size_t const segs_per_shep = a->dist_specific.stripes.segs_per_shep;
      size_t const extras = a->dist_specific.stripes.extras;
    ...
    case DIST:
      return qarray_internal_segment_shep_read(
        a, qarray_elem_nomigrate(a, seg * a->segment_size));
...
qarray_internal_shepof_ch(qarray const *a, void const *segment_head) {
  switch (a->dist_type) {
    ...
    case FIXED_FIELDS: {
      size_t const bytes_in = (uintptr_t)segment_head - (uintptr_t)a->base_ptr;
      size_t const segments_in = bytes_in / a->segment_bytes;
...
static qarray *qarray_create_internal(size_t const count,
  ...
  switch (d) {
    ...
    case ALL_RAND:
    case ALL_LEAST:
    case ALL_SAME: ret->dist_type = ALL_SAME; break;
    case FIXED_FIELDS: ret->dist_type = FIXED_FIELDS; break;
    case FIXED_HASH:
    default: ret->dist_type = FIXED_HASH; break;
    case DIST_STRIPES:
    case DIST_FIELDS:
    case DIST_RAND:
    case DIST_LEAST:
    case DIST: ret->dist_type = DIST; break;
  }
  ...
  switch (d) {
    ...
    case ALL_LEAST: {
      ...
      for (i = 1; i < qthread_num_shepherds(); i++) {
        if (chunk_distribution_tracker[i] < chunk_distribution_tracker[least]) {
          least = i;
        }
      }
  ...
  switch (d) {
    ...
    default:
      if (qthread_internal_shep_to_node(ret->dist_specific.dist_shep) ==
        ...
        ret->base_ptr = (char *)qt_affinity_alloc_onnode(
          segment_count * ret->segment_bytes,
          qthread_internal_shep_to_node(ret->dist_specific.dist_shep));
      }
      break;
  }
    ...
    for (segment = 0; segment < segment_count; segment++) {
      switch (d) {
        ...
        case FIXED_FIELDS:
          ...
          break;
        case DIST: /* assumed equivalent to DIST_RAND */
        case DIST_RAND: target_shep = random() % max_sheps; break;
        case DIST_FIELDS: { /* roughly copied from FIXED_FIELDS logic */
  ...
  qgoto(badret_exit);
  if (ret) {
    if (ret->base_ptr) { qt_free(ret->base_ptr); }
...
API_FUNC void qarray_destroy(qarray *a) {
  ...
  switch (a->dist_type) {
    case DIST: {
      size_t segment;
      size_t const segment_count =
        a->count / a->segment_size + ((a->count % a->segment_size) ? 1 : 0);
      for (segment = 0; segment < segment_count; segment++) {
        char *segmenthead = qarray_elem_nomigrate(a, segment * a->segment_size);
        qthread_incr(
...
qthread_shepherd_id_t qarray_shepof(qarray const *a, size_t const index) {
  ...
  switch (a->dist_type) {
    ...
    default: {
      ...
      qthread_shepherd_id_t const retval =
...
void *qarray_elem_migrate(qarray const *a, size_t const index) {
  ...
  qassert_ret((index >= a->count), NULL);
  {
    ...
    ret =
      segment_head + ((index - segment_num * a->segment_size) * a->unit_size);
...
static aligned_t qarray_strider(void *arg_void) {
  ...
  distribution_t const dist_type = arg->a->dist_type;
  ...
  switch (dist_type) {
    case ALL_SAME:
      if (shep != arg->a->dist_specific.dist_shep) { goto qarray_strider_exit; }
      break;
    case FIXED_FIELDS: {
    ...
    default: // use this when our starting point is somewhat unpredictable
      if ((count > 0) && (qarray_shepof(arg->a, count) != shep)) {
        ...
        if (count >= max_count) { goto qarray_strider_exit; }
      }
  ...
  while (1) {
    ...
    size_t const max_offset =
      ((max_count - count) > segment_size) ? segment_size : (max_count - count);
    ...
    switch (dist_type) {
      case FIXED_FIELDS:
      case ALL_SAME: count += segment_size; break;
      case FIXED_HASH:
      default: count += segment_size * qthread_num_shepherds(); break;
...
static aligned_t qarray_loop_strider(void *arg_void) {
  ...
  size_t max_count = arg->stopat;
  size_t count = arg->startat;
  qa_loop_f const ql = arg->func.ql;
  ...
  switch (dist_type) {
    ...
    case FIXED_FIELDS: {
      ...
      if (shep != start_shep) {
        ...
        /* this relies on sheps being zero-indexed */
        count =
          shep * segment_size * arg->a->dist_specific.stripes.segs_per_shep;
      }
      {
    ...
    default:
      if ((count > 0) && (qarray_shepof(arg->a, count) != shep)) {
        /* jump to the next segment boundary */
        count += segment_size - (count % segment_size);
        if (count >= max_count) { goto qarray_loop_strider_exit; }
      }
      while (qarray_shepof(arg->a, count) != shep) {
        count += segment_size;
        if (count >= max_count) { goto qarray_loop_strider_exit; }
  ...
  switch (dist_type) {
    ...
    case FIXED_FIELDS:
      ...
      goto qarray_loop_strider_exit;
    default: /* aka NOT the special case */ break;
  }
  while (1) {
    {
      size_t const max_offset = ((max_count - count) > segment_size)
                                  ? segment_size
                                  : (max_count - count);
      ql(count, count + max_offset, arg->a, arg->arg);
    }
    switch (dist_type) {
      default: QTHREAD_TRAP(); break;
      case FIXED_HASH: count += segment_size * qthread_num_shepherds(); break;
      case DIST: /* XXX: this is awful - slow and bad for cache */
        count += segment_size;
        if (count >= max_count) { goto qarray_loop_strider_exit; }
        while (qarray_shepof(arg->a, count) != shep) {
...
static aligned_t qarray_loopaccum_strider(void *arg_void) {
  ...
  distribution_t const dist_type = arg->a->dist_type;
  qthread_shepherd_id_t const shep = qthread_shep();
  qt_accum_f const acc = arg->acc;
  qa_loopr_f const ql = arg->func.ql;
  size_t max_count = arg->stopat;
  size_t count = arg->startat;
  char *myret = arg->ret;
  char *tmpret = NULL;
...
API_FUNC void qarray_iter(qarray *a,
  ...
  switch (a->dist_type) {
    case ALL_SAME:
      qthread_fork_to(
        (qthread_f)qarray_strider, &qfwa, NULL, a->dist_specific.dist_shep);
      while (donecount == 0) { qthread_yield(); }
      break;
    case FIXED_FIELDS: {
      ...
      while (donecount < num_spawns) { qthread_yield(); }
      break;
    }
    default:
      ...
      if (stopat - startat < a->segment_size) {
        qthread_fork_to(
          (qthread_f)qarray_strider, &qfwa, NULL, qarray_shepof(a, startat));
        while (donecount == 0) { qthread_yield(); }
      } else {
        qthread_shepherd_id_t i;
...
API_FUNC void qarray_iter_loop(qarray *a,
  ...
  switch (a->dist_type) {
    case ALL_SAME:
      qthread_fork_to((qthread_f)qarray_loop_strider,
                      &qfwa,
                      NULL,
    ...
    default:
      ...
      if (stopat - startat < a->segment_size) {
        ...
        for (i = 0; i < maxsheps; i++) {
          qthread_fork_to((qthread_f)qarray_loop_strider, &qfwa, NULL, i);
        }
        while (donecount < maxsheps) { qthread_yield(); }
      }
      break;
...
API_FUNC void qarray_iter_loopaccum(qarray *a,
  ...
  switch (a->dist_type) {
    ...
    case FIXED_FIELDS: {
      qthread_shepherd_id_t start_shep = qarray_shepof(a, startat);
      qthread_shepherd_id_t stop_shep = qarray_shepof(a, stopat - 1);
      size_t const num_spawns = (stop_shep - start_shep) + 1;
      struct qarray_accumfunc_wrapper_args *qfwa =
        MALLOC(sizeof(struct qarray_accumfunc_wrapper_args) * num_spawns);
      ...
      for (qthread_shepherd_id_t s = start_shep; s <= stop_shep; s++) {
        ...
        if (s > start_shep) { qfwa[i].ret = rets + ((i - 1) * retsize); }
        qthread_fork_to(
          (qthread_f)qarray_loopaccum_strider, &qfwa[i], &rv[i], s);
      }
      for (i = 0; i < num_spawns; i++) {
        qthread_readFF(NULL, &(rv[i]));
        if (i > 0) { acc(ret, &rets[i - 1]); }
      }
    ...
    default:
      ...
      {
        qthread_shepherd_id_t i;
        size_t const segsize = a->segment_size;
        qthread_shepherd_id_t const maxsheps = qthread_num_shepherds();
        struct qarray_accumfunc_wrapper_args *qfwa =
        ...
        assert(rv);
        qfwa[0].ret = ret;
        if ((QT_CEIL_RATIO(a->count, segsize) /*tot_segs*/ / maxsheps) >
            /*range_segs*/ ((stopat - startat) / segsize)) {
          ...
          for (i = 0; i < maxsheps; i++) {
            if (rv[i] == 0) {
              ...
              qfwa[i].stopat = stopat;
              qfwa[i].retsize = retsize;
              if (i > 0) { qfwa[i].ret = rets + ((i - 1) * retsize); }
              qthread_fork_to(
                (qthread_f)qarray_loopaccum_strider, &qfwa[i], &rv[i], i);
            }
          }
          {
            int first = 1;
          ...
          for (i = 0; i < maxsheps; i++) {
            qthread_readFF(NULL, &(rv[i]));
            if (i > 0) { acc(ret, rets + ((i - 1) * retsize)); }
          }
        }
  ...
  switch (ref->dist_type) {
    case ALL_SAME: {
      ...
      switch (mod->dist_type) {
        case ALL_SAME: qarray_set_shepof(mod, 0, shep); break;
        case DIST:
          for (i = 0; i < mod->count; i += mod->segment_size) {
            qarray_set_shepof(mod, i, shep);
          }
          break;
        default: /* should not happen *ever*, so trigger a segfault for corefile
                    analysis */
          QTHREAD_TRAP();
      }
      break;
