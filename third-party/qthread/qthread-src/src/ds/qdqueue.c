...
struct qdqueue_adheap_s {
  aligned_t
    gateway_lock; /* unnecessary, but avoids compiler warnings about punning */
...
qdqueue_adheap_pop(struct qdqueue_adheap_s *heap) { /*{{{ */
  ...
  if (heap->first != NULL) {
    ...
    ret = tmp->ad;
    if ((heap->first = tmp->next) != NULL) { heap->first->prev = NULL; }
    tmp->inheap = 0;
    qthread_unlock(&(heap->gateway_lock));
  } else {
  emptyheap:
 ...
 * yet) */
/*static void qdqueue_adheap_pushcond(qlfqueue_t * heap, void *shep, aligned_t
 * gen)
 * {
 *  printf("pushcond\n");
 * }*/
...
static void qdqueue_adheap_push(struct qdqueue_adheap_s *heap,
  ...
  for (i = 0; i < maxsheps; i++) {
    if (heap->heap[i].ad.shep == shep) { break; }
  }
  assert(heap->heap[i].ad.shep == shep);
  qthread_lock(&(heap->gateway_lock));
  /* update the gen record */
  if ((heap->heap[i].ad.generation < gen) || (gen == 0)) {
    if (gen != 0) { heap->heap[i].ad.generation = gen; }
    if (heap->heap[i].inheap == 1) { goto done_pushing; }
    ...
    if (heap->first == NULL) {
      ...
      if (&(heap->heap[i]) < heap->first) {
         ...
         * ... so we start here and work backwards until we find some part of
         * the heap */
        for (j = i - 1;; j--) {
...
static void qdqueue_internal_sortedsheps(qthread_shepherd_id_t shep,
  ...
  for (i = 0; i < maxsheps; i++) {
    if (maxdist <= distances[i]) { maxdist = distances[i] + 1; }
  }
  ...
  while (i < (maxsheps - 1)) { /* -1 because we're not recording *self* */
    ...
    for (j = 0, k = 0; j < maxsheps && k < count; j++) {
      if (j == shep) { continue; }
      if (distances[j] == mindist) { thisdist[k++] = j; }
    }
    assert(k == count);
...
#define NONME_DIST 10
  ...
  for (i = 0; i < maxsheps; i++) {
    if (i == shep) { continue; }
    if (distances[i] < (mindist + NONME_DIST)) { numN++; }
  }
  ...
  for (i = 0; i < numN; i++) {
    ...
    for (k = randpick; k < (numN - i - 1); k++) { temp[k] = temp[k + 1]; }
  }
...
qdqueue_t *qdqueue_create(void) { /*{{{ */
  ...
  if (maxsheps == 0) { maxsheps = qthread_num_shepherds(); }
  assert(maxsheps > 0);
  ...
  for (curshep = 0; curshep < maxsheps; curshep++) {
    ...
    qdqueue_internal_sortedsheps(
      curshep, ret->Qs[curshep].allsheps, ret->Qs, sheparray[curshep]);
    ret->Qs[curshep].neighbors = qdqueue_internal_getneighbors(
      curshep, ret->Qs, &(ret->Qs[curshep].nNeighbors), sheparray[curshep]);
  ...
  if (ret) {
    if (ret->Qs) { FREE(ret->Qs, maxsheps * sizeof(struct qdsubqueue_s)); }
...
int qdqueue_destroy(qdqueue_t *q) { /*{{{ */
  ...
  for (i = 0; i < maxsheps; i++) {
    if (q->Qs[i].theQ) { qlfqueue_destroy(q->Qs[i].theQ); }
    ...
    if (q->Qs[i].neighbors != NULL) {
      FREE(q->Qs[i].neighbors,
           q->Qs[i].nNeighbors * sizeof(struct qdsubqueue_s *));
    }
  ...
  checkads: {
  ...
  }
    {
      ...
      for (shep = 0; shep < (maxsheps - 1); shep++) {
        ...
        if (!qdqueue_adheap_empty(&myq->ads)) { goto checkads; }
      }
    }
    return NULL;
  }
  ...
  if (!qlfqueue_empty(myq->theQ)) {
    ...
    for (shep = 0; shep < (maxsheps - 1); shep++) {
      ...
      if (remoteshep->last_consumed == remoteshep) {
        /* it's working on its own queue */
        if (!qlfqueue_empty(remoteshep->theQ)) { return 0; }
      }
    }
