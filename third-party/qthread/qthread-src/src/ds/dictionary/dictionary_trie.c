...
static void destroy_spine(qt_hash h, spine_t *spine, qt_dict_cleanup_f f);
static void destroy_element(hash_entry *element, qt_dict_cleanup_f f);
...
static inline qt_hash qt_hash_create(qt_dict_key_equals_f eq,
  ...
  tmp->spines =
    (spine_t **)qt_calloc(tmp->maxspines, sizeof(spine_element_t *));
...
#define SPINE_BUCKET(key, depth)                                               \
  ((((uintptr_t)(key)) >>                                                      \
    (BASE_BUCKET_WIDTH + (SPINE_BUCKET_WIDTH * ((depth) - 1)))) &              \
   (SPINE_LENGTH - 1))
#define SPINE_COUNT(x) ((x << 1) | 1)
#define SPINE_PTR_UPDATE(ptr, count)                                           \
  ((spine_element_t)(uint64_t)(SPINE_PTR_ID(ptr) | SPINE_COUNT(count)))
...
#define DECREMENT_COUNT_BY(bkt, decrby)                                        \
  {                                                                            \
    ...
    if (count == 0) { deallocate_spine(h, oldval.s.id); }                      \
  }
#define DECREMENT_COUNT(bkt) DECREMENT_COUNT_BY(bkt, 1)

static void deallocate_spine(qt_hash h, size_t id) {
...
static size_t allocate_spine(qt_hash h, spine_t **realspine) {
  ...
  assert(id < maxspines);
  if (realspine != NULL) { *realspine = (spine_t *)newspine; }

  return id;
}

static void
destroy_spine(qt_dictionary *h, spine_t *spine, qt_dict_cleanup_f f) {
  assert(spine);
...
static void destroy_element(hash_entry *element, qt_dict_cleanup_f f) {
  assert(element);
  if (f) { f(element->key, element->value); }
...
void *
qt_hash_put_helper(qt_dictionary *h, qt_key_t key, void *value, int put_choice);
...
void *qt_hash_put_helper(qt_dictionary *h,
  ...
  hash_entry *e =
    qt_malloc(sizeof(hash_entry)); // XXX: should be from a memory pool
  ...
  do {
    if (child_val.e == NULL) {
      ...
      if (bucket1 != bucket2) { // both elements will be in the new spine
        ...
        if ((cur.e = CAS(&(child_id->e), child_val.e, newspine.e)) ==
            child_val.e) {
          // success!
          if (cur_id) { DECREMENT_COUNT(cur_id); }
          return value;
        ...
        while (crt) {
          if (h->op_equals(crt->key, key)) {
            ...
            if (put_choice != PUT_IF_ABSENT) {
              ...
              while ((qthread_cas_ptr(crt_val_adr, crt_val, value)) !=
                     crt_val) {
                crt_val = crt->value;
              }
            }
      ...
      do {
        if (h->op_equals(e->key, key)) {
          ...
          if ((cur.e = CAS(prev, e, e->next)) == e) {
            ...
            // Second, walk back up the parent pointers, removing empty spines
            // (if any) cur_id is the current spine pointer's location (if its
            // null, we're in the base spine)
            while (cur_id) {
  ...
  do {
    if (child_val.e == NULL) {
      ...
      if (child_val.e->hashed_key == lkey) {
        ...
        while (e) {
          if (h->op_equals(e->key, key)) { return e->value; }
          e = e->next;
        }
        return NULL;
      } else {
 ...
 * #ifdef EBUG
 *                             (void
 * *)(intptr_t)SPINE_PTR_COUNT(spine->elements[i]), #else arg, #endif depth +
 * 1); } else { DEBUG(for (size_t j = 0; j < depth; ++j) printf("%li ",
 * (intptr_t)arg); ); f(spine->elements[i].e->hashed_key,
 * spine->elements[i].e->value, arg);
 *          }
 *      }
 *  }
 * }
 ...
 * { if (h->base[base_bucket].e != NULL) { if
 * (SPINE_PTR_TEST(h->base[base_bucket])) { // spine spine_callback(h,
 *                             SPINE_PTR(h, h->base[base_bucket]),
 *                             f,
 * #ifdef EBUG
 *                             (void
 * *)(intptr_t)SPINE_PTR_COUNT(h->base[base_bucket]), #else arg, #endif 1); }
 * else { // element f(h->base[base_bucket].e->hashed_key,
 * h->base[base_bucket].e->value, arg);
 *          }
 *      }
 *  }
 *  DEBUG(printf("============\n"));
