...
#define rlock(a)                                                               \
  {                                                                            \
    if (qthread_library_initialized == 1) {                                    \
      ...
      /* this if protects against crash when using after qthreads was          \
       * finalized*/                                                           \
      /* if qthreads is finalized, use sequential semantics */                 \
      if (id < MAX_READERS) { rwlock_rdlock(a, id); }                          \
    }                                                                          \
  }
...
#define runlock(a)                                                             \
  {                                                                            \
    if (qthread_library_initialized == 1) {                                    \
      int id = qthread_worker_unique(NULL);                                    \
      if (id < MAX_READERS) { rwlock_rdunlock(a, id); }                        \
    }                                                                          \
  }
#define wlock(a)                                                               \
  {                                                                            \
    if (qthread_library_initialized == 1) {                                    \
      int id = qthread_worker_unique(NULL);                                    \
      if (id < MAX_READERS) { rwlock_wrlock(a, id); }                          \
    }                                                                          \
  }
...
#define wunlock(a)                                                             \
  {                                                                            \
    if (qthread_library_initialized == 1) {                                    \
      int id = qthread_worker_unique(NULL);                                    \
      if (id < MAX_READERS) { rwlock_wrunlock(a); }                            \
    }                                                                          \
  }
#define rwlinit(a)                                                             \
  {                                                                            \
    /* fail instead of silently accepting more readers than we can handle*/    \
    /* setting the number of workers dynamically to be larger than             \
     * MAX_READERS*/                                                           \
...
qt_dictionary *qt_dictionary_create(qt_dict_key_equals_f eq,
  ...
  int i;
  for (i = 0; i < NO_BUCKETS; i++) { ret->content[i] = NULL; }
...
void qt_dictionary_destroy(qt_dictionary *d) {
  ...
  for (i = 0; i < NO_BUCKETS; i++) {
    ...
    while (top != NULL) {
      tmp = top;
      top = top->next;
      if (d->op_cleanup) { d->op_cleanup(tmp->key, tmp->value); }
    ...
    while (walk != stop && walk != NULL) {
      if ((walk->hashed_key == hash) && (dict->op_equals(walk->key, key))) {
        ...
        if (put_type == PUT_ALWAYS) {
          ...
          while ((qthread_cas_ptr(crt_val_adr, crt_val, value)) != crt_val) {
...
void *qt_dictionary_delete(qt_dictionary *dict, void *key) {
   ...
   * else if((walk -> hashed_key == hash) && (dict -> op_equals(walk -> key,
   * key))) {
   *      //remove list head
   *      to_free = walk;
   *      to_ret = walk -> value;
   ...
   *      if ((walk -> hashed_key == hash) && (dict -> op_equals(walk -> next ->
   * key, key))) { to_free = walk -> next; to_ret = walk -> next -> value; walk
   * -> next = walk -> next -> next; if (dict->op_cleanup != NULL)
   *                      dict->op_cleanup(to_free->key);
   *              free(to_free);
   *              break;
   *      }
  ...
  while (walk != NULL) {
    if ((walk->hashed_key == hash) && (dict->op_equals(walk->key, key))) {
      ...
      to_free = walk;
      to_ret = walk->value;
      if (dict->op_cleanup != NULL) { dict->op_cleanup(to_free->key, NULL); }
...
list_entry *qt_dictionary_iterator_next(qt_dictionary_iterator *it) {
  ...
  if (it->bkt == -1) {
    ...
    // if item was deleted or there are no more elements in the list, return
    // NULL
    if (walk == NULL) {
      it->bkt = -1;
      return ERROR;
    ...
    while (it->bkt < NO_BUCKETS) {
      ...
      it->bkt++;
      if (it->bkt < NO_BUCKETS) { walk = it->dict->content[it->bkt]; }
    }
