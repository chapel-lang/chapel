...
typedef void (*qt_hash_callback_fn)(qt_key_t const, void *, void *);
...
static int qt_lf_list_insert(marked_ptr_t *head,
  ...
  while (1) {
    ...
    if (qt_lf_list_find(head, hashed_key, key, &lprev, &cur, NULL, op_equals) !=
    ...
    node->next = (hash_entry *)CONSTRUCT(0, cur);
    if (qthread_cas(lprev,
                    (marked_ptr_t)(uintptr_t)node->next,
...
static int qt_lf_list_delete(marked_ptr_t *head,
  ...
  while (1) {
    ...
    if (qthread_cas(lprev, CONSTRUCT(0, lcur), CONSTRUCT(0, lnext)) ==
        CONSTRUCT(0, lcur)) {
      if (cleanup != NULL) { cleanup(PTR_OF(lcur)->key, NULL); }
      qpool_free(hash_entry_pool, PTR_OF(lcur));
    } else {
      qt_lf_list_find(head,
                      hashed_key,
                      key,
                      NULL,
                      NULL,
                      NULL,
                      op_equals); // needs to set cur/prev/next
    }
    return 1;
  }
}
  ...
  while (1) {
    ...
    while (1) {
      ...
      if (!MARK_OF(next)) { // if next pointer is not marked
        if (ckey >= hashed_key) {
          // if current key > hashed_key, the key isn't in the list; if current
          // key == hashed_key, the key IS in the list if (foundInsertPos == 0)
          // {
          if (oprev) { *oprev = prev; }
          if (ocur) { *ocur = cur; }
          if (onext) { *onext = next; }
        ...
        if (qthread_cas(prev, CONSTRUCT(0, cur), CONSTRUCT(0, next)) ==
            CONSTRUCT(0, cur)) {
...
static inline so_key_t so_regularkey(key_t const key) {
  return REVERSE(key | MSB);
}

static inline so_key_t so_dummykey(key_t const key) { return REVERSE(key); }

#define PUT_ALWAYS 0
#define PUT_IF_ABSENT 1

// old public method; added last param to distinguish between put and put if
// absent
static inline hash_entry *
qt_hash_put(qt_hash h, qt_key_t key, void *value, int put_choice) {
  ...
  if (h->B[bucket] == UNINITIALIZED) { initialize_bucket(h, bucket); }
...
// old public method
static inline void *qt_hash_get(qt_hash h, qt_key_t const key) {
  size_t bucket;
  uint64_t lkey = (uint64_t)(uintptr_t)(h->op_hash(key));
...
// old public method
static inline int qt_hash_remove(qt_hash h, qt_key_t const key) {
  size_t bucket;
  uint64_t lkey = (uint64_t)(uintptr_t)(h->op_hash(key));
  ...
  if (h->B[bucket] == UNINITIALIZED) { initialize_bucket(h, bucket); }
  if (!qt_lf_list_delete(&(h->B[bucket]),
                         so_regularkey(lkey),
                         key,
                         h->op_equals,
                         h->op_cleanup)) {
...
static void initialize_bucket(qt_hash h, size_t bucket) {
  ...
  if (h->B[parent] == UNINITIALIZED) { initialize_bucket(h, parent); }
 ...
 *      assert(MARK_OF(tmp) == 0);
 *      if (key & 1) {
 *          f((qt_key_t)(uintptr_t)REVERSE(key ^ 1), PTR_OF(cursor)->value,
 * arg); } else {
...
struct qt_dictionary_iterator {
  qt_dictionary *dict;
  list_entry *crt; // =NULL if iterator is newly created or reached the end;
                   // =crt elem otherwise.
...
static list_entry *
qt_dictionary_iterator_next_element(qt_dictionary_iterator *it) {
  ...
  if (it->crt == UNINITIALIZED) { return NULL; }
