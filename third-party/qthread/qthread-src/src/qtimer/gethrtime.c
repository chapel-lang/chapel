...
void qtimer_start(qtimer_t q) { q->start = gethrtime(); }

unsigned long qtimer_fastrand(void) { return (unsigned long)(gethrtime()); }

void qtimer_stop(qtimer_t q) { q->stop = gethrtime(); }
...
double qtimer_secs(qtimer_t q) { return ((double)(q->stop - q->start)) * 1e-9; }
