...
void INTERNAL initialize_hazardptrs(void) {
  ...
  for (qthread_shepherd_id_t i = 0; i < qthread_num_shepherds(); ++i) {
    for (qthread_worker_id_t j = 0; j < qlib->nworkerspershep; ++j) {
      memset(qlib->shepherds[i].workers[j].hazard_ptrs,
             0,
             sizeof(uintptr_t) * HAZARD_PTRS_PER_SHEP);
      memset(&qlib->shepherds[i].workers[j].hazard_free_list,
             0,
             sizeof(hazard_freelist_t));
      qlib->shepherds[i].workers[j].hazard_free_list.freelist =
        qt_calloc(freelist_max + 1, sizeof(hazard_freelist_entry_t));
      assert(qlib->shepherds[i].workers[j].hazard_free_list.freelist);
      qlib->shepherds[i]
        .workers[j]
        .hazard_free_list.freelist[freelist_max]
        .ptr = free_these_freelists;
      free_these_freelists =
        qlib->shepherds[i].workers[j].hazard_free_list.freelist;
    }
  }
  ...
  if (hzptrs == NULL) {
    {
      ...
      if (wkr == NULL) {
        hzptrs = qt_calloc(sizeof(uintptr_t), HAZARD_PTRS_PER_SHEP + 1);
        assert(hzptrs);
        do {
          hzptrs[HAZARD_PTRS_PER_SHEP] =
            (uintptr_t)QTHREAD_CASLOCK_READ(hzptr_list);
...
static void hazardous_scan(hazard_freelist_t *hfl) {
  ...
  assert(plist);
  tmpfreelist.freelist =
    qt_calloc(freelist_max, sizeof(hazard_freelist_entry_t));
  ...
  do {
        ...
        for (qthread_worker_id_t j = 0; j < qlib->nworkerspershep; ++j) {
          if (&(qlib->shepherds[i].workers[j].hazard_free_list) != hfl) {
            memcpy(plist + (i * qlib->nworkerspershep * HAZARD_PTRS_PER_SHEP) +
                     (j * HAZARD_PTRS_PER_SHEP),
                   qlib->shepherds[i].workers[j].hazard_ptrs,
                   sizeof(void *) * HAZARD_PTRS_PER_SHEP);
          } else {
            memset(plist + (i * qlib->nworkerspershep * HAZARD_PTRS_PER_SHEP) +
                     (j * HAZARD_PTRS_PER_SHEP),
                   0,
                   sizeof(void *) * HAZARD_PTRS_PER_SHEP);
          }
    ...
    for (size_t i = 0; i < freelist_max; ++i) {
      uintptr_t const ptr = (uintptr_t)hfl->freelist[i].ptr;
      if (ptr == 0) { break; }
  ...
  memcpy(hfl->freelist,
         tmpfreelist.freelist,
         tmpfreelist.count * sizeof(hazard_freelist_entry_t));
