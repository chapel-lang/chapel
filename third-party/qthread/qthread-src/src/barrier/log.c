...
struct qt_barrier_s {
  ...
  size_t allocatedSize; // lowest size power of 2 equal or bigger than barrier
                        // -- for allocations
  int doneLevel;        // height of the tree
  char barrierDebug;    // flag to turn on internal printf debugging
  syncvar_t *upLock;
  //    int64_t *upLock;	// array of counters to track number of people
  //    that have arrived

  syncvar_t *downLock;
  //    int64_t *downLock;	// array of counters that allows threads to
  //    leave
} /* qt_barrier_t */;
...
qt_barrier_t API_FUNC *qt_barrier_create(size_t size, qt_barrier_btype type) {
  ...
  if (b) {
    ...
    switch (type) {
      case REGION_BARRIER: qtb_internal_initialize_fixed(b, size, 0); break;
      default: qtb_internal_initialize_variable(b, size, 0); break;
    }
  }
  return b;
}
...
//   higher number waits for the down walk to release.  When all of the nodes
//   arrive Thread 0 starts the walk down (after executing functions to resync
...
qtb_internal_up(qt_barrier_t *b, int myLock, int64_t val, int level) {
  ...
  if (pairedLock >=
      b->activeSize) { // my pair is out of range don't wait for it
    ...
    if (level != 0) {
      qtb_internal_down(
        b, myLock, level); // everyone is here and I have people to release
    }
    return; // done
  }
  ...
  if (pairedLock < myLock) { // I'm higher -- wait for release
    qthread_syncvar_writeEF(&b->upLock[myLock], &dummy);
    if (debug) {
      printf("about to wait on lock %d paired with %d level %d val %ld\n",
             myLock,
             pairedLock,
             level,
             (long int)val);
    ...
    if (debug) {
      printf(
        "released lock %d level %d val %ld\n", myLock, level, (long int)val);
    }
    if (level != 0) {
      qtb_internal_down(
        b, myLock, level); // everyone is here and I have people to release
  ...
  if ((level + 1 < b->doneLevel) || (myLock != 0)) { // not done?  yes
    ...
    // myLock == 0 here -- don't need to fill either because Zero never waits on
    // them qthread_syncvar_writeEF(&b->upLock[myLock],&dummy);
