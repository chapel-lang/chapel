...
void API_FUNC qt_barrier_enter(qt_barrier_t *b) {
  qt_barrier_enter_id(b,
                      qthread_incr(&(b->participant), 1) % b->numParticipants);
}
...
void API_FUNC qt_barrier_enter_id(qt_barrier_t *b, size_t id) {
  ...
  if (leftchild < b->numParticipants) {
    ...
    // (unsigned)leftchild, qthread_feb_status(&(b->up[leftchild])));
    WAIT(&(b->up[leftchild]));
  ...
  if (leftchild < b->numParticipants) {
    ...
    if (rightchild < b->numParticipants) { SIGNAL(&(b->down[rightchild])); }
  }
}
...
/* debugging... */
void API_FUNC qt_barrier_dump(qt_barrier_t *b, qt_barrier_dtype dt) {}
