...
static void qt_wavefront_regionworker(qarray *restrict left,
                                      ...
                                      char *restrict *R,
                                      wave_comp_f func) {
  size_t const U_S = left->unit_size;
  size_t const Vert_Count = left->count;
  size_t const Col_Count = below->count - 1;
...
static void qt_wavefront_worker(struct qt_wavefront_wargs *const arg) {
  ...
  { /* step 0: allocate local memory */
    size_t const horizCount = L->slats.seg_len - 1;
    size_t const vertCount = L->struts.seg_len;

    local = qarray_create_configured(
      vertCount * horizCount, L->unit_size, ALL_LOCAL, 1, 1);
    assert(local);
  ...
  while (1) {
    ...
    if (wu == NULL) {
      ...
      if (qthread_cas_ptr((void **)&(L->slats.strips[wu->row + 1][wu->col]),
        ...
        assert(right == NULL);
        right =
          qarray_create_configured(left->count, L->unit_size, ALL_LOCAL, 1, 1);
        assert(right);
        assert(above == NULL); // when I found it, that is
        above =
          qarray_create_configured(below->count, L->unit_size, ALL_LOCAL, 1, 1);
        assert(above);
        R[below->count - 2] = qarray_elem_nomigrate(
          right,
          0); // so the computation dumps right into the qarray on the right
        ...
        /* -- don't have to copy {right}, because data was placed in there
         * directly */
...
qt_wavefront_lattice *qt_wavefront(qarray *restrict const vertical,
  ...
  assert(func);
  {
    ...
    if (horizontal->count > horizontal->segment_size) {
      L->struts.num =
        2 + QT_CEIL_RATIO((horizontal->count - horizontal->segment_size),
    ...
    for (size_t i = 0; i < L->struts.segs; i++) {
      size_t const array_len =
        (vertical->count - (i * L->struts.seg_len) > L->struts.seg_len)
          ? L->struts.seg_len
          : (vertical->count - (i * L->struts.seg_len));
      qarray *tmp =
        qarray_create_configured(array_len, L->unit_size, ALL_RAND, 1, 1);
    ...
    for (size_t i = 0; i < L->slats.segs; i++) {
      size_t const array_len =
        (horizontal->count - (i * L->slats.seg_len) > L->slats.seg_len)
          ? L->slats.seg_len
    ...
    while (donecount < maxsheps) { qthread_yield(); }
...
void qt_wavefront_destroy_lattice(qt_wavefront_lattice *const L) {
  size_t const slatCount = L->slats.num;
  size_t const slatSegCount = L->slats.segs;
  size_t const strutCount = L->struts.num;
  size_t const strutSegCount = L->struts.segs;
...
void qt_wavefront_print_lattice(qt_wavefront_lattice const *const L) {
  for (ssize_t slat = L->slats.num - 1; slat >= 0; slat--) {
    ...
    for (size_t stripSeg = 0; stripSeg < L->slats.segs; stripSeg++) {
      qarray const *const thisSeg = L->slats.strips[slat][stripSeg];
      if (thisSeg) {
    ...
    // strut 0
    if (slat == 0) { continue; }
    size_t const strutSeg = slat - 1;
    qarray *const *const strut0 = (qarray **)(L->struts.strips[0]);
    assert(strut0);
    qarray const *thisSeg = strut0[strutSeg];
    assert(thisSeg);
    for (ssize_t segRow = thisSeg->count - 1; segRow >= 0; segRow--) {
      for (size_t strut = 0; strut < L->struts.num; strut++) {
        ...
        assert(thisStrut);
        qarray const *curSeg = thisStrut[strutSeg];
        if (strut > 0) { printf("|"); }
        if (curSeg) {
          printf("%9.3f ", *(double *)qarray_elem_nomigrate(curSeg, segRow));
        } else {
          printf("(NULL ) ");
        }
        if (strut < L->slats.num - 1) {
          size_t const spacecount = L->slats.strips[0][strut]->count - 1;
