...
typedef enum { ALIGNED, SYNCVAR_T, SINC_T, DONECOUNT, NO_SYNC } synctype_t;
...
static aligned_t qloop_wrapper(void *restrict arg_void) {
  ...
  struct qloop_wrapper_args *arg = (struct qloop_wrapper_args *)arg_void;
  size_t tot_workers = arg->spawnthreads - 1; // -1 because I already exist
  size_t level = arg->level;
  size_t my_id = arg->id;
  size_t new_id = my_id + (1 << level);
  synctype_t const sync_type = arg->sync_type;
  void *const sync = arg->sync;

  switch (sync_type) {
    case SYNCVAR_T:
      while (new_id <=
             tot_workers) { // create some children? (tot_workers zero based)
  ...
  switch (sync_type) {
    ...
    case DONECOUNT:
      atomic_fetch_add_explicit(
        (_Atomic uint64_t *)sync, 1, memory_order_release);
      break;
  }
...
static aligned_t qt_loop_wrapper(void *restrict arg_void) {
  struct qt_loop_wrapper_args const *arg =
    (struct qt_loop_wrapper_args const *)arg_void;
  arg->func(arg->startat, arg->stopat, arg->arg);
  switch (arg->sync_type) {
    default: break;
    case SINC_T: qt_sinc_submit(arg->sync, NULL); break;
    case DONECOUNT: qthread_incr((aligned_t *)arg->sync, 1); break;
  }
  return 0;
}
...
static void
qt_loop_spawner(size_t const start, size_t const stop, void *args_) {
  size_t i, threadct;
  size_t steps = stop - start;
  struct qt_loop_wrapper_args qwa;
  unsigned int flags = 0;
  synctype_t const sync_type = ((struct qt_loop_spawner_arg *)args_)->sync_type;
  qt_loop_f const func = ((struct qt_loop_spawner_arg *)args_)->func;
  ...
  for (i = start, threadct = 0; i < stop; ++i, ++threadct) {
    ...
    qassert(qthread_spawn((qthread_f)qt_loop_wrapper,
                          &qwa,
                          sizeof(struct qt_loop_wrapper_args),
                          retptr,
                          0,
                          NULL,
                          NO_SHEPHERD,
  ...
  switch (sync_type) {
    ...
    case SINC_T:
      ...
      break;
    case DONECOUNT:
      while (atomic_load_explicit((_Atomic uint64_t *)&dc,
                                  memory_order_relaxed) != threadct) {
        qthread_yield();
      }
      break;
    case NO_SYNC: abort();
...
static void qt_loop_inner(size_t const start,
                          size_t const stop,
                          qt_loop_f const func,
                          void *argptr,
                          uint8_t flags,
                          synctype_t const sync_type) {
...
API_FUNC void qt_loop(size_t start, size_t stop, qt_loop_f func, void *argptr) {
  qt_loop_inner(start, stop, func, argptr, 0, DONECOUNT);
}

API_FUNC void
qt_loop_simple(size_t start, size_t stop, qt_loop_f func, void *argptr) {
...
API_FUNC void
qt_loop_aligned(size_t start, size_t stop, qt_loop_f func, void *argptr) {
...
static inline void qt_loop_balance_inner(size_t const start,
  ...
  qthread_shepherd_id_t const maxworkers =
    ((stop - start) > qthread_num_workers()) ? qthread_num_workers()
                                             : (stop - start);
  struct qloop_wrapper_args *const qwa = (struct qloop_wrapper_args *)MALLOC(
    sizeof(struct qloop_wrapper_args) * maxworkers);
  size_t const each = (stop - start) / maxworkers;
  ...
  switch (sync_type) {
    ...
    case SINC_T:
      ...
      break;
    case DONECOUNT: break;
    case NO_SYNC: abort();
  }
  switch (flags) {
    case QT_LOOP_BALANCE_SIMPLE: internal_flags |= QTHREAD_SPAWN_SIMPLE; break;
  }

  /* The reason we use this big array, rather than using argcopy, is to make
   * tree-spawning easier. */
  for (i = 0; i < maxworkers; i++) {
    ...
    switch (sync_type) {
      case SYNCVAR_T:
        ...
        break;
      case ALIGNED: qthread_empty(&sync.aligned[i]);
      case DONECOUNT: qwa[i].sync = &sync.dc; break;
      case SINC_T: qwa[i].sync = sync.sinc; break;
      case NO_SYNC: abort();
    }
    if (extra > 0) {
      qwa[i].stopat++;
  ...
  switch (sync_type) {
    ...
    default:
      qassert(
        qthread_spawn((qthread_f)qloop_wrapper, qwa, 0, NULL, 0, NULL, 0, 0),
        QTHREAD_SUCCESS);
      break;
  }
  switch (sync_type) {
    ...
    case SINC_T:
      ...
      break;
    case DONECOUNT:
      while (atomic_load_explicit((_Atomic uint64_t *)&sync.dc,
                                  memory_order_acquire) != maxworkers) {
        qthread_yield();
      }
      break;
    case NO_SYNC: abort();
...
static aligned_t qloopaccum_wrapper(void *restrict arg_void) {
  struct qloopaccum_wrapper_args *arg =
    (struct qloopaccum_wrapper_args *)arg_void;
  /* tree-based spawning (credit: AKP) */
  size_t tot_workers = arg->spawnthreads - 1; // -1 because I already exist
  size_t level = arg->level;
  size_t my_id = arg->id;
  ...
  switch (sync_type) {
    ...
    default:
      while (new_id <=
        ...
        size_t offset = new_id - my_id;  // need how much past current locations
        (arg + offset)->level = ++level; // increase depth for created thread
        qthread_fork_syncvar_to(
          (qthread_f)qloopaccum_wrapper, arg + offset, NULL, new_id);
        new_id = (1 << level) + my_id; // level has been incremented
      }
  }
  ...
  switch (sync_type) {
    default: break;
    case SINC_T: qt_sinc_submit(arg->sync, arg->ret); break;
    case DONECOUNT: qthread_incr((aligned_t *)arg->sync, 1); break;
  }
  return 0;
}
...
static inline void qt_loopaccum_balance_inner(size_t const start,
  ...
  struct qloopaccum_wrapper_args *const qwa =
    (struct qloopaccum_wrapper_args *)MALLOC(
      sizeof(struct qloopaccum_wrapper_args) * maxworkers);
  ...
  switch (flags) {
    case QT_LOOP_BALANCE_SIMPLE: spawn_flags |= QTHREAD_SPAWN_SIMPLE; break;
  }
  ...
  switch (sync_type) {
    ...
    default:
      qassert(
        qthread_spawn(
          (qthread_f)qloopaccum_wrapper, qwa, 0, NULL, 0, NULL, 0, spawn_flags),
        QTHREAD_SUCCESS);
      break;
  }
  switch (sync_type) {
    case SYNCVAR_T:
      for (qthread_shepherd_id_t i = 0; i < maxworkers; i++) {
        qthread_syncvar_readFF(NULL, sync.syncvar + i);
        if (i > 0) { acc(out, realrets + ((i - 1) * size)); }
      }
      FREE(sync.syncvar, maxworkers * sizeof(syncvar_t));
      break;
    case SINC_T:
      ...
      break;
    case DONECOUNT:
      while (sync.dc != maxworkers) { qthread_yield(); }
      for (qthread_shepherd_id_t i = 0; i < maxworkers; i++) {
        if (i > 0) { acc(out, realrets + ((i - 1) * size)); }
      }
      qt_internal_aligned_free(sync.aligned, QTHREAD_ALIGNMENT_ALIGNED_T);
  ...
  if (realrets) { FREE(realrets, size * (maxworkers - 1)); }
...
API_FUNC void qt_loopaccum_balance_dc(size_t const start,
                                      ...
                                      qt_accum_f const acc) {
  qt_loopaccum_balance_inner(
    start, stop, size, out, func, argptr, acc, 0, DONECOUNT);
}
...
static int qqloop_get_iterations_guided(
  ...
  saligned_t const stop = iq->stop;
  qthread_shepherd_id_t const sheps = sa->activesheps;
...
static inline int qqloop_get_iterations_factored(
  ...
  saligned_t iterations = 0;
  saligned_t phase =
    atomic_load_explicit(&iq->type_specific_data.phase, memory_order_relaxed);
  qthread_shepherd_id_t const sheps = sa->activesheps;
  ...
  while (ret < iq->stop && ret != ret2) {
    ...
    while (ret >= phase && ret < iq->stop) {
      ...
      if (newphase != phase) {
        phase = qthread_cas(
          (int64_t *)&(iq->type_specific_data.phase), phase, newphase);
      } else {
        phase =
          qthread_cas((int64_t *)&(iq->type_specific_data.phase), phase, stop);
      }
    }
...
static inline int qqloop_get_iterations_chunked(
  ...
  int retval = 1;
  saligned_t ret =
    atomic_fetch_add_explicit(&iq->start, chunk_size, memory_order_relaxed) -
    chunk_size;

  if (ret < iq->stop) {
    if (ret + chunk_size > iq->stop) { chunk_size = iq->stop - ret; }
    range->startat = ret;
...
static inline int
qqloop_get_iterations_timed(qqloop_iteration_queue_t *restrict const iq,
                            struct qqloop_static_args *restrict const sa,
                            struct qqloop_wrapper_range *restrict const range) {
  qthread_shepherd_id_t const workerCount = sa->activesheps;
  qthread_shepherd_id_t const shep = qthread_shep();
  saligned_t const localstop = iq->stop;
  saligned_t const localstep = iq->step;
  ...
  while (localstart < localstop) {
    ...
    if (loop_time >= 7.5e-7) { /* KBW: XXX: arbitrary constant */
      dynamicBlock =
        ((localstop - localstart) / ((workerCount << 1) * (localstep))) + 1;
    }
    if ((localstart + dynamicBlock) > localstop) {
      dynamicBlock = (localstop - localstart);
    }
    tmp =
      qthread_cas((int64_t *)&iq->start, localstart, localstart + dynamicBlock);
    if (tmp == localstart) {
...
qqloop_create_iq(size_t const startat,
  ...
  switch (type) {
    case FACTORED:
      atomic_store_explicit(&iq->type_specific_data.phase,
                            (startat + stopat) / 2,
                            memory_order_relaxed);
      break;
    case TIMED: {
      qthread_shepherd_id_t const max = qthread_num_workers();
      assert(max != 0);
      qthread_shepherd_id_t i;
      qtimer_t *timers = MALLOC(sizeof(qtimer_t) * max);
      assert(timers);
      for (i = 0; i < max; i++) { timers[i] = qtimer_create(); }
      iq->type_specific_data.timed.timers = timers;
      iq->type_specific_data.timed.lastblocks =
        MALLOC(sizeof(saligned_t) * max);
...
static aligned_t qqloop_wrapper(void *arg_void) {
  struct qqloop_wrapper_args const *arg =
    (struct qqloop_wrapper_args const *)arg_void;
  struct qqloop_static_args *restrict const stat = arg->stat;
  qqloop_iteration_queue_t *restrict const iq = stat->iq;
  qt_loop_f const func = stat->func;
  void *restrict const a = stat->arg;
  aligned_t *const dc = &(stat->donecount);
  qq_getiter_f const get_iters = stat->get;
  qthread_shepherd_id_t const shep = arg->shep;
  ...
  if (safeexit) { qthread_incr(dc, 1); }
  return 0;
}

API_FUNC qqloop_handle_t *qt_loop_queue_create(qt_loop_queue_type const type,
                                               size_t const start,
                                               size_t const stop,
                                               size_t const incr,
                                               qt_loop_f const func,
  ...
  qassert_ret(func, NULL);
  {
    ...
    if (h) {
      qthread_shepherd_id_t const maxsheps = qthread_num_workers();
      qthread_shepherd_id_t i;
      ...
      h->stat.arg = argptr;
      h->stat.chunksize =
        (stop - start) / qthread_num_workers() / 10; // completely arbitrary
      if (h->stat.chunksize == 0) { h->stat.chunksize = 1; }
      switch (type) {
        case FACTORED: h->stat.get = qqloop_get_iterations_factored; break;
        case TIMED: h->stat.get = qqloop_get_iterations_timed; break;
        case GUIDED: h->stat.get = qqloop_get_iterations_guided; break;
        case CHUNK: h->stat.get = qqloop_get_iterations_chunked; break;
      }
      for (i = 0; i < maxsheps; i++) {
        h->qwa[i].stat = &(h->stat);
        h->qwa[i].shep =
          i; // this is the only thread-specific piece of information...
      }
    }
    return h;
  }
...
API_FUNC void qt_loop_queue_run(qqloop_handle_t *loop) {
  ...
  {
    qthread_shepherd_id_t i;
    qthread_shepherd_id_t const maxwkrs = qthread_num_workers();
     ...
     * during the loop */
    while (atomic_load_explicit((_Atomic uint64_t *)dc, memory_order_acquire) <
           *as) {
      qthread_yield();
    }
...
API_FUNC void qt_loop_queue_run_there(qqloop_handle_t *loop,
  ...
  qassert_retvoid(shep < qthread_num_shepherds());
  {
    ...
    qqloop_destroy_iq(loop->stat.iq);
    FREE(loop->qwa,
         sizeof(struct qqloop_wrapper_args) * qthread_num_shepherds());
    FREE(loop, sizeof(qqloop_handle_t));
  }
}
...
API_FUNC void qt_loop_queue_addworker(qqloop_handle_t *loop,
...
#define PARALLEL_FUNC(category, initials, _op_, type, shorttype)               \
  static void qt##initials##_febworker(const size_t startat,                   \
                                       const size_t stopat,                    \
                                       void *restrict arg,                     \
                                       void *restrict ret) {                   \
    size_t i;                                                                  \
  ...
  static void qt##initials##_worker(const size_t startat,                      \
                                    const size_t stopat,                       \
                                    void *restrict arg,                        \
                                    void *restrict ret) {                      \
    size_t i;                                                                  \
...
static int dcmp(void const *restrict a, void const *restrict b) {
...
static aligned_t qt_qsort_partition(struct qt_qsort_args *args) {
  ...
  size_t const chunksize = args->chunksize;
  size_t const length = args->length;
  size_t const jump = args->jump;
  ...
  while (a[leftwall] <= pivot) {
    ...
    if (rightwall < leftwall) { goto quickexit; }
  }
  while (a[rightwall] > pivot) {
    if (rightwall % chunksize != 0) {
      if (rightwall == 0) { goto quickexit; }
      rightwall--;
    } else {
      if (rightwall < jump) { goto quickexit; }
      rightwall -= jump;
    }
    if (rightwall < leftwall) { goto quickexit; }
  }
...
static qt_qsort_iprets_t qt_qsort_inner_partitioner(double *array,
                                                    size_t const length,
                                                    double const pivot) {
  size_t const chunksize = 10;

  /* choose the number of threads to use */
  size_t const numthreads = qthread_num_shepherds();
  ...
  size_t const megachunk_size = chunksize * numthreads;

  /* just used as a boolean test */
  size_t const extra_chunks = length % megachunk_size;
  ...
  args =
    (struct qt_qsort_args *)MALLOC(sizeof(struct qt_qsort_args) * numthreads);
  ...
  for (i = 0; i < numthreads; i++) { qthread_syncvar_readFF(NULL, rets + i); }
...
static aligned_t qt_qsort_inner(const struct qt_qsort_iargs *a) {
  size_t const len = a->length;
  double *array = a->array;
  size_t i;
  qt_qsort_iprets_t furthest;
  size_t const thread_chunk = len / qthread_num_shepherds();
  ...
  i = len / 2;
  if (array[0] > array[i]) { SWAP(array, 0, i); }
  if (array[0] > array[len - 1]) { SWAP(array, 0, len - 1); }
  if (array[i] > array[len - 1]) { SWAP(array, i, len - 1); }
  {
    double const pivot = array[i];

    while (furthest.rightwall > furthest.leftwall &&
           furthest.rightwall - furthest.leftwall > (2 * thread_chunk)) {
      size_t const offset = furthest.leftwall;

      furthest =
        qt_qsort_inner_partitioner(array + furthest.leftwall,
                                   furthest.rightwall - furthest.leftwall + 1,
                                   pivot);
      furthest.leftwall += offset;
      furthest.rightwall += offset;
    }
    /* data between furthest.leftwall and furthest.rightwall is unlikely to be
     * partitioned correctly */
    {
      ...
      if (array[rightwall] <= pivot) { rightwall++; }
