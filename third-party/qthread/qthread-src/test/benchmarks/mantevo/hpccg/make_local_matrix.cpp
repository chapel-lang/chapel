...
void make_local_matrix(HPC_Sparse_Matrix *A) {
  ...
  MPI_Allreduce(
    tmp_buffer, global_index_offsets, size, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
  ...
  MPI_Allreduce(
    tmp_neighbors, tmp_buffer, size, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
  ...
  for (i = 0; i < num_recv_neighbors; i++)
    MPI_Send(
      tmp_buffer + i, 1, MPI_INT, recv_list[i], MPI_MY_TAG, MPI_COMM_WORLD);
  ///
