...
class cr_loop {
public:
  cr_loop(double const *const V1, double const *const V2):
    ret(0.0), v1(V1), v2(V2) {}
...
private:
  ...
  double const *const v2;
};

int compute_residual(int const n,
                     double const *const v1,
                     double const *const v2,
  ...
  MPI_Allreduce(
    &local_residual, &global_residual, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
