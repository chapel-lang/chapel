...
int main(int argc, char *argv[]) {
  ...
  if ((argc != 2) && (argc != 4)) {
    if (rank == 0) {
      fprintf(
        ...
        "     where HPC_data_file is a globally accessible file containing "
        "matrix data.\n",
        argv[0],
        argv[0]);
    }
  ...
  if (rank == 0) { // Only PE 0 needs to compute and report timing results
    ...
    printf("     Minimum DDOT MPI_Allreduce time (over all processors) = %g\n",
           t4min);
    printf("     Maximum DDOT MPI_Allreduce time (over all processors) = ",
           t4max);
    printf("     Average DDOT MPI_Allreduce time (over all processors) = ",
           t4avg);
#endif
    ...
    printf("SPARSEMV PARALLEL OVERHEAD Time       = %g ( %g%% ).\n\n",
           (times[4] + times[5]),
           (times[4] + times[5]) / totalSparseMVTime * 100.0);
    printf(
      "     SPARSEMV PARALLEL OVERHEAD (Setup) Time         = %g ( %g%% ).\n\n",
      times[4],
      times[4] / totalSparseMVTime * 100.0);
    printf(
      "     SPARSEMV PARALLEL OVERHEAD (Bdry Exchange) Time = %g ( %g%% ).\n\n",
      times[5],
