...
static syncvar_t const boundary_value = SYNCVAR_STATIC_INITIALIZE_TO(BOUNDARY);
#ifdef DEBUG
static syncvar_t const internal_value = SYNCVAR_STATIC_INITIALIZE_TO(10);
...
#define GHOST_NORTH(pos)                                                       \
  (WEST == pos || CENTER == pos || EAST == pos || SW == pos || SOUTH == pos || \
   SE == pos || COL_CENTER == pos || COL_SOUTH == pos)
#define GHOST_WEST(pos)                                                        \
  (NORTH == pos || NE == pos || CENTER == pos || EAST == pos ||                \
   SOUTH == pos || SE == pos || ROW_CENTER == pos || ROW_EAST == pos)
#define GHOST_EAST(pos)                                                        \
  (NW == pos || NORTH == pos || WEST == pos || CENTER == pos || SW == pos ||   \
   SOUTH == pos || ROW_WEST == pos || ROW_CENTER == pos)
#define GHOST_SOUTH(pos)                                                       \
  (NW == pos || NORTH == pos || NE == pos || WEST == pos || CENTER == pos ||   \
   EAST == pos || COL_NORTH == pos || COL_CENTER == pos)

#define HAS_NW_CORNER(pos)                                                     \
  (CENTER == pos || EAST == pos || SOUTH == pos || SE == pos)
#define HAS_SW_CORNER(pos)                                                     \
  (CENTER == pos || NORTH == pos || NE == pos || EAST == pos)
#define HAS_NE_CORNER(pos)                                                     \
...
static inline void print_stencil(stencil_t *stencil, size_t step) { /*{{{*/
  ...
  size_t const num_parts = stencil->prows * stencil->pcols;
  for (int pi = 0; pi < num_parts; pi++) {
    partition_t const *part = stencil->parts[pi];
    fprintf(stderr,
            "\tPartition: (%lu,%lu) %s, %lu x %lu, %lu x %lu\n",
            part->row,
            part->col,
            pos_strs[part->pos],
            part->nrows,
...
static inline void print_status(stencil_t *stencil, size_t step) { /*{{{*/
  ...
  for (int pi = 0; pi < num_parts; pi++) {
    ...
    for (int i = part->nrows - 1; i >= 0; i--) {
      fprintf(
        stderr, "\t\t%d", qthread_syncvar_status(&part->stages[step][i][0]));
      for (int j = 1; j < part->ncols; j++) {
        fprintf(
          stderr, " %d", qthread_syncvar_status(&part->stages[step][i][j]));
      }
      fprintf(stderr, "\n");
    }
  }
...
static inline void get_position(stencil_t *points, partition_t *part) { /*{{{*/
  ...
  size_t const pcols = points->pcols;

  if (prows > 1 && pcols > 1) {
    if (i == 0)
      if (j == 0) part->pos = SW;
      else if (j == pcols - 1) part->pos = SE;
      else part->pos = SOUTH;
    else if (i == prows - 1)
      if (j == 0) part->pos = NW;
      else if (j == pcols - 1) part->pos = NE;
      else part->pos = NORTH;
    else if (j == 0) part->pos = WEST;
    else if (j == pcols - 1) part->pos = EAST;
    else part->pos = CENTER;
  } else if (prows == 1 && pcols > 1) {
    if (j == 0) part->pos = ROW_WEST;
    else if (j == pcols - 1) part->pos = ROW_EAST;
    else part->pos = ROW_CENTER;
  } else if (pcols == 1 && prows > 1) {
    if (i == 0) part->pos = COL_SOUTH;
    else if (i == prows - 1) part->pos = COL_NORTH;
    else part->pos = COL_CENTER;
  } else {
    part->pos = NWES;
  }
} /*}}}*/
...
static aligned_t update_point_corner(void *arg_) {
  upx_args_t const *arg = (upx_args_t *)arg_;
  syncvar_t ***stages = arg->stages;
  position_t const dir = arg->dir;
  size_t timestep = arg->timestep;
  size_t const num_rows = arg->num_rows;
  size_t const num_cols = arg->num_cols;
    ...
    switch (dir) {
      case NW:
        ...
        qthread_fork_copyargs(
          clear_point_corner, &cpc_args, sizeof(cpc_args_t), NULL);
        break;
      case NE:
        i = num_rows - 2;
        j = num_cols - 2;
 ...
 */
static void
update_point_edge(size_t const start, size_t const stop, void *arg_) {
  upx_args_t const *arg = (upx_args_t *)arg_;
  syncvar_t ***stages = arg->stages;
  position_t const dir = arg->dir;
  size_t const num_rows = arg->num_rows;
...
static aligned_t update_point_edge_loop(void *arg_) {
  ...
  while (timestep <= num_timesteps) {
    ...
    switch (dir) {
      case NORTH: {
        size_t const lb = GHOST_WEST(pos) ? 2 : 1;
        size_t const ub = GHOST_EAST(pos) ? num_cols - 2 : num_cols - 1;
      ...
      case WEST: {
        size_t const lb = GHOST_SOUTH(pos) ? 2 : 1;
        size_t const ub = GHOST_NORTH(pos) ? num_rows - 2 : num_rows - 1;
...
/*static aligned_t update_stage(void *arg_)
{
    ...
    upx_args_t upx_args = {stages, now, arg->timestep, 0, pos, num_rows,
num_cols, arg->part_lid};
...
static aligned_t send_block(void *arg_) {
  ...
  size_t timestep = arg->timestep;
  size_t const lb = arg->lb;
  size_t const ub = arg->ub;
  size_t const sk = arg->sk;
  size_t const tk = arg->tk;
  // const size_t part_lid = arg->part_lid;
...
static aligned_t send_updates(void *arg_) {
  su_args_t const *arg = (su_args_t *)arg_;
  size_t const src_lid = arg->part_lid;
  stencil_t const *points = arg->points;

  partition_t *src_part = points->parts[src_lid];
  position_t const src_pos = src_part->pos;
  syncvar_t ***src_stage = src_part->stages;
  size_t const src_nrows = src_part->nrows;
  size_t const src_ncols = src_part->ncols;

  size_t const bsize = points->bsize;

  size_t const stage = 0;
  size_t const timestep = 1;

  // Send updates along edges
  if (GHOST_NORTH(src_pos)) {
    size_t const tgt_lid =
    ...
    for (size_t j = lb; j < ub; j += bsize) {
      ...
      qthread_fork_copyargs(send_block, &sb_args, sizeof(sb_args_t), NULL);
    }
  }
  if (GHOST_SOUTH(src_pos)) {
    ...
    partition_t *tgt_part = points->parts[tgt_lid];
    syncvar_t ***tgt_stage = tgt_part->stages;
    size_t const tgt_nrows = tgt_part->nrows;
  ...
  if (GHOST_WEST(src_pos)) {
    ...
    partition_t *tgt_part = points->parts[tgt_lid];
    syncvar_t ***tgt_stage = tgt_part->stages;
    size_t const tgt_ncols = tgt_part->ncols;

    size_t const lb = 1;
    size_t const ub = src_nrows - 1;
...
static void begin(size_t const start, size_t const stop, void *arg_) { /*{{{*/
  ...
  su_args_t const su_args = {part_lid, arg};
  ...
  if (HAS_NW_CORNER(pos)) {
    ...
    qthread_fork_syncvar_copyargs(
      update_point_corner, &upx_args, sizeof(upx_args), NULL);
  }
...
setup_stencil(size_t const start, size_t const stop, void *arg_) { /*{{{*/
  ...
  double const row_per = row_splits[part->row] * 0.01;
  double const col_per = col_splits[part->col] * 0.01;
  ...
  // Setup boundary
  {
    position_t const pos = part->pos;
    size_t const nrows = part->nrows;
    size_t const ncols = part->ncols;
  ...
  {
    position_t const pos = part->pos;
    size_t const num_rows = part->nrows;
    size_t const num_cols = part->ncols;
...
int main(int argc, char *argv[]) { /*{{{*/
  ...
  size_t const num_parts = prows * pcols;
  ...
  int num_sends = (prows - 1) * (ncols / bsize) * 2 +
                  (pcols - 1) * (nrows / bsize) * 2 + prows * pcols;
  int num_corners = (prows - 1) * (pcols - 1) * 4;
  int num_edges = num_corners * 2;
  iprintf("DEBUG: creating barrier with %d participants\n",
          num_sends + num_corners + num_edges);
  debug_barrier =
    qt_barrier_create(num_sends + num_corners + num_edges, REGION_BARRIER);
#endif
