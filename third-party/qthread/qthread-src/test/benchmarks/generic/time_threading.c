...
static aligned_t null_thread(void *arg) { return 0; }
...
static aligned_t team_thread(void *arg) {
  ...
  for (unsigned long i = 0; i < THREADS; ++i) { qthread_empty(rets + i); }
  ...
  {
    syncvar_t const empty = {.u.s = {.data = 0, .state = 2, .lock = 0}};
    for (unsigned long i = 0; i < THREADS; ++i) { srets[i] = empty; }
  }
  qtimer_start(timer);
  for (unsigned long i = 0; i < THREADS; ++i) {
