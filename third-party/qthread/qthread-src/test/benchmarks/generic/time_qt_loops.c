...
static void sumrand(size_t const startat, size_t const stopat, void *arg_) {
  size_t tmp, tmp2;
...
typedef void (*loop_f)(size_t const a,
                       size_t const b,
                       qt_loop_f const f,
                       void *c);
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < numincrs; ++i) { randlen[i] = qtimer_fastrand() % 10; }
