...
static void
balanced_readFF(size_t const startat, size_t const stopat, void *arg) {
  size_t i;

  for (i = startat; i < stopat; i++) { qthread_readFF(NULL, (aligned_t *)arg); }
}

static void
balanced_syncvar_readFF(size_t const startat, size_t const stopat, void *arg) {
  size_t i;
...
static void balanced_falseshare_syncreadFF(size_t const startat,
                                           size_t const stopat,
                                           void *arg) {
  size_t i;
  ...
  for (i = startat; i < stopat; i++) { qthread_syncvar_readFF(NULL, myloc); }
}

static void balanced_falseshare_readFF(size_t const startat,
                                       size_t const stopat,
                                       void *arg) {
  size_t i;
  qthread_shepherd_id_t shep = qthread_shep();
  ...
  for (i = startat; i < stopat; i++) { qthread_readFF(NULL, myloc); }
}

static void balanced_noncomp_syncreadFF(size_t const startat,
                                        size_t const stopat,
                                        void *arg) {
  size_t i;
  syncvar_t myinc = SYNCVAR_STATIC_INITIALIZER;

  for (i = startat; i < stopat; i++) { qthread_syncvar_readFF(NULL, &myinc); }
}

static void
balanced_noncomp_readFF(size_t const startat, size_t const stopat, void *arg) {
  size_t i;
  aligned_t myinc = 0;

  for (i = startat; i < stopat; i++) { qthread_readFF(NULL, &myinc); }
}

static aligned_t justreturn(void *arg) { return 7; }
...
int main(int argc, char *argv[]) {
  ...
  if (TEST_SELECTION & 1) {
    ...
    qtimer_start(timer);
    qt_loop_balance(
      0, MAXPARALLELISM * ITERATIONS, balanced_syncvar_readFF, shared);
    qtimer_stop(timer);
    free(shared);
  ...
  if (TEST_SELECTION & (1 << 2)) {
    ...
    qtimer_start(timer);
    qt_loop_balance(
      0, ITERATIONS * MAXPARALLELISM, balanced_falseshare_syncreadFF, shared);
    qtimer_stop(timer);
    free(shared);
  ...
  if (TEST_SELECTION & (1 << 3)) {
    ...
    qtimer_start(timer);
    qt_loop_balance(
      0, ITERATIONS * MAXPARALLELISM, balanced_falseshare_readFF, shared);
    qtimer_stop(timer);
    free(shared);
  ...
  if (TEST_SELECTION & (1 << 4)) {
    ...
    qtimer_start(timer);
    qt_loop_balance(
      0, ITERATIONS * MAXPARALLELISM, balanced_noncomp_syncreadFF, NULL);
    qtimer_stop(timer);
  ...
  if (TEST_SELECTION & (1 << 5)) {
    ...
    /* prime it */
    qt_loop_balance(
      0, ITERATIONS * MAXPARALLELISM, balanced_noncomp_readFF, NULL);
    /* time it */
    qtimer_start(timer);
    qt_loop_balance(
      0, ITERATIONS * MAXPARALLELISM, balanced_noncomp_readFF, NULL);
  ...
  if (TEST_SELECTION & (1 << 6)) {
    ...
    for (i = 0; i < ITERATIONS; i++) { qthread_syncvar_readFF(NULL, rets + i); }
    qtimer_stop(timer);
    free(rets);
  ...
  if (TEST_SELECTION & (1 << 7)) {
    ...
    for (i = 0; i < ITERATIONS; i++) { qthread_readFF(NULL, rets + i); }
    qtimer_stop(timer);
    free(rets);
