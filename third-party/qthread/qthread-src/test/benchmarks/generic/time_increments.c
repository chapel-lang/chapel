...
static void init_prism(void) { memset(prism, 0, sizeof(prism)); }
    ...
    if (try1 & 0x1) {      // try is odd
      ...
      if (prism[node][pindex] != try1 + 1) { // did anyone come by?
        ...
        if (try2 != try1 + 1) {               // anyone now?
          ...
          qthread_writeF(&prism[node][pindex],
                         &try1); // as if never in'prism'ed
          aligned_t addend = qthread_incr(&prism[node][0], 1) & 0x1;
          if (addend) { offset += 1 << depth; }
...
int main(int argc, char *argv[]) {
  ...
  if (!MACHINE_READABLE) { printf("%u shepherds...\n", shepherds); }
  ...
  if (TEST_SELECTION & 1) {
    ...
    rate =
      (ITERATIONS * MAXPARALLELISM * sizeof(aligned_t)) / qtimer_secs(timer);
    switch (MACHINE_READABLE) {
      ...
      case 1:
        printf(
          "%f,%g,", (ITERATIONS * MAXPARALLELISM) / qtimer_secs(timer), rate);
        break;
      case 2:
        printf(
          "%f\n%g\n", (ITERATIONS * MAXPARALLELISM) / qtimer_secs(timer), rate);
        break;
    }
  }
  ...
  if (TEST_SELECTION & (1 << 1)) {
    ...
    switch (MACHINE_READABLE) {
      case 0:
        ...
        break;
      case 1:
        printf("%f,%g,", (ITERATIONS * 256) / qtimer_secs(timer), rate);
        break;
      case 2:
        printf("%f\n%g\n", (ITERATIONS * 256) / qtimer_secs(timer), rate);
        break;
    }
  }
  ...
  if (TEST_SELECTION & (1 << 3)) {
    ...
    for (i = 0; i < MAXPARALLELISM; i++) { qthread_readFF(NULL, rets + i); }
    qtimer_stop(timer);
  ...
  if (TEST_SELECTION & (1 << 4)) {
    ...
    switch (MACHINE_READABLE) {
      ...
      case 1:
        printf(
          "%g,%f,", (ITERATIONS * MAXPARALLELISM) / qtimer_secs(timer), rate);
        break;
      case 2:
        printf(
          "%g\n%f\n", (ITERATIONS * MAXPARALLELISM) / qtimer_secs(timer), rate);
  ...
  if (TEST_SELECTION & (1 << 8)) {
    ...
    increments =
      (aligned_t *)calloc(MAXPARALLELISM * ITERATIONS, sizeof(aligned_t));
    qtimer_start(timer);
