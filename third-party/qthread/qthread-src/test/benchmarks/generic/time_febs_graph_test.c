...
int main(int argc, char *argv[]) {
  ...
  fprintf(stderr,
          "feb time num_tasks num_elems block_size num_sheps workers "
          "workers_per_shep\n");
  ...
  // Aligned FEB
  {
    ...
    for (i = 0; i < NUM_TASKS; i++) {
      for (size_t j = 0; j < rvs_count; j++) { rvs[j] = rand() % NUM_ELEMS; }
    ...
    for (i = 0; i < NUM_TASKS; i++) { qthread_readFF(NULL, rets + i); }
    qtimer_stop(timer);
  ...
  // Syncvar FEB
  {
    ...
    for (i = 0; i < NUM_ELEMS; i++) { syncvar_elems[i] = SYNCVAR_INITIALIZER; }
