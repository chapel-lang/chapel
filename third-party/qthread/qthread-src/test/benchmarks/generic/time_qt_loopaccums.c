...
static void
count(size_t const startat, size_t const stopat, void *arg_, void *ret_) {
  multiple_op_t ret;
  ...
  for (size_t i = startat + 1; i < stopat; ++i) {
    ...
    ret.avg = ((ret.avg * ret.cnt) / (ret.cnt + 1)) +
              (random_numbers[i] / (ret.cnt + 1));
    ret.cnt++;
    usleep(sleeplen[i]);
  }
...
static void multi_op(void *restrict a_, void const *restrict b_) {
  ...
  if (b->cnt > 0) {
    if (a->cnt > 0) {
      a->avg = ((a->avg * a->cnt) / (a->cnt + b->cnt)) +
               ((b->avg * b->cnt) / (a->cnt + b->cnt));
    } else {
      a->avg = b->avg;
    }
  }
...
typedef void (*qtloopaccum_f)(size_t const a,
                              size_t const b,
                              size_t const sz,
                              void *out,
                              qt_loopr_f const f,
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < numincrs; ++i) {
    ...
    known_correct.max = (random_numbers[i] > known_correct.max)
                          ? random_numbers[i]
                          : known_correct.max;
    known_correct.min = (random_numbers[i] > known_correct.min)
                          ? known_correct.min
                          : random_numbers[i];
  ...
  run_iterations(
    qt_loopaccum_balance_sv, count, multi_op, overhead, "balanced", "syncvar");
  run_iterations(
    qt_loopaccum_balance_sinc, count, multi_op, overhead, "balanced", "sinc");
  run_iterations(qt_loopaccum_balance_dc,
                 count,
                 multi_op,
                 overhead,
                 "balanced",
