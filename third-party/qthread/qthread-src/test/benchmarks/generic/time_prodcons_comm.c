...
static void syncvar_prodcons(size_t const startat,
  ...
  qthread_fork_syncvar_copyargs(
    syncvar_prodcons_reader, &a, sizeof(struct arg), &rets[0]);
  qthread_fork_syncvar_copyargs(
    syncvar_prodcons_writer, &a, sizeof(struct arg), &rets[1]);
  qthread_yield();
...
static void aligned_prodcons(size_t const startat,
  ...
  qthread_fork_syncvar_copyargs(
    aligned_prodcons_reader, &a, sizeof(struct arg), &rets[0]);
  qthread_fork_syncvar_copyargs(
    aligned_prodcons_writer, &a, sizeof(struct arg), &rets[1]);
  qthread_yield();
  qthread_syncvar_readFF(NULL, &rets[0]);
  qthread_syncvar_readFF(NULL, &rets[1]);
} /*}}} */
...
int main(int argc, char *argv[]) {
  ...
  if (TEST_SELECTION & (1 << 0)) {
    ...
    printf(
      "%11g secs (%u iters)\n", qtimer_secs(timer), (unsigned)(ARRAY_SIZE));
