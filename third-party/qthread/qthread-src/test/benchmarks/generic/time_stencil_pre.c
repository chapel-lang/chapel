...
#define NEIGHBORS(stage, i, j)                                                 \
  NORTH(stage, i, j), WEST(stage, i, j), EAST(stage, i, j), SOUTH(stage, i, j)
...
static inline void perform_local_work(void) {
...
#endif // TIME_WORKLOAD
  unsigned long volatile work = workload;
  ...
  for (int i = 0; i < work; i++) { work = work % 1000000000; }
  work++;
#ifdef TIME_WORKLOAD
...
static aligned_t update(void *arg) {
  ...
  aligned_t sum = *(NORTH(prev, i, j)) + *(WEST(prev, i, j)) +
                  *(HERE(prev, i, j)) + *(EAST(prev, i, j)) +
                  *(SOUTH(prev, i, j));
...
#ifdef BOUNDARY_SYNC
    qthread_fork_copyargs_precond(update,
                                  &args,
                                  sizeof(update_args_t),
                                  NULL,
                                  NUM_NEIGHBORS,
                                  NEIGHBORS(points->stage[this_stage], i, j));
#else
    if (i == 1) { // North edge
      if (j == 1) // West edge: EAST & SOUTH
        qthread_fork_copyargs_precond(update,
                                      ...
                                      NULL,
                                      2,
                                      EAST(points->stage[this_stage], i, j),
                                      SOUTH(points->stage[this_stage], i, j));
        ...
        qthread_fork_copyargs_precond(update,
                                      ...
                                      NULL,
                                      2,
                                      WEST(points->stage[this_stage], i, j),
                                      SOUTH(points->stage[this_stage], i, j));
      ...
      if (j == 1)                    // West edge: NORTH & EAST
        qthread_fork_copyargs_precond(update,
                                      ...
                                      NULL,
                                      2,
                                      NORTH(points->stage[this_stage], i, j),
                                      EAST(points->stage[this_stage], i, j));
...
#endif
  } else qt_barrier_enter(points->barrier);
