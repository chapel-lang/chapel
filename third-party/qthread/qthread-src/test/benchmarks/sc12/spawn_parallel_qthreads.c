...
static void par_null_task2(size_t start, size_t stop, void *args_) {
...
static void par_spawn(size_t start, size_t stop, void *args_) {
  for (size_t i = start; i < stop; ++i) {
    qthread_spawn(
...
int main(int argc, char *argv[]) {
  ...
  switch (loop_style) {
    ...
    case 7:
      ...
      while (donecount != count) { qthread_yield(); }
      qtimer_stop(timer);
      break;
    case 8:
      qtimer_start(timer);
