...
static aligned_t cpqueuer(void *arg) {
  ...
  for (i = 0; i < ELEMENT_COUNT; i++) {
    if (cp_list_append(q, (void *)(intptr_t)qthread_id()) == NULL) {
      fprintf(stderr,
              "%i'th cp_list_append(q, %p) failed!\n",
              (int)i,
...
static aligned_t cpdequeuer(void *arg) {
  ...
  for (i = 0; i < ELEMENT_COUNT; i++) {
    while (cp_list_remove_head(q) == NULL) { qthread_yield(); }
  }
  return 0;
}
...
static aligned_t dequeuer(void *arg) {
  ...
  for (i = 0; i < ELEMENT_COUNT; i++) {
    while (qlfqueue_dequeue(q) == NULL) { qthread_yield(); }
  }
  return 0;
}
...
loop_cpdequeuer(size_t const startat, size_t const stopat, void *arg) { /*{{{ */
  ...
  for (i = startat; i < stopat; i++) {
    if (cp_list_remove_head(q) == NULL) {
      fprintf(stderr,
              "cp_list_remove_head(q, %p) failed!\n",
              (void *)(intptr_t)qthread_id());
      exit(-2);
    }
  }
...
int main(int argc, char *argv[]) {
  ...
  printf("loop balance enqueue: %g secs (%g nsecs/enqueue)\n",
         qtimer_secs(timer),
         1e9 * qtimer_secs(timer) / (THREAD_COUNT * ELEMENT_COUNT));
  qtimer_start(timer);
  qt_loop_balance(0, THREAD_COUNT * ELEMENT_COUNT, loop_dequeuer, q);
  qtimer_stop(timer);
