...
static void
assign1_loop(size_t const startat, size_t const stopat, qarray *qa, void *arg) {
  double *ptr = (double *)qarray_elem_nomigrate(qa, startat);
  size_t const max = stopat - startat;

  for (size_t i = 0; i < max; i++) { ptr[i] = 1.0; }
}

static void
assert1_loop(size_t const startat, size_t const stopat, qarray *qa, void *arg) {
  double const *ptr = (double const *)qarray_elem_nomigrate(qa, startat);
  size_t max = stopat - startat;

  for (size_t i = 0; i < max; i++) { assert(ptr[i] == 1.0); }
}

static void assignall1_loop(size_t const startat,
                            size_t const stopat,
                            qarray *qa,
                            void *arg) {
...
static void assertall1_loop(size_t const startat,
                            size_t const stopat,
                            qarray *qa,
                            void *arg) {
...
static void
assignoff1(size_t const startat, size_t const stopat, qarray *qa, void *arg) {
  for (size_t i = startat; i < stopat; i++) {
    char *ptr = (char *)qarray_elem_nomigrate(qa, i);
...
static void
assertoff1(size_t const startat, size_t const stopat, qarray *qa, void *arg) {
...
int main(int argc, char *argv[]) {
  ...
  distribution_t disttypes[] = {FIXED_HASH,
                                FIXED_FIELDS,
                                ...
                                DIST_FIELDS,
                                DIST_LEAST};
  char const *distnames[] = {"FIXED_HASH",
                             "FIXED_FIELDS",
                             "ALL_LOCAL", /*"ALL_RAND", "ALL_LEAST", */
                             "DIST_RAND",
                             "DIST_STRIPES",
  ...
  if (enabled_types & 1) {
    ...
    if (enabled_tests & 1) {
      ...
      for (j = 0; j < ITERATIONS; j++) {
        qtimer_start(timer);
        for (i = 0; i < ELEMENT_COUNT; i++) { a[i] = 1.0; }
        qtimer_stop(timer);
      ...
      for (j = 0; j < ITERATIONS; j++) {
        qtimer_start(timer);
        for (i = 0; i < ELEMENT_COUNT; i++) { assert(a[i] == 1.0); }
        qtimer_stop(timer);
        acctime += qtimer_secs(timer);
      }
      free(a);
  ...
  for (dt_index = 0; dt_index < (sizeof(disttypes) / sizeof(distribution_t));
       dt_index++) {
    ...
    if (enabled_tests & 1) {
      int j;
      double acc = 0.0;
      qarray *a = qarray_create_configured(
        ELEMENT_COUNT, sizeof(double), disttypes[dt_index], 1, 0);
      assert(a != NULL);
      for (j = 0; j < ITERATIONS; j++) {
        qtimer_start(timer);
    ...
    if (enabled_tests & 2) {
      ...
      double acc = 0.0;
      qarray *a = qarray_create_configured(
        ELEMENT_COUNT, sizeof(bigobj), disttypes[dt_index], 0, 0);
      assert(a != NULL);
      for (j = 0; j < ITERATIONS; j++) {
        qtimer_start(timer);
    ...
    if (enabled_tests & 4) {
      ...
      double acc = 0.0;
      qarray *a = qarray_create_configured(
        ELEMENT_COUNT, sizeof(offsize), disttypes[dt_index], 0, 0);
      assert(a != NULL);
      for (j = 0; j < ITERATIONS; j++) {
        qtimer_start(timer);
