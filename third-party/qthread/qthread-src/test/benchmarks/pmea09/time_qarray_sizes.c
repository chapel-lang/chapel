...
assert1_loop(size_t const startat, size_t const stopat, qarray *qa, void *arg) {
  ...
  for (i = startat; i < stopat; i++) {
    ...
    if (memcmp(ptr, example, size) != 0) {
      fprintf(
        stderr, "...assignment failed! (%lu bytes)\n", (unsigned long)size);
      assert(0);
    }
  }
  free(example);
...
int main(int argc, char *argv[]) {
  ...
  size_t const sizes[] = {
    ...
    23, 32, 64, 65, 71, 100, 128, 256, 301, 333, 512 /*, 1024, 2048, 4096, 5000,
                                                        10000, 16384 */
  };
  ...
  for (dt_index = 0; dt_index < (sizeof(disttypes) / sizeof(distribution_t));
       dt_index++) {
    if (((dist_requested >> dt_index) & 1) == 0) { continue; }
    printf("%s:\n", distnames[dt_index]);
    {
      size_t size_i;
      ...
      for (size_i = 0; size_i < (sizeof(sizes) / sizeof(size_t)); size_i++) {
        size_t const size = sizes[size_i];
        qarray *a = qarray_create_configured(
          ELEMENT_COUNT, size, disttypes[dt_index], 0, 0);
        ...
        qtimer_stop(timer);
        printf(
          "\t%lu, %f", (unsigned long)sizes[size_i], qtimer_secs(timer) / 10.0);
        qtimer_start(timer);
        for (unsigned i = 0; i < 10; i++) {
      ...
      for (size_i = 0; size_i < (sizeof(sizes) / sizeof(size_t)); size_i++) {
        size_t const size = sizes[size_i];
        size_t const count = constant_size / size; /* 1GB */
        qarray *a =
          qarray_create_configured(count, size, disttypes[dt_index], 0, 0);
