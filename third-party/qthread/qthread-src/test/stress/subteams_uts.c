...
static double normalize(int n) {
  if (n < 0) { iprintf("*** toProb: rand n = %d out of range\n", n); }
...
static aligned_t visit(void *args_) {
  ...
  if (0 == root_context) { qthread_incr(&donecount, 1); }
...
int main(int argc, char *argv[]) {
  ...
  switch (root_context) {
    case 0:
      ...
      while (nodecount != donecount) { qthread_yield(); }
      break;
    case 1:
      ...
      break;
    case 2:
      qthread_fork_new_subteam(
        visit, &root, &ret); // Root in subteam of default team
      qthread_readFF(NULL, &ret);
      break;
    case 3:
      qthread_fork_new_team(
        run_visit, &root, &ret); // Root in subteam of non-default team
      qthread_readFF(NULL, &ret);
      break;
    default:
