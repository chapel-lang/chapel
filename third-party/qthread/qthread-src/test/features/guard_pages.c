...
void bus_sigaction(int signal, siginfo_t *si, ucontext_t *arg) { exit(0); }

static aligned_t buserr_thread(void *arg) {
  char volatile *eos; // end of stack

  eos = (char volatile *)((unsigned long long int)((
    (qthread_id() % 2) ? qthread_bos() + 256 : qthread_tos() - 256)));
  iprintf(
...
int main(int argc, char *argv[]) {
  ...
  if (!getenv("GUARD_PAGES")) setenv("GUARD_PAGES", "1", 0);
  ...
  for (iter = 0; iter <= iterations; ++iter) {
    for (i = 1; i < threads; ++i) {
      qthread_spawn(buserr_thread, NULL, 0, rets + i, 0, NULL, i, 0);
    }
  }
