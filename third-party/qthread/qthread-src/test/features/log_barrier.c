...
aligned_t volatile *initme = NULL;
...
static aligned_t barrier_thread(void *arg) {
  ...
  // iprintf("I woke up on shep %u worker %u, barrier %p, using ID number %u,
  // idx %u\n", shep, wkr, b, id, idx);
...
int main(int argc, char *argv[]) {
  ...
  wait_on_me =
    qt_barrier_create(threads, REGION_BARRIER, 0); // all my spawnees plus me
  assert(wait_on_me);
  ...
  for (iter = 0; iter < iterations; iter++) {
    ...
    for (i = 1; i < threads; i++) {
      ...
      qthread_spawn(
        barrier_thread, arg, sizeof(void *) * 2, rets + i, 0, NULL, i, 0);
    }
    ...
    qtimer_stop(t);
    iprintf(
      "%i: main thread exited barrier in %f seconds\n", iter, qtimer_secs(t));
    tot += qtimer_secs(t);
