...
static void runs(void) {
  ...
  sigmab2 = (2.0 * n1 * n2 * ((2.0 * n1 * n2) - ITER)) /
            (double)(ITER * ITER * (ITER - 1));
  ...
  for (i = 1; i < ITER; ++i) {
    if (A[i] > A[i - 1]) {
      if (lastdir == -1) {
        if (maxrun < runlengths[rl]) { maxrun = runlengths[rl]; }
        rl++;
      }
      lastdir = 1;
      runlengths[rl]++;
  ...
  for (i = 0; i < a; ++i) { oi[0][runlengths[i] - 1]++; }
  /* expected run calculation */
  for (i = 0; i < maxrun; ++i) {
    if (i <= ITER - 2) {
      oi[1][i] = (2.0 / bang(i + 3.0)) *
                 (ITER * (i * i + 3 * i + 1) - (i * i * i + 3 * i * i - i - 4));
    } else {
  ...
  chisquare = 0.0;
  for (i = 0; i < maxrun; ++i) { chisquare += oi[2][i]; }
...
static void ks_test(void) {
  ...
  for (size_t i = 1; i < ITER; ++i) {
    if (A[1][i] - A[0][i] > Dplus) { Dplus = A[1][i] - A[0][i]; }
...
static void autocorrelation(void) {
  ...
  iprintf("\nAutocorrelation:\n");
  for (size_t i = 0; i < ITER + 1; ++i) { A[i] = qtimer_fastrand(); }

  for (eye = 1; eye < 51; ++eye) {
    for (m = 1; m < 11; ++m) {
      M = floor((ITER - eye) / (double)m) - 1;
      pim = 0.0;
      for (k = 0; k <= M; ++k) { pim += A[eye + k * m] * A[eye + (k + 1) * m]; }
      pim *= (1.0 / (M + 1.0));
      pim -= 0.25;
...
int main(int argc, char *argv[]) {
  ...
  if (qtimer_secs(t) == 0) {
    ...
    fprintf(stderr,
            "qtimer_secs(t) thinks time went backwards (%g).\n",
            qtimer_secs(t));
  }
