...
static aligned_t use_default_space(void *arg) {
  ...
  for (int i = 0; i < 64; ++i) { local_int_arr[i] = i; }
  iprintf(
    "use_default_space(): assigned tasklocal data successfully (42 = %d)\n",
    local_int_arr[42]);
  local_int_arr = NULL;
  qthread_yield();
  local_int_arr = (int *)qthread_get_tasklocal(size);
...
static aligned_t use_allocated_space(void *arg) {
  ...
  local_int_arr = (int *)qthread_get_tasklocal(size);
  for (int i = 0; i < 2048; ++i) { local_int_arr[i] = 2048 - i; }
  iprintf(
    "use_allocated_space(): assigned tasklocal data successfully (42 = %d)\n",
    local_int_arr[42]);
  local_int_arr = NULL;
  qthread_yield();
  local_int_arr = (int *)qthread_get_tasklocal(size);
