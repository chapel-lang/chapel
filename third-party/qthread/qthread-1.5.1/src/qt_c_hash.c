...
static inline size_t encompassing_power_of_two(
    size_t k)
{
    size_t z = 1;
    while (z < k)
...
static inline void qt_hash_internal_create(
    qt_hash ret,
    size_t entries)
{
...
static inline void **qt_hash_internal_find(
    qt_hash h,
    qt_key_t key)
{
...
void qt_hash_destroy_deallocate(
    qt_hash h,
    ...
    size_t len)
{
	...
	for (i = 0; i < h->num_entries; ++i) {
	    if (h->entries[i].key > KEY_DELETED) {
		qassertnot(qt_hash_put_locked
			   (d, h->entries[i].key, h->entries[i].value), NULL);
		++copied;
		if (copied == h->population) {
		    break;
		}
