...
#endif
      {
/* 	fprintf(stderr, "%d:: Sending REQ to proc=%d bytes=%d ehlen=%d\n", armci_me, proc,bytes,msginfo->ehlen); */
       armci_send_req_msg(proc,msginfo, bytes);
/*        fprintf(stderr, "%d:: Done sending REQ to proc=%d bytes=%d ehlen=%d\n", armci_me, proc,bytes,msginfo->ehlen); */
      }
}
...
/*     fprintf(stderr,"%d(serv):got %d request from %d (ehlen=%d)\n",armci_me,msginfo->operation, */
/* 	   from,msginfo->ehlen); */
/*     fflush(stdout); */
