                          ...
                          char *dscr, char* buf, int buflen)
{
    ...
    switch(msginfo->operation) {
    case GET:
/*       fprintf(stderr, "%d(serv):: Got a vector message!!\n", armci_me); */
      if(msginfo->ehlen) {
#if defined(ARMCI_ENABLE_GPC_CALLS) && (defined(GM) || defined(VAPI) || defined(DOELAN4) || defined(SOCKETS)) 
/* 	fprintf(stderr, "%d(serv)::Invoking gpc_call_process\n",armci_me); */
	gpc_call_process(msginfo, len, dscr, buf, buflen, sbuf);
/* 	fprintf(stderr, "%d(serv)::Returned from gpc_call_process\n",armci_me); */
#else
...
#if defined(GM) || defined(VAPI) || defined(QUADRICS) || defined(SOCKETS)
gpc_buf_t *gpc_req=NULL;
...
/*     fprintf(stderr, "%d: Entered armci_rem_gpc\n", armci_me); */
/*     sleep(1); */
...
#endif
    {
/*       fprintf(stderr, "%d:: rem_gpc. bufsize=%d Alloca-ing buf\n", armci_me, bufsize); */
/*       sleep(2); */
      buf = buf0= GET_SEND_BUFFER(bufsize,op,proc);
/*       fprintf(stderr, "%d:: rem_gpc. bufsize=%d DONE alloc-ing buf\n", armci_me, bufsize); */
    }
/*     fprintf(stderr, "%d:: rem_gpc. bufsize=%d bytes=%d\n", armci_me, bufsize,bytes); */
/*     sleep(1); */
/*     fprintf(stderr, "%d:: !!!! Obtained Send Buffer!!! Rest look later. Yay!\n", armci_me); */
/*     assert(0); */
    msginfo = (request_header_t*)buf;
    bzero(msginfo,sizeof(request_header_t));
...
#if defined(USE_SOCKET_VECTOR_API) 
/*     fprintf(stderr, "%d:: rem_gpc. Defined sock vec api. Why??\n", armci_me); */
/*     assert(0); */
    if(flag&&(op==GET||op==PUT)){
    	armci_direct_vector(msginfo,darr,len,proc);
        return 0;
...
/*     fprintf(stderr, "%d:: rem_gpc. Sending request. dscrlen=%d bytes=%d bufsize=%d redhdr=%d\n",  */
/* 	    armci_me,msginfo->dscrlen,msginfo->bytes,bufsize,sizeof(request_header_t)); */
/*     assert(bufsize == msginfo->bytes+sizeof(request_header_t)); */
#ifdef SOCKETS
    /*SK: Wonder how other networks work with the inaccurate bufsize*/
    armci_send_req(proc, msginfo, sizeof(request_header_t)+msginfo->bytes);
#else
    armci_send_req(proc, msginfo, bufsize);
#endif
    if(nb_handle && op==GET)armci_save_vector_dscr(&buf0,darr,len,op,1);
...
/*       fprintf(stderr, "%d:: rem_gpc. Calling armci_complete_vector_get to=%d\n", armci_me,msginfo->to); */
       armci_complete_vector_get(darr,len,msginfo);
/*        fprintf(stderr, "%d:: rem_gpc. Returning from armci_complete_vector_get to=%d\n", armci_me,msginfo->to); */
    }
    return 0;
}
