...
/*     if(DEBUG_){ */
/*  printf("%d:armci_ReadFromDirect:  from %d. cluster=%d sock=%d \n",armci_me,proc,cluster,SRV_sock[cluster]); */
/*       fflush(stdout); */
/*     } */
...
#endif
/*     fprintf(stderr, "%d(serv):: ReadFromSocket. msg. p=%d from=%d op=%d dscrlen=%d bytes=%d ehlen=%d\n", */
/* 	    armci_me,p,msginfo->from, msginfo->operation,msginfo->dscrlen,msginfo->bytes,msginfo->ehlen); */
    *buflen = MSG_BUFLEN - hdrlen; 
    if (msginfo->operation == GET) {
      if(msginfo->format==VECTOR && msginfo->ehlen>0) {
	/*GPC message. Read the whole message*/
	bytes = msginfo->bytes;
      }
      else
	bytes = msginfo->dscrlen; 
...
/* 	 fprintf(stderr, "%d(serv): created sock=%d for talking with client=%d\n", */
/* 		 armci_me, CLN_sock[p],p); */
     }

    /*Memory for GPC data structures*/
#ifdef ARMCI_ENABLE_GPC_CALLS
    gpc_req = (gpc_buf_t *)malloc(MAX_GPC_REQ*sizeof(gpc_buf_t));
    assert(gpc_req != NULL);
#endif
...
/*       fprintf(stderr,"%d: client connected to %d. sock=%d port=%d\n",armci_me, */
/* 	      armci_clus_info[c].master, SRV_sock[c], AR_port[off_port + armci_me]); */
/*       assert((c==armci_clus_me) || (SRV_sock[c] != -1)); */
      assert((SRV_sock[c] != -1));
  }
