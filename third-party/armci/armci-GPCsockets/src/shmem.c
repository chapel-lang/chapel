...
{
int nreg, reg;
    fprintf(stderr, "%d:find_regions v2:ptr=%p\n",armci_me,addrp);
    ...
    if(last_allocated!=-1){
       ...
       if(reg == -1) {
          ...
          fprintf(stderr,"%d: find_regions: faild to locate shared region",armci_me,0L);
          pause();
       }
    }
...
size_t sz = (size_t)size;
  ...
  shmflag= SHM_SGI_ANYADDR;
    ...
    if(DEBUG2_){
      ...
      fprintf(stderr,"%d:allocate v2:id=%d addr=%p size=%ld addr-end=%p\n",armci_me,id,temp,size,temp+size);
#ifdef ALLOW_PIN
