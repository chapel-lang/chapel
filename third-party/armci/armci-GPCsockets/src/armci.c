...
#if defined(CRAY_SHMEM) || defined(PORTALS)
#  ifdef XT3
#    include <mpp/shmem.h>
#  else
#    include <shmem.h>
...
#if 0
void _print_armci_handle(armci_ihdl_t nb_hdl)
{
    if(nb_hdl){
	  printf("\n%d:tag=%d bufid=%d agg_flag=%d op=%d proc=%d cmpl_info=%p",
			  armci_me,nb_hdl->tag,nb_hdl->bufid,nb_hdl->agg_flag,
			  nb_hdl->op,nb_hdl->proc,nb_hdl->cmpl_info);
    }
}
#endif
...
/*   fprintf(stderr, "%d:: Invoking armci_rem_rpc\n", armci_me); */
/*   sleep(1); */
