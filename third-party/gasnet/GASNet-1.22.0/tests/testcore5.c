/*   $Source: /var/local/cvs/gasnet/tests/testcore5.c,v $
 *     $Date: 2013/06/20 21:42:06 $
 * $Revision: 1.1 $
...
#define SARGS(dest,args) (dest, hidx_Shandler(args), args, HARGS(args))
#define MARGS(dest,args) (dest, hidx_Mhandler(args), rand_payload, medsz, args, HARGS(args))
#define LARGS(dest,args,isRep) (dest, hidx_Lhandler(args), rand_payload, \
                                longsz, peerseg + isRep*longsz, args, HARGS(args))
...
#define HBODY(args) do {                                           \
    ...
    switch(operation) {                                            \
      case op_done:                                                \
        ...
        break;                                                     \
      case op_srep:                                                \
        DO_CALL(gasnet_AMReplyShort, SARGS(token,args));     \
        break;                                                     \
      case op_mrep:                                                \
        DO_CALL(gasnet_AMReplyMedium, MARGS(token,args));    \
        break;                                                     \
      case op_lrep:                                                \
        DO_CALL(gasnet_AMReplyLong, LARGS(token,args,1));    \
        break;                                                     \
      default:                                                     \
...
#define HTEST(args) \
  ...
  for (i = 0; i < iters; ++i) {                                      \
    ...
    randomize();                                                     \
    arg1 = op_srep;                                                  \
      DO_CALL(gasnet_AMRequestShort,SARGS(peer,args));         \
      GASNET_BLOCKUNTIL(flag == goal); ++goal;                       \
    arg1 = op_mrep;                                                  \
      DO_CALL(gasnet_AMRequestMedium,MARGS(peer,args));        \
      GASNET_BLOCKUNTIL(flag == goal); ++goal;                       \
    arg1 = op_lrep;                                                  \
      DO_CALL(gasnet_AMRequestLong,LARGS(peer,args,0));        \
      GASNET_BLOCKUNTIL(flag == goal); ++goal;                       \
      DO_CALL(gasnet_AMRequestLongAsync,LARGS(peer,args,0));   \
      GASNET_BLOCKUNTIL(flag == goal); ++goal;                       \
  }
...
void ping_shorthandler(gasnet_token_t token) {
    GASNET_Safe(gasnet_AMReplyShort(token, hidx_pong_shorthandler, 0));
}
...
void ping_medhandler(gasnet_token_t token, void *buf, size_t nbytes) {
    MSGCHECK(medsz);
    GASNET_Safe(gasnet_AMReplyMedium(token, hidx_pong_medhandler, rand_payload, nbytes, 0));
}
...
void ping_longhandler(gasnet_token_t token, void *buf, size_t nbytes) {
    ...
    GASNET_Safe(gasnet_AMReplyLong(token, hidx_pong_longhandler, rand_payload, nbytes, peerseg + longsz, 0));
}
...
int main(int argc, char **argv) {
  ...
  for (i = 0; i < iters; ++i) {
    ...
    GASNET_Safe(gasnet_AMRequestShort(peer, hidx_ping_shorthandler, 0));
    GASNET_BLOCKUNTIL(flag == goal); ++goal;

    GASNET_Safe(gasnet_AMRequestMedium(peer, hidx_ping_medhandler, rand_payload, medsz, 0));
    GASNET_BLOCKUNTIL(flag == goal); ++goal;

    GASNET_Safe(gasnet_AMRequestLong(peer, hidx_ping_longhandler, rand_payload, longsz, peerseg, 0));
    GASNET_BLOCKUNTIL(flag == goal); ++goal;
 
    GASNET_Safe(gasnet_AMRequestLongAsync(peer, hidx_ping_longhandler, rand_payload, longsz, peerseg, 0));
    GASNET_BLOCKUNTIL(flag == goal); ++goal;
  }
