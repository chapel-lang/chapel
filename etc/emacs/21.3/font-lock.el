  ...
  (let (;; We use `beginning-of-defun', rather than nil, for SYNTAX-BEGIN.
	...
	(c++-mode-defaults
	 '((c++-font-lock-keywords c++-font-lock-keywords-1
	   ...
	   (font-lock-mark-block-function . mark-defun)))
        (chpl-mode-defaults
         '((chpl-font-lock-keywords chpl-font-lock-keywords-1
            chpl-font-lock-keywords-2 chpl-font-lock-keywords-3)
           nil nil ((?_ . "w")) beginning-of-defun
           (font-lock-mark-block-function . mark-defun)))
	(objc-mode-defaults
    ...
    (list
     ...
     (cons 'c++-mode			c++-mode-defaults)
     (cons 'chpl-mode                   chpl-mode-defaults)
...
(defcustom chpl-font-lock-extra-types '("bradtype")
  "*List of extra types to fontify in Chpl mode.
Each  list item should be a regexp not containing word-delimiters.
For example, a value of (\"FILE\" \"\\\\sw+_t\") means the word FILE and words
ending in _t are treated as type names.
...
(defconst chpl-font-lock-keywords-1 nil
  "Subdued level highlighting for Chpl mode.")

(defconst chpl-font-lock-keywords-2 nil
  "Medium level highlighting for Chpl mode.
See also `chpl-font-lock-extra-types'.")

(defconst chpl-font-lock-keywords-3 nil
  "Gaudy level highlighting for Chpl mode.
See also `chpl-font-lock-extra-types'.")
...
(let* ((chpl-keywords
	...
	`(regexp-opt-depth ,chpl-type-names))
 ...
 (setq chpl-font-lock-keywords-1
  (list
   ...
   ;; Fontify variable name definitions
   '("const \\(\\sw+\\)[ \t]*" 1 font-lock-variable-name-face)
   '("param \\(\\sw+\\)[ \t]*" 1 font-lock-variable-name-face)
   '("var \\(\\sw+\\)[ \t]*" 1 font-lock-variable-name-face)
   ;;
   ;; Fontify error directives.
   '("^#[ \t]*error[ \t]+\\(.+\\)" 1 font-lock-warning-face prepend)
   ;;
   ...
   ;; Fontify otherwise as symbol names, and the preprocessor directive names.
   (list
    (concat "^#[ \t]*\\(" chpl-preprocessor-directives
	    "\\)\\>[ \t!]*\\(\\sw+\\)?")
    '(1 font-lock-builtin-face)
    (list (+ 2 chpl-preprocessor-directives-depth)
	  'font-lock-variable-name-face nil t))))

 (setq chpl-font-lock-keywords-2
  (append chpl-font-lock-keywords-1
   (list
    ;;
    ;; Simple regexps for speed.
    ;;
    ;; Fontify all type names.
    `(eval .
      (cons (concat "\\<\\(" ,chpl-type-names "\\)\\>") 'font-lock-type-face))
    ;;
    ;; Fontify operator overloading.
    (list (concat "\\<\\(function\\)\\>[ \t]*\\(" chpl-operators "\\)?")
	  '(1 font-lock-keyword-face)
	  '(2 font-lock-builtin-face nil t))
    ;;
    ;; Fontify all builtin keywords (except case, default and goto; see below).
    (concat "\\<\\(" chpl-keywords "\\|" chpl-type-specs "\\)\\>")
    ;;
    ;; Fontify case/goto keywords and targets, and case default/goto tags.
    '("\\<\\(case\\|goto\\)\\>"
      (1 font-lock-keyword-face)
 ...
 (setq chpl-font-lock-keywords-3
  (append chpl-font-lock-keywords-2
   ;;
   ;; More complicated regexps for more complete highlighting for types.
   ;; We still have to fontify type specifiers individually, as C is so hairy.
   (list
    ...
    ;; Fontify all storage specs and types, plus their items.
    `(eval .
      (list (concat "\\<\\(" ,chpl-type-specs "\\)\\>"
		    "[ \t]*\\(\\sw+\\)?")
	  (list 1 'font-lock-keyword-face)
	  (list ,(+ chpl-type-specs-depth 2) 'font-lock-type-face nil t)
	  (list 'font-lock-match-c-style-declaration-item-and-skip-to-next
		  nil 
		  ;; Finish with point after the variable name if
		  ;; there is one.
...
(defvar chpl-font-lock-keywords chpl-font-lock-keywords-1
...
;; (defun font-lock-match-chpl-style-declaration-item-and-skip-to-next (limit)
;;   ;; Regexp matches after point:		word<word>::word (
;;   ;;						^^^^ ^^^^   ^^^^ ^
;;   ;; Where the match subexpressions are:	  1    3      5  6
;;   ;;
...
;; 		       ;; Skip any leading whitespace.
;; 		       "[ \t*&]*"
;; 		       ;; This is `chpl-type-spec' from below.  (Hint hint!)
;; 		       "\\(\\sw+\\)"				; The instance?
;; 		       "\\([ \t]*<\\([^>\n]+\\)[ \t*&]*>\\)?"	; Or template?
;; 		       "\\([ \t]*::[ \t*~]*\\(\\sw+\\)\\)*"	; Or member?
;; 		       ;; Match any trailing parenthesis.
...
;; (defun font-lock-match-chpl-structor-declaration (limit)
;;   ;; Match Chpl constructors and destructors inside class declarations.
;;   (let ((res nil)
;; 	(regexp (concat "^\\s-+\\(\\(virtual\\|explicit\\)\\s-+\\)*~?\\(\\<"
;; 			(mapconcat 'identity
;; 				   chpl-font-lock-extra-types "\\|")
...
;; 	   chpl-font-lock-extra-types)
;; 	  "\\|"))
;;        (chpl-type-names-depth `(regexp-opt-depth ,chpl-type-names))
;;        ;;
;;        ;; A brave attempt to match templates following a type and/or match
;;        ;; class membership.  See and sync the above function
;;        ;; `font-lock-match-chpl-style-declaration-item-and-skip-to-next'.
;;        (chpl-type-suffix (concat "\\([ \t]*<\\([^>\n]+\\)[ \t*&]*>\\)?"
;; 				"\\([ \t]*::[ \t*~]*\\(\\sw+\\)\\)*"))
;;        (chpl-type-suffix-depth (regexp-opt-depth chpl-type-suffix))
;;        ;; If the string is a type, it may be followed by the cruft above.
;;        (chpl-type-spec (concat "\\(\\sw+\\)\\>" chpl-type-suffix))
;;        (chpl-type-spec-depth (regexp-opt-depth chpl-type-spec))
;;        ;;
;;        ;; Parenthesis depth of user-defined types not forgetting their cruft.
;;        (chpl-type-depth `(regexp-opt-depth
;; 			 (concat ,chpl-type-names ,chpl-type-suffix)))
;;        )
;;  (setq chpl-font-lock-keywords-1
;;   (append
;;    ;;
...
;;    (list
;;     ;;
;;     ;; The list `c-font-lock-keywords-2' for Chpl plus operator overloading.
;;     `(eval .
;;       (cons (concat "\\<\\(" ,chpl-type-names "\\)\\>")
;; 	    'font-lock-type-face))
;;     ;;
;;     ;; Fontify operator overloading.
;;     (list (concat "\\<\\(operator\\)\\>[ \t]*\\(" chpl-operators "\\)?")
;; 	  '(1 font-lock-keyword-face)
;; 	  '(2 font-lock-builtin-face nil t))
;;     ;;
;;     ;; Fontify case/goto keywords and targets, and case default/goto tags.
...
;;     ;; Fontify all storage classes and type specifiers, plus their items.
;;     `(eval .
;;       (list (concat "\\<\\(" ,chpl-type-names "\\)\\>" ,chpl-type-suffix
;; 		    "\\([ \t*&]+" ,chpl-type-spec "\\)*")
;; 	    ;; The name of any template type.
;; 	    (list (+ ,chpl-type-names-depth 3) 'font-lock-type-face nil t)
;; 	    ;; Fontify each declaration item.
;; 	    (list 'font-lock-match-chpl-style-declaration-item-and-skip-to-next
;; 		  ;; Start with point after all type specifiers.
;; 		  (list 'goto-char (list 'or (list 'match-beginning
;; 						   (+ ,chpl-type-depth 2))
;; 					 '(match-end 1)))
...
;;     ;; Fontify all storage specs and types, plus their items.
;;     `(eval .
;;       (list (concat "\\<" ,chpl-type-specs "\\>" ,chpl-type-suffix
;; 		    "[ \t]*\\(" ,chpl-type-spec "\\)?")
;; 	    ;; The name of any template type.
;; 	    (list ,(+ chpl-type-specs-depth 2) 'font-lock-type-face nil t)
;; 	    ;; The name of any type.
;; 	    (list (+ ,chpl-type-specs-depth ,chpl-type-suffix-depth 2)
;; 		  'font-lock-type-face nil t)
;; 	    ;; Fontify each declaration item.
;; 	    (list 'font-lock-match-chpl-style-declaration-item-and-skip-to-next
;; 		  ;; Start with point after all type specifiers.
...
;;     ;;
;;     ;; Fontify constructors and destructors inside class declarations.
;;     '(font-lock-match-chpl-structor-declaration
;;       (3 font-lock-function-name-face t))
;;     )))
;;  )
