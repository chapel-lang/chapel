...
;;;###autoload
(defun chpl-mode ()
  "Major mode for editing Chpl code.
To submit a problem report, enter `\\[c-submit-bug-report]' from a
c-mode buffer.  This automatically sets up a mail buffer with version
information already added.  You just need to add a description of the
problem, including a reproducible test case and send the message.
...
\\{chpl-mode-map}"
  ...
  (setq comment-start "-- "
        comment-end ""
        c-keywords (c-identifier-re c-Chpl-keywords)
        c-conditional-key c-Chpl-conditional-key
        c-comment-start-regexp c-Chpl-comment-start-regexp
        c-class-key c-Chpl-class-key
        c-baseclass-key nil
        c-extra-toplevel-key c-Chpl-extra-toplevel-key
;        c-access-key c-Chpl-access-key
        c-recognize-knr-p nil
;        c-bitfield-key c-Chpl-bitfield-key
        )
