...
static void* chunk_alloc(void *chunk, size_t size, size_t alignment, bool *zero, bool *commit, unsigned arena_ind) {
  ...
  //
  //   jemalloc 4.2.0 man: "The alignment parameter is always a power of two at
  //   least as large as the chunk size."
  cur_chunk_base = alignHelper(heap.base, heap.cur_offset, alignment);

  // jemalloc 4.2.0 man: "If chunk is not NULL, the returned pointer must be
  ...
  // jemalloc 4.2.0 man: "Zeroing is mandatory if *zero is true upon entry."
  if (*zero) {
...
static void initialize_arenas(void) {
  ...
  //
  //   jemalloc 4.2.0 man: "If the specified arena was not initialized
...
// our shared heap
//
//   jemalloc 4.2.0 man: "arenas may have already created chunks prior to the
