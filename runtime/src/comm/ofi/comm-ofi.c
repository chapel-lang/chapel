...
void mcmReleaseAllNodes(struct bitmap_t* b, struct perTxCtxInfo_t* tcip,
  ...
  BITMAP_FOREACH_SET(b, node) {
    bitmapClear(b, node);
    checkTxCQ(myTcip);
    while (myTcip->numTxnsOut >= txCQLen) { // need CQ room for at least 1 txn
      sched_yield();
      checkTxCQ(myTcip);
    }
...
void ofi_put_V(int v_len, void** addr_v, void** local_mr_v,
  ...
  //
  if (v_len > txCQLen - tcip->numTxnsOut) {
    checkTxCQ(tcip);
...
static
void ensureProgress(struct perTxCtxInfo_t* tcip) {
  if (tcip->txCQ != NULL) {
    checkTxCQ(tcip);
  } else {
    getTxCntr(tcip);
...
static inline
void getTxCntr(struct perTxCtxInfo_t* tcip) {
