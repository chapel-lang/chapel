...
void chpl_comm_ofi_oob_allgather(void* mine, void* all, int size) {
  ...
  chpl_no_overlap_memcpy(&g_mine->info, mine, size);
  ...
  for (int g_i = 0; g_i < chpl_numNodes; g_i++) {
    ...
    int i;
    chpl_no_overlap_memcpy(&i, g_pa + offsetof(gather_t, nodeID), sizeof(i));
    char* p_a = (char*) all + i * size;
    chpl_no_overlap_memcpy(p_a, g_pa + offsetof(gather_t, info), size);
  }
