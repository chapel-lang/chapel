  ...
  default:
    ...
    if (numRead != 0) {
      if (strstr(buf, "internal error: ") == NULL) {
        chpl_no_overlap_memcpy(outbuf, buf, numRead);
      } else {
...
char** chpl_bundle_exec_args(int argc, char *const argv[],
  ...
  if (largc > 0) {
    chpl_no_overlap_memcpy(newargv, largv, largc*sizeof(char *));
    newargc = largc;
  }
  if (argc > 0) {
    ...
    if (argc > 1) {
      ...
      chpl_no_overlap_memcpy(newargv+newargc, argv+1, (argc-1)*sizeof(char *));
    }
  }
