...
// Per-class information generated by the compiler, indexed by class id.
typedef struct _chpl_class_info {
  const chpl_fn_p* vmt;               // per-class virtual method table, NULL if none
  chpl__class_id   maxSubclassId;     // largest class id in this class's subtree
  int32_t          numVirtualMethods; // number of entries in vmt
  const char*      name;
} chpl_class_info;
