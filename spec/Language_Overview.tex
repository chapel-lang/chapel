...
module's public global symbols.  A module may be declared using a
module declaration that specifies its name as follows:

\begin{chapel}
module (*\emph{<moduleName>}*) { (*$\ldots$*) }
\end{chapel}

Alternatively, for convenience in exploratory programming,
...
%% More here on compiling multiple files?
...
Variable declarations in Chapel take the following basic form:

\begin{chapel}
(*\emph{<variable-kind>}*) (*\emph{<identifier>}*) (*$[\,$*): (*\emph{<definition>}*)(*$\,]$*) (*$[\,$*)= (*\emph{<initializer>}*)(*$\,]$*);
\end{chapel}

\noindent where \emph{variable-kind} indicates the kind of variable
being created, \emph{identifier} specifies the variable's name,
\emph{definition} indicates the variable's ``type'' (loosely
construed), and \emph{initializer} specifies its initial value.  A
variable's initializer may be omitted, in which case it will be
initialized to an value dependent on its definition for safety (\eg,
``zero'' for numerical types).  Alternatively, a variable's definition
...
\noindent Working backwards, the \chpl{var} keyword indicates that a
...
variable-kind, definition or initializer.  
...
also support for strings as primitive types.  Table~\ref{typeSizeTable}
...
{\tt imag} & 64 bits & {\tt imag(32)} & {\tt imag(65)} & {\tt imag(128)} & \\
...
\hline
\end{tabular}
\caption{Sizes of Chapel primitive types.}  
\end{center}
\label{typeSizeTable}
\end{table}
...
\begin{example}
The following lines of Chapel code give examples of different
...
The constant \chpl{n} can be set at runtime, or it is set to its
default value of \chpl{10}.  It is inferred to be of type \chpl{int} from
this integer default value.  Similarly, \chpl{x} and \chpl{y} are
inferred to be of type \chpl{real} and are initialized to \chpl{1.0}
and \chpl{10.0} respectively.  The  variable \chpl{z} is the only
one that has an explicit type declaration.  Because \chpl{z} is not
initialized, it has a default intial value of \chpl{0.0}.
\end{example}
