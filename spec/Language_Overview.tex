...
Sequences are constructs that, among other uses, may be used in iterating in \chpl{for}
and \chpl{for all} loops and in defining domains.  
...
\end{chapel}
\end{example}
The variable \chpl{order} is declared to be a sequence of integers
and it is initialized to be \chpl{(/ 1 /)}.  In general, literal
...
the sequence \chpl{order} is constructed an element at a time by concatenating
\chpl{i} at each iteration.  After the first iteration of the loop, 
\chpl{order = (/ 1, 2 /)}.  At the end of the loop, \chpl{order} is a sequence
of integers from \chpl{1} to \chpl{n}.

During the insertion sort, each time the list is changed so is \chpl{order},
...
In the following example, the iterator produces the first \chpl{n} even
numbers.  
\begin{chapel}
