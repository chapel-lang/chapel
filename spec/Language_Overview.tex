...
{\em producer thread}.  The consumer thread writes the value of the \chpl{sync} 
variable \chpl{s} at each iteration.  It reads the value of \chpl{s} when it is 
ready to be read and leaves it in a state that allows it to be written.  
The producer thread sleeps for
...
can be set at run-time to override default values, through command 
line switches.  For example, to change the number of iterations to 10
...
thread will suspend until another thread changes its state to full.  Once the thread
is able to read a sync variable, its state is atomically set to empty.  
...
in the \chpl{Time} standard module, and it cause the producer thread to
