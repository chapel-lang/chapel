...
In this case, tasks are spawned through the use of the \chpl{begin}
or \chpl{co-begin} statements.  Synchronization between these tasks is
managed through the use of \chpl{sync} and \chpl{single} variables.
...
the \chpl{begin} statement see~\rsec{Begin}.
...
\begin{example}
If three consumer tasks needed to be started, a \chpl{cobegin}
...
Synchronization between tasks is managed through \chpl{sync}
...
about synchronization variables see~\rsec{Sync_Variables}
...
see~\rsec{Locales} and~\rsec{Specifying_Locales}.
...
The \chpl{on} statement controls on which locale a task should be
