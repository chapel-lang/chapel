...
to the domain's distribution.  When domains are used as iterators in \chpl{for}
\chpl{forall} statements, the iterations are distributed among processors
according to the domain's distribution.

There are two domains that are used in this program, \chpl{ProblemSpace}
and \chpl{BigDomain}.  By defining \chpl{ProblemSpace} to be the interior
points of the larger grid, \chpl{BigDomain}, the computation can be cleanly specified
