...
Tuples are very useful constructs.  Two examples of how tuples
...
a Givens rotation.  The variables, \chpl{a, b, s, c} are declared
outside of the block and are, in this case, assumed to be visible
within the block as well.  The variable \chpl{tau} is declared 
...
The next \chpl{for} loop writes the indices
and array elements in a particular way.  This \chpl{for} loop
body is not a block statement, so the keyword \chpl{do} must
be used to delimit the loop body.  In this loop, the index
expression is a destructured tuple so that the \chpl{i} and
\chpl{j} components of the indices in \chpl{D} can be accessed
