...
following one with an \sntx{init-part} will be associated with an
integer value one higher than its predecessor.  An enumerated type
whose first constan has an \sntx{init-part} is called \emph{concrete},
since all constants in the enum will have an associated integer value,
whether explicit or implicit.  An enumerated type that specifies an
\sntx{init-part} for some constants, but not the first is called
\emph{semi-concrete}.  The numeric conversions which are
automatically supported for an enumerated type vary depending on
whether it is abstract, concrete, or semi-concrete
