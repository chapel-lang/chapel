...
There are several variants of the type for a \chpl{class C}:

\begin{itemize}
  \item \chpl{C} is equivalent to \chpl{borrowed C}
  \item \chpl{borrowed C} refers to a class instance that has a lifetime
    managed by another variable.
  \item \chpl{unmanaged C} the instance must have \chpl{delete} called on it explicitly.
  \item \chpl{owned C} the instance be deleted
  ...
  \item \chpl{shared C} will be deleted when all of the \chpl{shared}
