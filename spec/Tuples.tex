...
var x = 1, y = 2;
\end{chapelpre}
Given two variables of the same type, x and y, the statement
\begin{chapel}
(x, y) = (y, x);
\end{chapel}
assigns \chpl{y} to \chpl{x} and then \chpl{x} (already
assigned \chpl{y}) to \chpl{y}.
\begin{chapelfuture}
bug: order of tuple destructuring
\end{chapelfuture}
\begin{chapelpost}
writeln((x, y));
\end{chapelpost}
