...
to a scalar value \chpl{s} and a tuple \chpl{t}, the result is a tuple
\chpl{r} with each element \chpl{r(i)} defined by applying the operation
to \chpl{s} and \chpl{r(i)}.
...
Relational operators are defined in ~(\rsec{Tuple_Relational_Operators}).
The tuples must match each other in shape.

\subsubsection{Tuple Relational Operators}
\label{Tuple_Relational_Operators}
\index{tuples!relational}
...
Formal arguments declarations can be grouped into a tuple similarily to
