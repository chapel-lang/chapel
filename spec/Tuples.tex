...
writeln((x, y));
\end{chapel}
declares two variables \chpl{x} and \chpl{y} initialized to \chpl{1}
and \chpl{2} respectively, swaps the values of \chpl{x} and \chpl{y},
and outputs \chpl{(2, 1)}.  The right-hand side expression is
...
call), the values are squashed.  Line 4 outputs \chpl{3 4 3 4}.
\begin{chapelpost}
\end{chapelpost}
\begin{chapeloutput}
1 2 3 4
...
\end{chapel}
\begin{chapelfuture}
bug: type of tuple grouped formals
\end{chapelfuture}
\begin{chapelpost}
\end{chapelpost}
\begin{chapeloutput}
