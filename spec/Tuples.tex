...
from a procedure, tuples help to support multidimensional indices, to
group arguments to procedures, and to specify mathematical concepts.
...
\begin{chapelexample}{tuple-procedure.chpl}
The statement
\begin{chapel}
var x1 =      ("hello", 3.14),
    x2 = tuple("hello", 3.14),
    x3 = tuple(1);
\end{chapel}
creates two identical tuples: \chpl{x1} and \chpl{x2}; and \chpl{x3}: an integer 1-tuple
 initialized to \chpl{(1)}.
\begin{chapelpost}
writeln((x1,x2,x3));
\end{chapelpost}
\begin{chapeloutput}
...
to use the standard \chpl{tuple} procedure.

Tuple expressions are evaluated similarly to procedure calls where the
arguments are all generic with no explicit intent.  So a tuple
expression containing an array does not copy the array.  

When a tuple is passed as an argument to a procedure, it is passed as
...
\item In procedure calls where multiple formal arguments in a procedure
declaration are grouped using tuple notation.
...
because the tuple is created with array aliases (like a procedure
...
(x, ) = f();
\end{chapel}
defines a procedure that returns a 2-tuple, declares an integer
variable \chpl{x}, calls the procedure, assigns the first component in
...
this case, no variables are defined for the omitted components.
...
var (x, ) = f();
\end{chapel}
defines a procedure that returns a 2-tuple, calls the procedure,
...
\subsection{Splitting a Tuple into Mutiple Indices of a Loop}
...
\subsection{Splitting a Tuple into Multiple Formal Arguments in a Procedure Call}
\label{Formal_Argument_Declarations_in_a_Tuple}
\index{tuples!formal arguments grouped as}

When multiple formal arguments in a procedure declaration are grouped
using tuple notation, the actual expression is split into its
components during a procedure call.  The number of grouped formal
...
in~\rsec{Procedure_Definitions}.

\begin{chapelexample}{formals.chpl}
The procedure
\begin{chapel}
proc p(x: int, (y, z): (int, int)) {
  // body
}
\end{chapel}
is defined to take an integer value and a 2-tuple of integer values.
The 2-tuple is split when the procedure is called into two formals.  A
...
tuple notation, to ensure that the procedure is called with an actual
tuple of the correct size.  Arguments grouped in tuples may be
nested arbitrarily.  Procedures with arguments grouped into tuples may not be
...
The procedure \chpl{p} defined as
\begin{chapel}
proc p((x, (y, z))) {
  writeln((x, y, z));
}
\end{chapel}
is equivalent to the procedure \chpl{q} defined as
\begin{chapel}
proc q(t) where isTuple(t) && t.size == 2 && isTuple(t(2)) && t(2).size == 2 {
...
The following code defines two procedures, a procedure \chpl{first} that
returns the first component of a tuple and a procedure \chpl{rest} that
...
\section{Predefined Procedures on Tuples}
\index{tuples!predefined procedures}
