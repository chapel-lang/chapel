...
support data parallelism implicitly (whole-array assignment, procedure
...
A procedure that expects one or more scalar argument but is called with
...
scalar type of the argument.  The rules of when an overloaded procedure
is promoted are discussed in~\rsec{Procedure_Resolution}.

If a promoted procedure returns a value, the promoted procedure becomes
an iterator that is controlled by a loop over the iterator (or array,
domain, or range) that it is promoted by.  If the procedure does not
return a value, the procedure is controlled by a loop over the iterator
that it is promoted by, but the promotion does not become an iterator.

In addition to scalar promotion of procedures, operators and casts are
also promoted.
...
the \chpl{square} procedure over the values in the array \chpl{A}.  The
...
Consider a procedure \chpl{p} with formal
...
Given a procedure defined as
\begin{chapel}
proc foo(i: int, j: int) {
  return (i,j);
}
\end{chapel}
and a call to this procedure written
\begin{chapel}
writeln(foo(1..3, 4..6));
\end{chapel}
then the output is
...
If the procedure \chpl{p} were called by using square brackets instead
...
a forall expression making calls to a procedure \chpl{foo()}, storing
