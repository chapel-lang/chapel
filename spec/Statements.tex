...
When the assignment operator is \chpl{=}, there is
an implicit conversion~(\rsec{Implicit_Conversions})
of the right-hand side expression
to the type of the left-hand side expression.
The left-hand side expression is updated with the result
of this conversion.

Additionally, the validity and semantics of assigning between
...
are \emph{compound assignment} operators.
...
As with \chpl{=}, the left-hand side expression is evaluated only once,
...
%REVIEW: vass: this is inelegant, consider removing.
% Plus should be defined on int(8) to produce an int(8) instead of this.
In a compound assignment, an explicit cast to the type on the left-hand side is
...
%
% sungeun: Will we allow square brackets also?
%
If the iteratable-expression is a tuple delimited by parentheses, the
components of the tuple must support zipper iteration.
...
\begin{chapelexample}{use.chpl}
  ...
  symbols declared in \chpl{M1}'s global scope (like procedure
