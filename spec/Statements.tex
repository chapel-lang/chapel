...
\begin{itemize}
\item return statement \rsec{The_Return_Statement}
\item yield statement \rsec{The_Yield_Statement}
\item module declaration \rsec{Modules}
\item non-iterator procedure declaration \rsec{Procedure_Definitions}
\item iterator declaration \rsec{Iterator_Definitions}
...
\item forall statement \rsec{Data_Parallelism}
\end{itemize}
...
is already false when it is entered.
\begin{chapel}
var t = 11;
...
\begin{chapelexample}{tensor.chpl}
The output of
\begin{chapel}
for (i, j) in [1..3, 4..6] do
  write(i, " ", j, " ");
...
The following example illustrates how the use statement brings in the definition
of the \chpl{Timer} class, so it can be used to time program execution.
\begin{chapel}
use Time;
...
\begin{chapeloutput}
Write one line.
Non-zero time for writing one line.
\end{chapeloutput}
\end{chapelexample}
...
using the same mechanisms used to disambiguate procedure definitions.
...
procedure resolution mechanism.  The select expressions are the actual
arguments, the types following the \chpl{when} keywords are the types
of the formal arguments for different anonymous procedures.  The
procedure that would be selected by procedure resolution determines the
statement that is executed.  If none of the procedures are chosen, the
the statement associated with the keyword \chpl{otherwise}, if it
exists, will be selected.

As with procedure resolution, this can result in an ambiguous
situation.  Unlike with procedure resolution, in the event of an
