...
A \chpl{defer} statement causes its body to be run when exiting a
function. \chpl{defer} is useful because the deferred statements will
be run no matter how the function returns. At a given place where a
function returns, the in-scope \chpl{defer} and local variables
that need to be de-initialized will be handled in reverse declaration
order. The contents of each \chpl{defer} block will be run and any
local variables of record type will be deinitialized.
