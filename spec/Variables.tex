...
multiple variables that share a \sntx{initialization-part}
...
\end{chapel}
Note that the \chpl{readXX} function cannot be called directly by a
...
compile-time, \chpl{rank} can be set via to \chpl{3} or \chpl{2*n}
(assuming \chpl{n} is also a param variable) or any other expression
