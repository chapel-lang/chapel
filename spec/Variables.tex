...
to a default value described in~\rsec{Default_Values_For_Types}.
...
the procedure \chpl{readXX} on the first variable as in
\begin{chapel}
var v1: t = e; var v2: v1.type = readXX(v1); var v3: v1.type = readXX(v1);
\end{chapel}
where the procedure \chpl{readXX} is defined as follows:
\begin{chapel}
proc readXX(x: sync) return x.readXX();
proc readXX(x: single) return x.readXX();
proc readXX(x) return x;
\end{chapel}
Note that the non-method procedure \chpl{readXX}
is defined here solely for the purposes of this specification.
It cannot be called directly by a
Chapel programmer; adding an additional overload or shadowing
this procedure will not change the semantics of variable declarations.
\end{itemize}
...
procedure or block within that module are global variables.  Global
...
block statements and procedures).
...
\item
 Applications of the string concatenation operator \verb@+@, string comparison operators \verb@==@, \verb@!=@, \verb@<=@, \verb@>=@, \verb@<@, \verb@>@, and the string length and ascii procedures on parameter string expressions.
\item
 The conditional expression where the condition is a parameter and the
 then- and else-expressions are parameters.
\item
 Call expressions of parameter procedures.  See~\rsec{Parameter_Procedures}.
\end{itemize}
