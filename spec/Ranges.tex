...
The type of the step must be a signed integer of the same
bit size as the base range's \chpl{idxType}, or an implicit conversion must be allowed
to that type from the step's type.
The type of the step is also allowed to be
the base range's \chpl{idxType} itself.
It is an error for the step to be zero.

\begin{future}
We consider allowing the step to be of any integer type,
