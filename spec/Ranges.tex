...
named \chpl{stridable}.  If this parameter is set to \chpl{true}, the range's
stride can take on any signed integer value other than 0 of the same
bit-width as \chpl{idxType}.  If \chpl{stridable} is set to \chpl{false}, the range's stride is
...
is 2 or greater and its \chpl{aligned} flag is \chpl{false}, then its alignment is
...
is \chpl{false} unless modified using the \chpl{by} operator as described 
in~\rsec{By_Operator_For_Ranges}.
...
They are described in \rsec{Predefined_Range_Procedures}.
...
\item Otherwise, if the range has a \chpl{first} value (i.e. 
\chpl{r.hasFirst() == true}), then that value is used and \chpl{aligned} is set
to true.
\end{itemize}
...
\section{Predefined Procedures on Ranges}
\label{Predefined_Range_Procedures}
\index{ranges!predefined procedures}
...
\end{protohead}
\begin{protobody}
Returns \chpl{false} if either of the two ranges is ambiguously aligned.
...
\end{protohead}
\begin{protobody}
Returns \chpl{false} if this range is ambiguously aligned.
...
aligned with \chpl{start + n}.  Sets the \chpl{aligned} flag to \chpl{true}.  
If the range has no start value (as determined
by the \chpl{hasStart} predicate), then a run-time error is generated.
\end{protobody}
