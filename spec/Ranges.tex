...
%
  ...
  is of the signed integer type that has the same bit size as \emph{idxType}.
...
% gotta specify syntax for 'range-type' because it's refererenced in Types.tex
...
The \chpl{by} operator takes a range and a stride
and selects a subsequence of the range's represented sequence,
...
The type of the stride argument must be a signed integer of the same
bit size as \chpl{idxType}, or an implicit conversion must be allowed
to that type from the stride argument's type.

The type of the result of the \chpl{by} operator is the type of its range
argument, but with the \chpl{stridable} parameter set to \chpl{true}.

The result of the \chpl{by} operator is as follows:

\begin{itemize}

\item The low and upper bounds are the same as those of the range argument.

\item The stride is a product of the stride argument and
