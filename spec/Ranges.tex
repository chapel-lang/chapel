...
sets, depending on its alignment.  For a given alignment $a$, each index $i$
...
a \chpl{boundedNone} range.  If a bound is given, the index type is determined
by that bound.
...
range containing the specified number of indices.
...
the initial range.
It is an error for \emph{count} to be larger than the length of the range.

When the count is positive, indices are taken from the start of the range;
when the count is negative, indices are taken from the end of the range.  
The order of the indices in the resulting range is determined by the sign of
the input range's sride.
In other words, if the count is positive, the first index in the 
range \chpl{r \# c} is \chpl{r.first} and the stride is added to generate
successive indices.  If the count is negative, the first index is chosen such
that the last index coincides with \chpl{r.last}.  Depending on the indices
selected, the high or low bound is unchanged and the other bound is adjusted so
that it is $c * stride - 1$ units away.
...
has no starting index.  Likewise, it is an error to apply the count operator
