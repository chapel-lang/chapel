...
The sequence is increasing if $stride > 0$ and decreasing if $stride < 0$.
...
%REVIEW: vass: If needed, call such ranges "degenerate" instead of "empty".
...
\index{ranges!ambiguously aligned}
...
\item \chpl{idxType} is the type of the range's low bound and high bound
  (when they are not infinity) and alignment. \chpl{idxType} must be an
  integral type and is \chpl{int} by default.
  The type of the range's stride is the signed integer
  of the same bit size as \emph{idxType}.

\index{ranges!boundedType}
\item \chpl{boundedType} indicates which of the range's bounds are not infinity.
...
%REVIEW: vass: while the following is arguably trivial,
% I feel it is useful to remind the reader about it.
...
% (As well as the ``is summarized as follows'' phrase.)
% This presentation is an almost-copy of rectangular-domain-type.
...
\chpl{idxType}, \chpl{boundedType}, and \chpl{stridable} to be specified
...
\item The low bound is given by the expression to the left of \chpl{..},
if present, and is negative infinity otherwise.

\item The high bound is given by the expression to the right of \chpl{..},
...
%REVIEW: clarify what happens when (a) the bounds' types are different, or
% (b) both bounds are infinity.
\item \chpl{idxType} is determined by the types of the
low and high bound expressions.
It is either \chpl{int}, \chpl{uint},
\chpl{int(64)}, or \chpl{uint(64)}.
It is obtained by
applying integral promotions to the low and high bounds.

\item \chpl{boundedType} is chosen to reflect
which of the bounds are not infinity (\rsec{Range_Types}).

\item \chpl{stridable} is \chpl{false}.

\end{itemize}

\subsection{Range Bounds in Zippered Iterations}

%REVIEW: vass: this might not be the best place for the following.
% I created the subsection to separate this from the surrounding matter.
% The right place is perhaps in Concepts, but there we cannot give
% a code example. (Exemplifying this without Chapel code would not be fun.)
...
\begin{itemize}
\item \chpl{idxType} of the source range type is implicitly coercible
...
The \chpl{by} operator can be applied to any range to select
...
The expression following \chpl{by} is the \chpl{by} operator's stride argument.
It must be an integer that can be coersed to a signed integer value of the same
bit size as \chpl{idxType}.  

The type of the resulting range has the same
\chpl{idxType} and \chpl{boundedType} as the
argument type, but has its \chpl{stridable} parameter set to \chpl{true}.
...
%REVIEW: the alignment is defined using two "as follows" lists.
% Should be just one.
...
%REVIEW: vass: need to define what it means for a range to be "aligned" on smth
% Better yet, rephrase it to use already-defined terminology.
\begin{itemize}
...
%REVIEW: bradc: Is this right?
%REVIEW: hilde: I think so ... .  It expresses the "poison bit" interpretation of
...
%REVIEW: vass: need to rephrase (the resulting range cannot have
% a first value before its alignment is defined).
% Also, remove references to hasFirst (replace with e.g. "has first index").
...
\chpl{r.hasFirst() == true}), then \chpl{first} is used.
\end{itemize}
See the definition of \chpl{hasFirst}.
...
%REVIEW: bradc: We should really define to use op. range's idxType.
