...
named \emph{eltType}.  This must be one of the \chpl{int} or
...
stride can take on any \chpl{int} value except 0.  If set to false,
the range's stride is fixed to 1.  The default value is \chpl{false}.
...
The \chpl{by} operator takes a range and an integer to yield a new range that
