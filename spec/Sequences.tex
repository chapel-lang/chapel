...
name \chpl{elt_type}.  If \chpl{s} is a sequence, then the
type \chpl{s.elt_type} refers to type the type of its element.
...
the \chpl{length} and \chpl{reverse} methods and
the \chpl{spread}, \chpl{transpose}, and \chpl{reshape} functions.

\subsubsection{The {\em length} Method}
\label{The_em_length_Method}
\index{length@\chpl{length} method!on sequences}
...
\index{reverse@\chpl{reverse} method}
...
\index{spread@\chpl{spread} function}
...
\index{transpose@\chpl{transpose} function}
...
\index{reshape@\chpl{reshape} function}
...
create a strided arithmetic sequence.  The syntax for the \chpl{by}
operator is given by:
\begin{syntax}
by-expression:
  arithmetic-sequence-expression `by' integral-expression
\end{syntax}
