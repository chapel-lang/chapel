...
\begin{syntax}
method-declaration-statement:
  `proc' param-clause[OPT] type-binding function-name argument-list[OPT] return-intent[OPT] return-type[OPT]
    where-clause[OPT] function-body
  `iter' param-clause[OPT] type-binding function-name argument-list[OPT] return-intent[OPT] return-type[OPT]
    where-clause[OPT] function-body
...
each of which has a default value equal to the field's initializer (if present) or default value (if not).
