...
A variable of any class type is default-initialized to \chpl{nil}.  
...
The default initializer for a class first calls the default initializer for each class and record from which the
...
A constructor for a given class is called by placing the \chpl{new} operator in front of the type
...
If the user does not specify any constructors in a class definition, the
...
the \chpl{new} operator) only when the class does not have any user-defined constructors.
