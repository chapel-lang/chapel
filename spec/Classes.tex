...
All field accesses are resolved via getters that are methods with a \chpl{ref}
return intent (\rsec{Ref_Return_Intent}) defined in the class with the same
name as the field.  The default getter is defined to simply return the field if
the user does not redefine it.
