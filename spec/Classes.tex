...
The optional \sntx{this-intent} declares how the receiver argument is
constrained or will be passed for a given method call.

When \sntx{this-intent} is \chpl{param}, it specifies that the
function can only be applied to param objects of the given type
binding.  When \sntx{this-intent} is \chpl{type}, it specifies that
the function is a \emph{static method} (in the C++ sense) and can only
be called on the type itself rather than an instance of the type.
...
passed by reference, allowing modification of \chpl{this}.  If
no \sntx{this-intent} is specified, the receiver will be passed with
the blank intent as specified in \rsec{The_Blank_Intent}.
