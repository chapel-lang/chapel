...
The \chpl{new} operator allocates memory for the desired class and invokes
...
If the program declares an initializer method on a class, it is a user-defined
...
\chpl{init} and there must not be a return type specifier.

When an initializer is called, the usual function resolution mechanism
(\rsec{Function_Resolution}) is applied with the exception that an
initializer may not be virtually dispatched.

Initializers for generic classes (\rsec{Generic_Types}) handle certain
...
The second initializer initializes field \chpl{z} before field \chpl{y}, so a
compile-time error will be issued. Without this rule the explicit
initialization of field \chpl{y} would be assignment, which could be confusing
and surprising (especially for more complicated types than \chpl{real}).
\end{chapelexample}
...
Once the class is initialized, methods may be called and \chpl{this} may be
...
This ambiguity is resolved by calling a built-in method named \chpl{complete}.
...
and methods may be used after a call to \chpl{init}.
...
\begin{chapelexample}{conditionalInit.chpl}
In the following code:
\begin{chapel}
class Parent {
  var x, y : real;
...
\end{chapeloutput}
there are no user-defined initializers for \chpl{C}, so \chpl{new} operators
...
may not be accessed before explicit calls to \chpl{init}.
...
once super.init() returns. The dynamic type of \chpl{this} is the parent's
type until the \chpl{complete} method is invoked (except when local fields
are initialized and used). This means that before the \chpl{complete} method
is invoked, parent methods may be called on \chpl{this} and \chpl{this} may be
...
\chpl{foo}. After the \chpl{complete} method is invoked, a call to \chpl{foo}
