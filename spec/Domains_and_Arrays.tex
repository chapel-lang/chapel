...
This excludes ranges, domains, and arrays.  Domains have a rank and a
total order on their elements.  An {\em array} is generically a
function that maps from a {\em domain} to a collection of variables.
...
some type that is not an array, domain, or range.  Indefinite domains
...
index set.  The type of an indefinite domain is parameterized by the
...
arithmetic domain (the number of ranges that define it) and by an
underlying integeral type called the {\em dimensional index type}
which must be identical to each of the integral element types of the
ranges that define the arithmetic domain.  By default, the dimensional
index type of an arithmetic domain is \chpl{int}.
...
arithmetic domain is said to be strided.
...
Section~\rsec{Subdomains}.  The indices described by each range must
...
domains and arrays.  One could conceive of wanting sparse indefinite
...
sometimes called the {\em base domain} or \emph{parent domain}.
...
An enumerated domain is specified indentically to the indefinite
...
by a {\em base domain}.  A subdomain is specified by the following
syntax:
\begin{syntax}
subdomain-type:
