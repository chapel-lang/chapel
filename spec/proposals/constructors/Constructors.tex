...
(see \rsec{Default_Values_For_Types}).\footnote{There is a recommendation from
...
moved earlier in the \sntx{initialization-clause}, so that it precedes the first
...
important semantic meaning to the gap between the \sntx{initialization-clause}
...
permit the grammar for \sntx{initialization} expressions to be specified as
a subset of the most general \sntx{expression} nonterminal.  If so, then the
...
In any case, the restriction against calling methods on \chpl{this} before it is
...
types where appropriate.  The function \chpl{chpl__autoCopy} is redefined to increment
...
\chpl{chpl__convertRuntimeTypeToValue} controls the exact type of the array being
