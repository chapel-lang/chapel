...
declaration --- including \sntx{linkage-} \sntx{specifier}, \sntx{type-binding}
...
appears within the \sntx{constructor-body}, it must be the form that does not
...
\end{syntax}
\noindent
The \sntx{constructor-call} resembles and behaves like a function invocation.
If the \sntx{constructor-name} is omitted, the default constructor
name \chpl{__init__} is used.  Constructor invocations do not have a
...
initializers (\rsec{Default_Initializers}) for any fields which have not yet
...
execution before an attempt to call a method on \chpl{this}.  That permits such
...
perform initialization.  The portion of the \sntx{constructor-body}
...
The alternative --- using a \chpl{constructor} (or equivalent) keyword ---
...
stereotyped name (for example \chpl{__init__}) which is implied if just
