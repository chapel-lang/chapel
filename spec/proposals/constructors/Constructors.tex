...
automatically invoked in a \chpl{new} expression, thus providing a hook whereby
...
arbitrarily.  But there is also an implicit contructor name \chpl{__init__},
which is used in places where the constructor name can be and is omitted.

Constructors may be invoked in two ways: in the context of a \chpl{new} operator or as
a method.  A \chpl{new} expression allocates memory sufficient to hold the
object being constructed.  This uninitialized object is passed as the receiver
argument to the contructor named in the \chpl{new} expression.  That constructor
...
declaration --- including \sntx{linkage-specifier}, \sntx{type-binding}
and \sntx{where-clause} ---  have the same syntax as for a method declaration.
...
the user's class invariants.  These two phases are separated by the \chpl{init}
...
\begin{itemize}
\item Method calls involving \chpl{this} (i.e. the object being constructed) are not permitted.  
...
initializers\rsec{Default_Initializers} for any fields which have not yet been initialized
explicitly.  
...
analysis whether an \chpl{init} statement has been encountered along the current
branch of execution before an attempt to call a method on \chpl{this}.  That
...
names from argument names.  However, prior to a \chpl{init} statement, 
...
perform guaranteed initialization.  The portion of the \sntx{constructor-body}
following an \chpl{init} statement can perform common post-initialization steps.
(Those steps \emph{could} be performed at the call site if the \chpl{init}
...
the \chpl{init} statement --- not merely at the end.
\end{rationale}
...
stereotyped name (for example \chpl{__init__}) which would be implied if just
the class name is supplied in a constructor call.  Thus \chpl{var myFoo = new
...
and otherwise by the type-dependent default value.\rsec{Default_Initializers}  The arguments appear in the
...
in front of each \chpl{init} statement.  (If no \chpl{init} statement appears in
...
the \chpl{init} statment is inserted after the empty or simple body.)
...
When the \chpl{new} operator is present, a new object
is created: Space is
...
without the \chpl{new} operator, the constructor method is simply called with
...
\end{chapel}
\noindent
using \chpl{new} to create an initialize an object
\begin{chapel}
var c = new C(3);
\end{chapel}
\noindent 
...
Whether or not the \chpl{new} operator is used, the default constructor
name \chpl{__init__} may be omitted.  Thus the above base-class constructor call
can also be expressed as \chpl{super(_c:int, _d:int);}.

\begin{openissue}
Assuming that a constructor can be called like a normal method,
zero-initialization of the memory allocated by the \chpl{new} operator becomes
...
the \chpl{constructor} keyword and the \chpl{init} statement.
