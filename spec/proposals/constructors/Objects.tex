...
\chpl{nil} is not generated and assignmed to the corresponding variable.  In spite of
...
The autoCopy to initialize the \chpl{type_tmp} is arguably correct, although we have
argued that a constructor call could appear instead, to initialize the \chpl{type_tmp} directly.
...
attempt is made to reclaim resource initially held by the \chpl{type_tmp}.  Ownership of
...
and the autoCopy and assignments both avoided.  The final copy of the \chpl{type_tmp} back
...
\chpl{b} are the same.  The case for \chpl{c} also contains an initailizer that
...
The default-initialization code for a class object containing two records of the above type
looks like:
\begin{chapelcode}
    var this:C
