...
\hline
\chpl{+} & \multirow{2}{*}{left} & addition \\
\chpl{-} & & subtraction \\
\hline
\chpl{..} & left & range initialization \\
\hline
\chpl{<=} & \multirow{4}{*}{left} & less-than-or-equal-to comparison \\
\chpl{>=} & & greater-than-or-equal-to comparison \\
\chpl{<} & & less-than comparison \\
\chpl{>} & & greater-than comparison \\
\hline
\chpl{==} & \multirow{2}{*}{left} & equal-to comparison \\
...
\hline
\chpl{by} & \multirow{3}{*}{left} & range/domain stride application \\
\chpl{#} & & range count application \\
\chpl{align} & & range alignment \\
\hline
\chpl{in} & left & forall expression \\
\hline
\chpl{if then else} & \multirow{5}{*}{left} & conditional expression \\
\chpl{forall do} & & forall expression \\
\chpl{[ ]} & & forall expression \\
\chpl{for do} & & for expression \\
\chpl{sync single atomic} & & sync, single and atomic type \\
\hline
...
\chpl{a \!= b} is equivalent to \chpl{\!(a == b)}.
...
\end{verbatim}
\end{chapeloutput}
will cause \chpl{z} to be a new string containing the value
\chpl{"hi there"}.
\end{chapelexample}
