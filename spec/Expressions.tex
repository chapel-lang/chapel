...
\index{enumeration constant expression}
...
\index{parenthesized expressions}
...
\index{call expressions}
\index{expressions!call}
\index{procedure calls}

Procedures and procedure calls are defined in~\rsec{Procedures}.
...
The following code defines a generic procedure where the type of the
...
procedure can be rewritten without query expression as follows:
\begin{chapelnoprint}
{ // }
\end{chapelnoprint}
\begin{chapel}
...
that can be passed to a formal argument of a procedure that
has \chpl{out} or \chpl{inout} intent, or that can be returned by a
variable procedure.  Valid lvalue expressions include the following:
\begin{itemize}
\item
...
\item
 Call expressions of variable procedures.
\item
...
\verb@()@ & & procedure call or access (zipper) \\
\verb@[]@ & & procedure call or access (tensor) \\
\hline
...
\index{operators!operator expressions}
...
overloading~(\rsec{Procedure_Overloading}).

For each operator, implicit conversions are applied to the operands of
an operator such that they are compatible with one of the procedure
...
The procedure \chpl{isTrue} is predefined over bool type as follows:
\begin{chapel}
proc isTrue(a:bool) return a;
\end{chapel}
Overloading the logical and operator over other types is accomplished
by overloading the \chpl{isTrue} procedure over other types.
...
The procedure \chpl{isTrue} is predefined over bool type as described
...
\index{operators!ordered comparison}
...
expression does not require an else clause.  Such a conditional
expression filters the iterations of the for expression.
