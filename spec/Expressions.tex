...
explicit conversion:
\begin{itemize}
\item \index{tuples!cast to sequences}
...
\item \index{sequences!cast to tuples}
...
\item \index{strings!format string casts}
...
function that has \chpl{out} or \chpl{inout} intent, or returned by a
...
The value of \chpl{b} must be at least zero and at most the number of
bits used to represent \chpl{a}.
...
The methods \chpl{false?} and \chpl{true?} are predefined over bool
type as follows:
\begin{chapel}
...
by overloading the \chpl{false?} and \chpl{true?} methods over other
types.
...
The methods \chpl{false?} and \chpl{true?} are predefined over bool
type as described in~\rsec{Logical_And_Operators}.  Overloading the
logical or operator over other types is accomplished by overloading
the \chpl{false?} and \chpl{true?} methods over other types.
...
types.  It is described in~\rsec{Sequence_Concatenation}.
...
The operator \chpl{by} is predefined on arithmetic sequences.  It is
described in~\rsec{Strided_Arithmetic_Sequences}.
