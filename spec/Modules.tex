...
one or more modules.  Every symbol, including variables, procedures,
...
procedure call restricts the set of candidate procedures to those in the
specified module.
...
statements in a module other than procedure and type declarations are
...
the main procedure~(\rsec{The_main_Procedure}).

\subsection{The {\em main} Procedure}
\label{The_main_Procedure}

\index{main@\chpl{main}}
The main procedure must be called \chpl{main} and must have zero
arguments.  It can be specified with or without parentheses.  In any
Chapel program, there is a single main procedure that defines the
...
disambiguates between main procedures in multiple modules.

\begin{craychapel}
In the Cray Chapel compiler implementation, the \emph{--
--main-module} flag can be used to specify the module from which the
main procedure definition will be used.  Only modules passed to the
compiler on the command line will be searched for main procedures
...
M2's main
\end{chapelprintoutput}
Notice that main is treated like just another procedure if it is not in
...
not define a main procedure.  In this case, a default main procedure is
...
hello, world
\end{chapeloutput}
is a legal and complete Chapel program.  Its initialization procedure,
which also serves as the program's main procedure, is the top-level
...
Starting from the module that defines the main procedure, modules are
