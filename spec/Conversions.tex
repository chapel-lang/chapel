...
\item From \chpl{real($i$)} to \chpl{real($j$)}, \chpl{complex($k$)}, or \chpl{string} where $j > i$ and $k \geq 2i$
\item From \chpl{imag($i$)} to \chpl{imag($j$)}, \chpl{complex($k$)}, or \chpl{string} where $j > i$ and $k \geq 2i$
