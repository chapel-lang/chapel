...
\index{conversions!source expression}
...
\index{conversions!implicit!may occur at}
...
\item The actual argument of a function call or an operator may be converted
      ...
      intent is \chpl{in} or the blank intent with the semantics of
      \chpl{in}.
...
\item As a special case, the condition of a conditional expression,
      ...
      may be converted to the boolean type~(\rsec{Implicit_Compile_Time_Constant_Conversions}).

%VASS - move to "types"
\item As a special case, a parameter expression of an integral type
      may be converted to a certain different integral type~(\rsec{Implicit_Compile_Time_Constant_Conversions}).
...
\index{conversions!implicit!bool}
...
\begin{rationale}
We allow these implicit conversions because they are an important
...
A boolean cannot be converted to \chpl{int(1)} (if this type is
available) because 1 cannot be represented by \chpl{int(1)}.
A boolean cannot be converted to a real, imaginary, or complex type.
...
If a conversion is actually needed, a cast \rsec{Explicit_Conversions}
can be inserted.
\end{rationale}

Implicit numeric conversions may thus be tabulated as follows:
% VASS: coming up...Conversions.tp1
\begin{center}
...
\index{conversions!implicit!enumeration}
...
\index{conversions!bool!in a statement}
\index{conversions!implicit!bool}
...
\index{conversions!record}
\index{conversions!explicit!record}
