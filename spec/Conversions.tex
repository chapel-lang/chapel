...
conversions are supported between the enum's constants and any numeric
type or \chpl{bool}, including \chpl{param} conversions.  For a
semi-concrete enumerated type, if a conversion is attempted involving
a constant with no underlying integer value, it will generate a
compile-time error for a \chpl{param} conversion or an execution-time
error otherwise.
