...
The locale associated with expressions that are lvalues or of class or
locale type can be queried using the following syntax:
\begin{syntax}
locale-access-expression:
  locale-access-sub-expression . `locale'

locale-access-sub-expression:
  lvalue-expression
  class-expression
  locale-expression
...
locale-expression:
  expression
\end{syntax}
When the left operand to the locale query is an lvalue, the access
returns the locale on which that lvalue exists.  When it is of class
type (whether or not it is also an lvalue), the access returns the
locale on which the class object exists.  When it is of locale type,
the access returns the expression itself.
...
2
\end{chapel}
The variable \chpl{x} is a value.  It is declared and exists
on \chpl{Locales(0)}.  The variable \chpl{c} is a reference.  The
reference exists on \chpl{Locales(0)} but the object itself exists
...
\begin{syntax}
on-statement:
  `on' locale-access-sub-expression `do' statement
  `on' locale-access-sub-expression block-level-statement
\end{syntax}
The expression must be a legal expression for accessing a locale.
See~\rsec{Querying_the_Locale_of_a_Variable} for details.  Execution
of the statement occurs on the locale given by
the \sntx{locale-access-sub-expression}.  Execution continues after
...
can be changed by using the \chpl{on} keyword.
...
remote-variable-declaration-statement:
  `on' locale-access-sub-expression variable-declaration-statement
\end{syntax}
