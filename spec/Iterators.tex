\sekshun{Iterators}
\label{Iterators}
\index{procedures!iterators}
\index{iterators}

An \emph{iterator} is a procedure that can generate, or \emph{yield}, multiple values (consecutively or in parallel) via its yield statements.
...
\label{Iterator_Definitions}
\index{iterator!declarations}
...
The syntax of an iterator declaration is similar to a non-iterator procedure declaration, with
...
All details of iterator calls, including argument passing, procedure
resolution, the use of parentheses versus brackets to delimit the parameter
list, and so on,
are identical to non-iterator procedure calls as described in Chapter~\ref{Procedures}.
...
If an iterator procedure is captured into a new variable declaration or
...
An iterator call expression can be passed to a generic procedure argument that
...
Within the generic procedure the corresponding formal argument
can be used as an iterator, e.g. in for loops.
The arguments to the iterator call expression, if any, are evaluated
at the call site, i.e. prior to passing the iterator to the generic procedure.
