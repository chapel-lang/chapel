...
loop. An iterator function call, or iterator invocation, can be captured in a variable declaration or assigned to an array, in which
case it evaluates to an array of values. Iterator invocations can also be passed to a
...
An iterator call expression can be passed to a function argument that
is generic by being declared without a type and a default value
...
unless it is desired to aggregate its outcomes into an array
(\rsec{Iterators_as_Arrays}).

\begin{example}
A post-order traversal of a tree data structure could look like this:
\begin{chapel}
