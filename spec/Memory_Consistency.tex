...
tasks:
\todo{link to chapter 24 and 25 as the chapters that describe these features?}
...
Also note that \chpl{sync/single} variables will have memory consistency behavior
...
consistency, a operations on a \chpl{sync/single} variable will have the same
consistency implications as if an \chpl{atomic} variable were used with
SC as a lock to protect modifications of the \chpl{sync/single}
...
\todo{Don't count on data structures being safe for parallel update unless the data structure is designed to do so}
...
\todo{Include a version that spin waits on an atomic, and then explain why waitFor is better ( it ncludes the appropriate yelds/network servicing. Generally speaking, it's a bad idea to put a tight spin-loop in a cooperatively scheduled environment.)}
