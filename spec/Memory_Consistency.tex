...
\todo{should we use "legal" and "illegal" instead?} incorrect programs with
data races.

The \chpl{forall}, \chpl{coforall}, \chpl{cobegin}, \chpl{begin}, as well as
the \todo{is there a more formal name for square-bracket and promotion?}
square-bracket \chpl{forall} notation and promoted functions or operators can
...
Chapel has several \todo{maybe concepts or constructs instead of features?}
features that can be used to correctly \todo{link to chapter 24?}synchronize
tasks:
...
1. Sequential programs have \todo{use before def} program order semantics.
...
atomic operations.
\todo{Michael thinks this is too pessimistic and doesn't describe why
      they are problematic... mabye "should be reserved for advanced use"?}
...
no particular relationship between $Y$ and $Z$ in program order.
  ...
  \item \todo{use of "wait" before mention of translation in next paragraph} The program \chpl{t = begin\{Y\}; wait(t); Z;} implies $Y$ $<_p$ $Z$
...
is executing.
\todo{This paragraph gets a little wordy/repetitive. That said, I think
emphasis on this is good so I wouldn't go too far in the other direction.}
...
\todo{change "sync" to "sync/single"? Also this paragraph is pretty hard to
read. Is the intent just to say that sync/single vars always have SC
consistency and do not provide other consistencies? And so they are just like
SC atomic operations and don't need to discuss further?}
...
\todo{Where to we describe array/domain consistency?}
...
Some features of the language allow you to request \todo{are "weaker" and "more
relaxed" synonyms or should we try to unify on one wording?} weaker than SC
...
\begin{itemize}
 ...
 operations \todo{above where?}(Rule 3 above).
...
\todo{Why introduce waitFor(), instead of using the direct translation to
atomic vars?}
\todo{because waitFor includes the appropriate yelds/network servicing. Generally speaking, it's a bad idea to put a tight spin-loop in a cooperatively scheduled environment.}
