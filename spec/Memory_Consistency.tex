...
\begin{itemize}
  ...
  memory\_order\_seq\_cst operations.
...
\begin{itemize}
  ...
  \item a \textit{task} in Chapel corresponds to a \textit{thread} for the
  purposes of the memory model
  ...
  \item Chapel's \chpl{sync} variables are similar to locks in the C++11 memory
  model.
...
be avoided. (with the exception that \chpl{memory\_order\_relaxed} can
be used with fences).
