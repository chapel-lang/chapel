...
operations was strongly influenced by the C11 standard. The main
difference is that Chapel also supports atomic operations for real
variables. There are a few additional methods (waitFor, peek, and
poke) and a few methods not currently implemented (isLockFree,
threadFence, and signalFence.)
...
Please refer to \chpl{CHPL_HOME/examples/primers/atomics.chpl} for a
brief introduction to atomic variables. For more information about the
current atomic implementation refer to \newline
\chpl{CHPL_HOME/doc/release/technotes/README.atomics}
...
\end{protohead}
\begin{protobody}
Stores v as the new value and returns the original value. Defined for all
atomic types. 
\end{protobody}
...
\begin{protohead}
proc (atomic t).compareExchangeWeak(e: t, v: t): bool
proc (atomic t).compareExchangeStrong(e: t, v: t): bool
proc (atomic t).compareExchange(e: t, v: t): bool
\end{protohead}
\begin{protobody}
Stores v as the new value, if and only if the original value is equal to
e. Returns true if v was stored, false otherwise. The 'weak' variation
may return false even if the original value was equal to e if, for
example, the value could not be updated atomically. compareExchange() is
equivalent to compareExchangeStrong(). Note that the current
implementation of these methods differs slightly from the C11 standard.
In the C11 standard if the exchange fails the value of the atomic
variable is stored in e but the current Chapel implementation does
not change the value of e on failure.  Defined for all atomic types. 
\end{protobody}
...
\verb@&@, \verb@^@) to the original value and v and stores the result.
All of the methods are defined for integral atomic types. Only add and
sub (\verb@+@, \verb@-@) are defined for real atomic types.  None of the methods are
defined for the bool atomic type. 
\end{protobody}

\begin{future}
In the future we may overload certain operations such as \verb@+=@ to
call the above methods automatically. Currently the only way to change
an atomic variable is through one of the defined methods and operator
overloads are explicitly prohibited. 
\end{future}
...
\begin{protobody}
Applies the appropriate operator (\verb@+@, \verb@-@, \verb@|@,
\verb@&@, \verb@^@) to the original value and v, stores the result, and
returns the original value. All of the methods are defined for integral
atomic types. Only add and sub (\verb@+@, \verb@-@) are defined for real
atomic types.  None of the methods are defined for the bool atomic type. 
\end{protobody}


\index{testAndSet (atomic var)@\chpl{testAndSet} (atomic var)}
\index{predefined functions!testAndSet (atomic var)@\chpl{testAndSet} (atomic var)}
\begin{protohead}
proc (atomic bool).testAndSet(): bool
\end{protohead}
\begin{protobody}
Stores true as the new value and returns the old value. Equivalent to
exchange(true). Only defined for the bool atomic type. 
\end{protobody}

\index{clear (atomic var)@\chpl{clear} (atomic var)}
\index{predefined functions!clear (atomic var)@\chpl{clear} (atomic var)}
\begin{protohead}
proc (atomic bool).clear()
\end{protohead}
\begin{protobody}
Stores false as the new value. Equivalent to write(false). Only defined
...
\end{protohead}
\begin{protobody}
Waits until the stored value is equal to v. The implementation may yield
the running task while waiting. Note that this method is not part of the
C11 standard. Defined for all atomic types. 
\end{protobody}

\begin{future}
There are two additional methods defined for all atomics types: peek and
poke. These methods are similar to read and write respectively. The
difference is intended to be that peek and poke have relaxed memory
constraints for performance reasons. These methods are currently
implemented, but they have identical semantics to read and write. 

Additionally there are three methods in the C11 standard that are not
currently implemented: isLockFree, threadFence, and signalFence.
It is intended that these methods will be available in the future.  
\end{future}

\index{atomic types!memory order}
The above methods (with the exception of peek, poke, and waitFor) also
take an optional argument named 'order' of type memory\_order. The order
argument is used to specify the ordering constraints of atomic
operations. The supported memory\_order types are:
\begin{itemize}
...
The default value is memory\_order\_seq\_cst.  Not all implementations
support all memory\_order types and may default to a more conservative
ordering than specified. See the C11 standard for more information on 
memory orderings. 

\begin{future}
The current implementations do not change with different memory\_orders.
Currently memory\_order\_seq\_cst is used regardless of the actual order
specified. In the future we intend to support the various memory
orderings and will provide more details for their use and meanings. 
\end{future}
...
\begin{openissue}
We consider extending task intents to \chpl{forall} loops
(\rsec{Forall}, \rsec{Forall_Expressions}). This could be seen
as a natural application of the idea in yet another context,
to extend its benefits and for consistency. The details and
semantics of task intents for a \chpl{forall} need to be
worked out. We would like to gain some experience with task intents
for \chpl{begin}, \chpl{cobegin}, \chpl{coforall} before venturing
into \chpl{forall}.
\end{openissue}
