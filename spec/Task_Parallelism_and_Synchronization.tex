...
is \chpl{sync} is ``read'' before being passed to the procedure and the
...
\label{Procedures_on_Synchronization_Variables}
...
is written, thereby ensuring that each of the procedures has finished.
\end{chapelexample}
...
8
\end{chapeloutput}
defines a procedure \chpl{concurrentUpdate} that recursively walks over
a tree and spawns a new task to update a node if the
procedure \chpl{requiresUpdate} evaluates to true.
