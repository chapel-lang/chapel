...
\begin{example}
The following code defines a function \chpl{output} that takes an
actual argument that is a generic stack and outputs the elements in
the stack without any space between the elements.  The function is
generic on the type of its argument.
\begin{chapel}
class MyStack {
  type eltType;
  var value: eltType;
  var next: MyStack(eltType);
}

def output(s: MyStack) {
  write(s.value);
  if s.next then
    output(s.next);
}
...
could also be specified as \chpl{MyStack(?type)} in which case the
symbol \chpl{type} is equivalent to \chpl{s.eltType}.
...
\chpl{eltType} as the only argument to the first type.  Given the
definition of \chpl{Node} in the example
in~\rsec{Type_Aliases_in_Generic_Types}, one can specify the type
\chpl{Node(real)} or \chpl{Node(eltType=real)} by writing \chpl{Node
of real}.
