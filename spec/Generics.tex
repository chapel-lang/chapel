...
Chapel supports generic procedures and types that are parameterizable
over both types and parameters.  The generic procedures and types generally
look similar to non-generic procedures and types already discussed.

\section{Generic Procedures}
\label{Generic_Procedures}
\index{procedures!generic}
\index{generics!procedures}

A procedure is generic if any of the following conditions hold:
\begin{itemize}
\item
...
element type is queried and/or omitted or the domain is queried or
omitted.
\end{itemize}
...
If a formal argument is specified with intent \chpl{type}, then
the corresponding actual argument must be a type.  A copy of the
procedure is instantiated for each unique type that is passed to this
procedure at a call site.  The formal argument has the semantics of a
type alias.
\begin{chapelexample}{build2tuple.chpl}
The following code defines a procedure that takes two types at the call
...
\end{chapel}
The following procedure is instantiated with ``normal'' procedure call syntax
where the actual arguments are types:
\begin{chapel}
...
parameter must be passed to the procedure at the call site.  A copy of
the procedure is instantiated for each unique parameter that is passed
to this procedure at a call site.  The formal argument is a parameter.
\begin{chapelexample}{fillTuple.chpl}
The following code defines a procedure that takes an integer parameter
\chpl{p} at the call site as well as a regular actual argument of
integer type \chpl{x}.  The procedure returns a homogeneous tuple of
...
(3, 3, 3)
\end{chapeloutput}
The procedure call \chpl{fillTuple(3, 3)} returns a 3-tuple where each
...
procedure at the call site.  A copy of the procedure is instantiated for
...
defines a procedure that returns a homogeneous tuple of size \chpl{p}
...
3*real
\end{chapeloutput}
In this procedure, the type of the tuple is taken to be the type of the
...
argument passed to the procedure at the call site.  A copy of the
procedure is instantiated for each unique actual type.  The queried
...
to the procedure at the call site with the constraint that the type of
the actual argument is an instantiation of the generic type.  A copy
of the procedure is instantiated for each unique actual type.
\begin{example}
The following code defines a procedure \chpl{writeTop} that takes an
actual argument that is a generic stack
(see~\rsec{Example_Generic_Stack}) and outputs the top element of the
stack.  The procedure is generic on the type of its argument.
\begin{chapel}
...
default value.  Procedure \chpl{p} is defined to take an argument of
this class type where the type field is instantiated to the default.
Procedure \chpl{q}, on the other hand, is generic on its argument
...
for the purpose of procedure resolution.
\begin{example}
...
is equivalent to
\begin{chapel}
proc p(tuple: (?t,?tt)) where tuple.size == 2 && tt == real {
  // body
}
\end{chapel}
\end{example}
...
Type queries can also be used to constrain the types of other procedure arguments
...
argument is taken to be the type of the actual argument passed to the
procedure at the call site.  If the domain is omitted, the domain of
...
\section{Procedure Visibility in Generic Procedures}
\label{Procedure_Visibility_in_Generic_Procedures}
\index{generics!procedure visibility}

Procedure visibility in generic procedures is altered depending on the
instantiation.  When resolving procedure calls made within generic
procedures, the visible procedures are taken from any call site at which
the generic procedure is instantiated for each particular
instantiation.  The specific call site chosen is arbitrary and it is
referred to as the \emph{point of instantiation}.

For procedure calls that specify the module
...
\begin{chapeloutput}
\end{chapeloutput}
In the procedure \chpl{main}, the variable \chpl{r} is declared to be
of type \chpl{R} defined in module \chpl{M1} and a call is made to the
generic procedure \chpl{bar} which is defined in module \chpl{M2}.
...
procedures from within \chpl{main} and going through the use of
module \chpl{M1}.
\end{chapelexample}

If the generic procedure is only called indirectly through dynamic
...
\begin{rationale}
Visible procedure lookup in Chapel's generic procedures is handled
differently than in C++'s template procedures in that there is no split
...
A type constructor is a type procedure (\rsec{Type_Procedures}) that has
...
The instantiation of a generic procedure can be constrained by {\em
where clauses}.  A where clause is specified in the definition of a
procedure~(\rsec{Procedure_Definitions}).  When a procedure is
...
procedure is not a possible candidate for procedure resolution.
Otherwise, the procedure is instantiated.
\begin{chapelexample}{whereClause.chpl}
Given two overloaded procedure definitions
\begin{chapel}
...
second procedure is instantiated the where clause evaluates to false.
\end{chapelexample}
...
The special compiler diagnostic procedure calls \chpl{compilerError}
and \chpl{compilerWarning} generate compiler diagnostic of the
indicated severity if the procedure containing these calls may be
called when the program is executed and the procedure call is not
...
program from which the procedure containing the call is called.
...
Note that when a variable procedure is called in a context where the
implicit \chpl{setter} argument is true or false, both versions of the
variable procedure are resolved by the compiler.  Consequently,
