...
function \chpl{bar}:
\begin{chapelpre}
% test_point_of_instantiation.chpl
\end{chapelpre}
\begin{chapel}
module M1 {
  record R {
...
instantiated, the generic type is instantiated so that the type
alias is bound to the actual type passed to the constructor.
...
All methods bound to generic classes or records, including constructors, are generic over the
implicit \chpl{this} argument, in addition to any other argument that is generic.

\subsubsection{Generic Constructors}
\label{Generic_Constructors}
\index{generics!constructors}


If a generic field of a class does not have an initialization expression,
each constructor for that class must provide a formal argument whose name
matches the name of the field.
If the name of a formal argument in a constructor matches the name of
a generic field that either does not have an initialization
expression, is a type field, or is a parameter field, the field is
automatically initialized to the actual value for that argument upon
