...
\section{Declaring Specific Instantiations}
\label{Declaring_Specific_Instantiations}
\index{specific instantiations}
\index{generic specialization}

The Chapel language facility supports three mechanisms for creating generic
functions along with concrete functions that work with specific types.
One can use these mechanisms to create specialized versions of generic
functions.

The first mechanism enables generic functions to be specialized.
...
selected in preference to those with a totally generic argument:
...
The second mechanism applies when the generic type to be specialized is a
method receiver. When declaring a secondary method, the receiver type
can be a parenthesized expression. In that case, the compiler will
evaluate the parenthesized expression at compile time in order to find
...
where clause can be used to declare a version of a function that works
with generic argument - but where the argument is constrained in some way.
See also \rsec{Where_Expressions}.
