...
(\rsec{Using_Modules}).  Both automatic and optional standard modules can be
overridden by a user-supplied version having the same name.
See~\rsec{Module_Search_Paths} for details on how
module overriding is accomplished.

Internal modules are always loaded by a chapel program \emph{as if} the
statement \chpl{use ChapelStandard;} has been inserted ahead of the first
...
\index{optional modules!UtilMath}
...
\section{Modules Search Paths}
\label{Module_Search_Paths}
\index{modules!overriding}
\index{modules!search paths}

The compiler uses four module search paths: one for internal modules, one for
standard modules and one for user modules.  The user path first includes all the
paths used to specify module files directly on the \chpl{chpl} command line and
then includes the paths specified via \chpl{-M} commands or through
the \chpl{CHPL_MODULE_PATH} environment variable.

The module search paths are initialized as follows:
\begin{tabular}{ll}
Internal & \$CHPL\_HOME/modules/internal/\$CHPL\_THREADS \\ 
& \$CHPL\_HOME/modules/internal/\$CHPL\_TASKS \\ 
& \$CHPL\_HOME/modules/internal \\ 
Standard & \$CHPL\_HOME/modules/standard \\
& \$CHPL\_HOME/modules/layouts \\
& \$CHPL\_HOME/modules/dists \\
User & (User paths) \\
\end{tabular}

When the root internal module \chpl{ChapelStandard} and its dependencies are
