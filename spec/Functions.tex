...
function~(\rsec{Function_Definitions}).  When a function is resolved, the
expression in the where clause must be a parameter expression and must
evaluate to either \chpl{true} or \chpl{false}.  If it evaluates to
\chpl{false}, the function is rejected and thus is not a possible candidate
