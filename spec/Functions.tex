...
During function resolution, if a producure with a \chpl{ref} return
intent and a procedure without a \chpl{ref} return intent are both
possible candidates, and either neither use promotion or both do,
this pair fo procedures is called a ref-pair.

When a procedure in a ref-pair is called on the left-hand side of an
assignment statement or in the context of a call to a formal argument by
\chpl{out}, \chpl{inout}, or \chpl{ref} intent, the \chpl{ref} return
intent procudure is used. Otherwise, the other candidate is used.  This
behavior can be useful for adjusting the behavior depending to the
calling context.

Previous versions of Chapel included an implicit \chpl{setter} param of
type \chpl{bool} for \chpl{ref} return intent functions. The old behavior
can be recreated by making both ref-pair functions call another function
and explicitly pass the \chpl{setter} value, as in the the example below.
