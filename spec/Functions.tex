...
Recursive and mutually-recursive functions are supported.
...
function-declaration-statement:
  `proc' function-name argument-list[OPT] var-param-type-clause[OPT] where-clause[OPT]
    function-body
...
%REVIEW: hilde
% <= may be considered symmetrical with > and >= with <.
% Yet, I see only == and not != in the list above.
% One could argue for complete control over the meaning of all six
% relationals, and at the other extreme for a single overloaded signum
% function that governed them all.  Compatibility with C# is not
...
The example \chpl{foo(y=5)} shows that we have to use a named argument
...
% test_setter.chpl
var x, y = 0;
proc A(i: int) var { // } to fool latex
  if i < 0 || i > 1 then
    halt("array access out of bounds");
\end{chapelpre}
\begin{chapel}
...
function defined in the Types module~(\rsec{Types_standard_module}).
\end{example}
...
type in a function declaration is optional.

\subsection{Explicit Return Types}
\label{Explicit_Return_Types}

If a return type is specified and is not \chpl{void}, the values that the function returns
...
For functions without any return statements, the return type is \chpl{void}.
...
%bsubsection{Accessing Outer Variables}
