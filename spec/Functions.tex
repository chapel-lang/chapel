...
the \chpl{inout} intent specifies to make a copies of the argument on
the way in and out of the function, while \chpl{ref} allows direct
...
Therefore the expressions \chpl{tuple(1, 2)} and \chpl{(1,2)} are equialent,
...
\index{functions!var keyword and@\chpl{var} keyword and}
...
by \chpl{out}, \chpl{inout}, or \chpl{ref} intent, the lvalue that is
...
For a non-variable procedure, an implicit conversion must be allowed
between the type of each return expression and the specified return type.
For variable procedures~(\rsec{Variable_Functions}), the return
...
For a variable procedure (\rsec{Variable_Functions}), they all must be
the same exact type, which becomes the inferred return type.
For a non-variable procedure, there must exist exactly one type
such that an implicit conversion is allowed between every other type
and that type, and that type becomes the inferred return type.
