...
The intents \chpl{in}, \chpl{out}, \chpl{inout} and \chpl{ref} are discussed
...
specifier \chpl{inline} indicates that the function body should be inlined at
...
The formal argument can be generic and is treated as a variable
declaration.  If the type is generic, it is taken from the type of
the actual argument.  It is initialized to the default value for its
type unless it has a default expression.
...
\subsection{The Ref Intent}
\label{The_Ref_Intent}
\index{ref (intent)@\chpl{ref} (intent)}
\index{intents!ref@\chpl{ref}}

If \chpl{ref} is specified as the intent, the actual argument is passed
by reference.  The formal argument can be assigned to and read from within
the function and changes are reflected in the actual at the call site.
The actual argument must be an lvalue.

The \chpl{ref} intent is different from the \chpl{inout} intent in that
the \chpl{inout} intent specifies to make a copy of the argument upon
entry into the function and to copy it back after exiting the function,
while \chpl{ref} allows direct access to the actual argument through the
formal argument without copies.
