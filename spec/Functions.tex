...
Anonymous first-class functions, those not formally defined as named functions, are also available using the lambda keyword.
...
Lambdas can also ``capture'' variables that are defined outside of the lambda by referring to them in the body of the lambda.  These form a closure where modifications to the variables modify the original variables (this is sometimes called capturing the variables by reference).  Because the lambdas refer to variables directly, this type of lambda can not be returned from functions safely and must be used before the function which holds the captured variables exits.
...
The type signature for functions captured as first-class value has the following form:

\begin{example}
\begin{chapel}
var f : func(arg_types, return_type);
\end{chapel}
\end{example}
