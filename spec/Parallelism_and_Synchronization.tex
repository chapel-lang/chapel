...
with \emph{synchronization variables} and \emph{atomic} statements.  To avoid
...
sequence returned by the \sntx{iterator-expression}.  Each instance of
...
the loop.  The keyword \chpl{ordered}, described
in~\rsec{Ordered_Forall}, can be used to constrain the parallelism to
...
[i in S] f(i);
\end{chapel}
The function \chpl{f} is evaluated for each index in \chpl{S} and the
...
ordered [i in S] f(i) 
\end{chapel}
\chpl{f} is a function and \chpl{S} is a
...
encountered begin-statement is executed serially with the current
computation; no new computation is spawned.  Control continues to the
...
variables \chpl{x} and \chpl{y} are impliclty single.
\end{example}
