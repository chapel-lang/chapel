...
the \chpl{Actor} class defined in the preceding chapter~\rsec{Class_Fields}.
\end{example}

\subsubsection{Record Methods}
...
declarations (\rsec{Class_Method_Declarations}).

\subsubsection{Nested Record Types}
...
\subsubsection{Record Initialization}
...
default initializer. The \chpl{new TimeStamp} expression creates a record that is assigned to \chpl{timestampCustom}, effectively initializing \chpl{timestampCustom} via a call to the constructor with desired arguments. The records created with \chpl{new UniqueID()} are discarded after they are used.
\end{example}
...
\index{member access!records}

A record field is accessed via a member access expression
(\rsec{Member_Access_Expressions}).  When such an expression is used as an
...
\subsubsection{Field Getter Methods}
...
\index{method access!records}

A record method may be called as well, using member access syntax
(\rsec{Member_Access_Expressions}).  Unlike class methods, record methods are
