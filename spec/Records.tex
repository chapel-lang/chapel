...
%REVIEW vass: this is a repetition of the first paragraph
Records are assigned by value;
see \rsec{Record_Assignment} for more details.  Records are also passed
by value unless argument intents (\rsec{Intents}) are used.
...
record-declaration-statement:
  `extern'[OPT] `record' identifier record-inherit-list[OPT] {
...
identifier.  A record inherits data from other records
if the \sntx{record-inherit-list} is specified.  A record type can inherit from
other record types, but not from class types.

\begin{future}
Records should inherit methods as well.  A record of a derived type contains all
of the fields of the base record type, so a method in the base class should work
without modification if invoked through a derived record instance.
\end{future}
...
of the assignment are overwritten with the values in the corresponding fields of the record
on the right-hand side as if by assignment.  The correspondence is established by
field name.  Specifically, the fields in the record on the left-hand side are
enumerated by name.  For each, the matching field in the right-hand is assigned
to the field having the same name on the left-hand side.  Since records can
contain fields of any type, the definition of assignment is applied recursively
until all assignments are resolved in terms of fundamental types.
...
(\rsec{Implicit_Conversions}) must exist between the fields' types.  That is,
...
%REVIEW: hilde
%Deviation from the C requirement of layout-compatibility means that the memcpy
%optimization is surrendered in most cases.  However, in the specific case that
%the RHS is of a type derived from the LHS type, the records *will* be layout
%compatible and the optimization can be applied.  It is also fairly simple for
%the compiler to determine whether two records are layout-compatible, even if
%there is no inheritance relationship between them.

Assignment from record to a variable of class type is not permitted.
...
Assignment between objects and records has not yet been implemented.
...
passed by value unless intents (\rsec{Intents}) are used.  The type of the
