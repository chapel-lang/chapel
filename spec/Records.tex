...
A record is a data structure that is similar to a class but instead has
...
variable is also initialized at that time.
...
because of the close relationship between constructors and type
...
A \chpl{these} method can be defined for records as well as classes
(\rsec{The_these_Method}).  It provides an iterator which iterates over
the contents of the record in a user-defined manner.
...
to class arguments which pass by \chpl{in} intent by default.

Similarly, the \chpl{this} receiver argument is passed by \chpl{in} by
default for class methods. In contrast, it is passed by \chpl{ref} or
\chpl{const ref} for record methods.
...
they are not supplied by the user. In contrast, the compiler only
provides \chpl{==} and \chpl{!=} for classes and these perform pointer
comparison.
