...
Record formal arguments with the \chpl{in} intent will be copy-initialized
into the function's formal argument (\rsec{Copy_Initialization_of_Records}).

Record formal arguments with \chpl{inout} or \chpl{out} intent will
be updated by the record assignment function (\rsec{Record_Assignment}).
...
following signatures for a record \chpl{R}:
...
Each of these compare the fields, one at a time, returning \chpl{false}
if the property is not satisfied by the given pair of fields.
