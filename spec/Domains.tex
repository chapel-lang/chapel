...
declare an array.  The domain's \chpl{dim()} accessor is used to fill the array with
...
created using the \chpl{add} procedure or \chpl{+} operator.
...
\index{domains!as procedure arguments}
...
scalar procedure as defined in~\rsec{Promotion}.

\begin{example}
Given a procedure \chpl{foo()} that accepts real floating point values
...
Chapel supplies built-in operators and procedures that can be used to manipulate
...
\index{domains!predefined procedures}
...
The following procedures are available only on irregular domain types.
