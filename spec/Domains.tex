...
supports the ability to create \emph{simple subdomains} and \emph{sparse
...
as all sparse subdomains (including those of regular domains).  An irregular
domain can store an arbitrary set of indices of an arbitrary but homogenous
index type.  Irregular domains typically require space proportional to the
number of indices being represented.
...
depending on whether its members have a well-defined order relationship.
...
Irregular base domain types include all of the associative domain types.  Associative
...
A subdomains has the same type as its parent domain, and by default
...
\subsubsection{Sparse Subomain Values}
...
to initialize a variable of a sparse subdomain type.
