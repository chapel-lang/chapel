...
In~\rsec{Base_Domains}, we discuss the types and values that
can be associated with a base domain.  In~\rsec{Simple_Subdomains}, we discuss the
types and values of simple subdomains that can be created from those base
domains.  In~\rsec{Sparse_Subdomains}, we discuss the types and values of sparse
subdomains.  The remainder sections describe the important manipulations
that can be perfomed with domains, as well as the built-in operators and
functions defined for domains.

\subsection{Domain Roadmap}
...
\begin{example}
The following declarations both create an unintialized rectangular domain with three dimensions, with \chpl{int} indices:
\begin{chapelpre}
% typeFunctionDomain.chpl
\end{chapelpre}
\begin{chapel}
...
% TODO: example here
% I need a hint on what you want here. -- THH
...
\begin{openissue}
Should there be an \chpl{opt_idxTypeEquals} in the domain type above?
\end{openissue}
...
\item Opaque domain types are parameterized by the type \chpl{opaque},
...
% TODO: assign too?
% Not sure what is needed here. -- THH
...
created using the \chpl{add} function or \chpl{+} operator.
...
discussed in the following section (\rsec{Sparse_Subdomains}).  A simple
...
\subsubsection{Simple Subdomain Types}
...
\subsubsection{Simple Subdomain Values}

%%The following paragraphs need to be reviewed.  I just wrote down some stuff
%%because we need to say something here. --- THH

\begin{openissue}
Needs review:  By default, the value of subdomain is an empty index set.  A rectangular subdomain
can be initialized using a domain literal (\rsec{Rectangular_Domain_Values}).
Associative subdomains must be initialized explicitly, by using the \chpl{add}
...
%% I would suggest "uninteresting values" or values that can be omitted. --- THH
See~\rsec{Sparse_Arrays} for details about sparse arrays.

\subsubsection{Sparse Subdomain Types}
...
\subsubsection{Sparse Subdomain Values}
...
If the domain's indices are ordered, a \chpl{for} loop will traverse the
indices in order.


\subsection{Domains as Arguments}
...
\subsection{Predefined Methods on Domains}
...
\subsubsection{Methods on All Domain Types}
...
\subsubsection{Methods on Regular Domains}
...
\subsubsection{Methods on Irregular Domains}
