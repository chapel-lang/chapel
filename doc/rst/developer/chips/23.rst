...
intent is to avoid passing an object with uninitialized fields, and to
...
One choice is to allow derived classes to override one or both
of init() and postInit() without any further extensions.  Additionally
Derived.postInit() would not explicitly invoke Parent.postInit(); the
