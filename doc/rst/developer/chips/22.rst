...
Further more, in order to optimize the execution on GPU, users can
...
Example: ::
	...
	var kernel = string(“_kernel_ helloword(){ “\\
...
kernel.
	...
	**ii. Rely on intent concept (with clause).**
	...
	provide the workroup and the grid size for the kernel. In other
	words the Intent concept needs to be extended to support a new class
	defined as **Execution Parameters** that includes workgoup and grid
...
concepts:
	...
	**ii. Special Memory Management**
	...
	Alternatively, we propose to introduce a new keyword like
...
Assume the user writes an I/O callsinside a block statement that
...
-  **Invocation and launching extension**: to provide and easy and
...
Distribute the data among the 2\ :sup:`nd` sublocale on each local (
...
{(0,1), (1,1), (2,1), …(n,1)} ::
	...
	SublocalBlockSpace = Space dmapped Block(boundingBox=Space,
						  targetLocales=MySublocales);

	var A : [SublocalBloclSpace] int; 
