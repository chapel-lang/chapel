...
Uh-oh! Once again Global is not initailized properly, and we can expect
...
What's going wrong? Function disambiguation rules prefer a more specific match over a more visible one, so the call setup(x) that used to rely on a coercion from int to real now resolves to the function in the other library that doesn't require a coercion.

Solution: Can visibility take priority over specificity in resolution? Or could it be an error if these don't match? D's overload sets are an example.
...
New Method is Accidentally Overriden
++++++++++++++++++++++++++++++++++++
...
Unexpected new Overload for an Overriden method
+++++++++++++++++++++++++++++++++++++++++++++++
...
In that event, even though ``sort`` is generic, it might be suprising if
