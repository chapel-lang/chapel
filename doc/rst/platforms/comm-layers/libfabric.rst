...
#. Set the ``CHPL_LIBFABRIC`` environment variable indicating whether to
   ...
   .. note::
     When using a non-Slingshot host to build a libfabric library for a 
     Slingshot enabled target (e.g., building Chapel on a linux64 host for 
     execution on an HPE-Cray-EX target), make sure that your libfabric is built
     with ``cxi`` support. That generally requires the corresponding HPE CXI 
     headers, the ``libcxi`` development files, and the matching CXI driver 
     headers used by this build path. Public sources for these are available 
