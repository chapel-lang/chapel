...
Details on the ``AstNode`` Hierarchy
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
Details on the ``Error`` Hierarchy
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Chapel's compiler library attempts to include all the information required
to print an error message in the error object itself. This can include
IDs of AST nodes, AST nodes themselves, and more. The compiler considers each
...
.. code-block:: python
   ...
   assert len(asts) == 1
   assert isinstance(asts[0], Module)
   mod_children = list(asts[0])
   assert isinstance(mod_children[-1], NamedDecl)
   decl = mod_children[-1]

   with ctx.track_errors() as errors:
       decl.resolve()

   assert len(errors) == 1
   assert isinstance(errors[0], MismatchedInitializerResult)

   (ast, start_type, end_type, mismatched_fields) = errors[0].info()
   print("Started as: ", start_type)
...
.. code-block:: text
   ...
   Number of mismatched fields:  1

.. note::

   The error message classes in the Chapel compiler library expose a variety
   of data types, not all of which are currently mapped to the Python API. To
   keep the tuple returned by ``info`` a consistent shape as the mapping evolves,
   ``chapel-py`` replaces unsupported types with placeholders: empty tuples.
   The rationale is that, when the mapping is eventually implemented, existing
   code that unpacks the data returned by ``info`` will continue to work.
