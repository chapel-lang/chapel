...
\item Insert casts on any \cc{MOVE} primitive that requires it.
...
\item Transform the code in local blocks to check that wide references
  are local (unless checks are disabled).  TODO

\item Call \cc{narrowWideReferences} to try to eliminate wide
  references in preference to narrow references if the compiler can
  prove they are local.  TODO
\end{enumerate}
