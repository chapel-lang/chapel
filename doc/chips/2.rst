...
equality constraint in its where clause, then the function is type checked
at its point of definition. The rules for functional visibility in generic
functions (Section 22.2 of the Chapel Language Specification 0.796) do not
...
that the required behavior is occuring. For example:
...
.. code-block:: chapel
  ...
  // Idea 2:
  // explicitly support nominal interfaces
  explicit interface AssociativeAddition(T) {
    // this interface is nominal
  }
...
strategy like the one outline above in "Semantics Requirements"? Should we
default to nominal and opt-in to structural? Or vice versa?
