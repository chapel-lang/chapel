 ...
 * The `return` statement in a `ref` or `const ref` return intent function
   ...
   `retVar` is just set to the returned `ref`. For arrays, `moveRef`
   and `copyRef` would be called instead of `move` and `copy`. Note that
   types with runtime
   components need  a `moveRef` in order
...
made when returning an array from a function with `ref` return intent.
Instead, `moveRef` and `copyRef` calls would be made. These calls support
returning an array slice by reference. Note
 ...
 * If returning a slice of a local array is a program error, we could
   make functions returning arrays behave the same as if they were marked
   with the `ref` return intent. This is in some ways more consistent
