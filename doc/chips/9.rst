...
and git repository. For example, after ``mason new MyPackage`` is run,
the project will have the following directory hierarchy:
...
This will fetch all the dependencies and build the project for a fully
functional copy.
...
As lock file is a serialized directed acyclic graph of the dependencies
in the project. It is more concrete than a manifest because it specifies
Git SHA revisions rather than abstract "versions" or "packages". The purpose
of the lock is to ensure repeatable builds on a given system; whether the
lock file is checked into the repository to ensure identical builds across
all systems is left to the project.
