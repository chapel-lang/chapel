...
We also considered allowing child's fields to be initialized based on
parent's fields. For example, the child could control where, in its Phase 1,
the parent's Phase 1 is invoked, not necessarily as the last step.

The following example shows that based on the second syntax above.
It also shows the option of having the child control when the parent's Phase 2
is invoked.
...
This approach seems to require the syntax 2.
We chose not to incorporate it because it makes less clear
...
actions after the child class(s) have established their initialization
...
This can be done by introducing the optional "lastly" phase, to be executed
in the parent after all actions specified by child class(s) have completed.
