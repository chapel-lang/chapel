 ...
 * `Defining the Initializer Body`_
   ...
   - `Syntax`_
   - `Parent/Child Field Dependence`_
   - `Ordering of Parent/Child Initialization`_
...
Parent/Child Field Dependence
*****************************

We also considered allowing a child's fields to be initialized based on
its parent's fields. For example, the child could control the point
during its Phase 1 when the parent's Phase 1 is to be invoked.
The following example is based on the second syntax above:
...
Analogously, we could allow the child to control the point during its Phase 2
when the parent's Phase 2 is to be invoked. For example:
...
This approach seems to require syntax 2.
We chose not to incorporate this feature because of the additional complexity
it introduces in how initialization is performed.

Ordering of Parent/Child Initialization
***************************************

We also considered allowing the parent class to perform final initialization
actions after the child class(es) have established their initialization
invariants. An example of such an action is adding the new object to a shared
list. If the list is concurrently accessible, all other invariants on the
new object must have been established prior to such addition.

One way to achieve this is by introducing the optional "lastly" phase, to be executed
in the parent after all actions specified by child class(es) have completed.
It allows parent to assume "final control" over newly-created objects.
...
to perform such "final" actions outside of the initializer.
