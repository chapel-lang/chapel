...
def get_arguments():
    ...
    parser.add_argument(
        "chapelfiles", nargs="+", help="Chapel files to convert to rst"
    )
    parser.add_argument(
        "--output",
        default="rst",
...
def gen_link(link, chapelfile):
    ...
    hyperlink = "https://github.com/chapel-lang/chapel/blob/{0}/{1}".format(
        link, chplpath
    )
...
def main_args(**kwargs):
    ...
    for chapelfile in chapelfiles:
        islearnChapelInYMinutes = (
            os.path.basename(chapelfile) == "learnChapelInYMinutes.chpl"
        )
