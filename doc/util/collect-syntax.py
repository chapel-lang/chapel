...
_capture_production = re.compile(
    r"(?P<indent>\s+)" + r"(?P<symbol>([a-zA-Z0-9._-]+)):.*"
)

# Group 1 = 'quoted' | Group 2 = \escaped | Group 3 = nonterminal
_capture_nonterminal = re.compile(
    r"\s+('[ a-zA-Z0-9-()]+')|" + r"(\\[a-zA-Z]+)|" + r"([a-zA-Z0-9-]+)"
)
...
def get_script_arguments():
    ...
    parser.add_argument(
        "rstfiles", nargs="+", help="gather syntax rules from these RST files"
    )
    parser.add_argument(
        "--outdir",
        ...
        while i < len(self._lines):
            ...
            if not re.match(_match_all_whitespace, line):
                ...
                if production:
                    ...
                    continue
                msg = (
                    "Garbled syntax block at " + self._fname + ":" + str(i + 1)
                )
                raise Exception(msg)
            i += 1
        return i
