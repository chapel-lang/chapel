...
def to_pieces(handle, islearnChapelInYMinutes=False):
    ...
    def flush():
        ...
        if len(current_content) != 0 and any(s != "" for s in current_content):
            ...
            while current_content[-1] == "":
                current_content.pop(-1)
        ...
        if "comment" in state:
            ...
            if islearnChapelInYMinutes and "multi-line comment" in proseline:
                ...
                if foundCommentExample:
                    print(
                        'Error: Found more than one line containing "multi-line comment" in \
    ...
    if islearnChapelInYMinutes and not foundCommentExample:
        print(
            "Error: Failed to find special case of comment example in learnChapelInYMinutes.chpl"
        )
        sys.exit(1)
    return output
