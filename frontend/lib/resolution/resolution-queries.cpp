...
resolveConcreteFunction(Context* context, ID id, const CallerDetails& caller) {
  ...
  const TypedFnSignature* sig = typedSignatureInitial(context, uSig, caller);
...
static ApplicabilityResult
isCandidateApplicableInitialImpl(Context* context,
                                 const ID& candidateId,
                                 const CallInfo& ci,
                                 const CallerDetails& caller) {
  AstTag tag = candidateId ? parsing::idToTag(context, candidateId)
  ...
  auto ret = typedSignatureInitial(context, ufs, caller);
...
isCandidateApplicableInitialQuery(Context* context,
  ...
  auto result = isCandidateApplicableInitialImpl(context, candidateId,
                                                 call, {});
  return QUERY_END(result);
}
...
filterCandidatesInitialGatherRejectedImpl(Context* context,
  ...
  for (const BorrowedIdsWithName& ids : lst) {
    for (const ID& id : ids) {
      bool isNestedCandidate = parsing::idIsNestedFunction(context, id);

      if (isNestedCandidate) ret.evaluatedAnyNestedFunction = true;

      auto s = isNestedCandidate && !caller.isEmpty()
          ? isCandidateApplicableInitialImpl(context, id, call, caller)
...
resolveFnCallFilterAndFindMostSpecific(Context* context,
  ...
  CandidatesAndForwardingInfo candidates = gatherAndFilterCandidates(
      context, call, ci, inScopes, firstPoiCandidate, rejected, caller);
...
CallResolutionResult resolveCall(Context* context,
  ...
  if (call->isFnCall() || call->isOpCall()) {
    ...
    return resolveFnCall(context, call, ci, inScopes, rejected, caller);
