...
setupCallForCopyOrMove(Resolver& resolver,
  ...
  if (!lhsType.isUnknown() &&
    ...
    ci = CallInfo (/* name */ freeFn,
                        ...
                        /* isParenless */ false,
                        std::move(actuals));
  } else if (!lhsType.isUnknown() && lhsType.type()->isTupleType()) {
    // For tuples, copy/convert to a value tuple.
    // Using _build_tuple here is something of a placeholder (or white lie)
    // since it has no body until generated by production later. But it matches
    // the spirit of production's approach.
    auto freeFn = UniqueString::get(resolver.context, "_build_tuple");
    auto arg = QualifiedType(QualifiedType::CONST_VAR, rhsType.type(),
                             rhsType.param());
    actuals.push_back(CallInfoActual(arg, UniqueString()));
    outAsts.push_back(rhsAst);
    ci = CallInfo(/* name */ freeFn,
                  /* calledType */ QualifiedType(),
                  /* isMethodCall */ false,
                  /* hasQuestionArg */ false,
                  /* isParenless */ false, std::move(actuals));
  } else {
