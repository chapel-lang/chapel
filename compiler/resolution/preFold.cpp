...
static Expr* preFoldPrimOp(CallExpr* call) {
  ...
  } else if (call->isPrimitive(PRIM_SIZEOF_RAW) == true) {
    // Fix up arg to sizeof_raw(), as we may not have known the type earlier
    SymExpr* sizeSym  = toSymExpr(call->get(1));
    Type*    sizeType = sizeSym->symbol()->typeInfo();

    retval = new CallExpr(PRIM_SIZEOF_RAW, sizeType->symbol);
    call->replace(retval);
