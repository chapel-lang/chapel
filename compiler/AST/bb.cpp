...
#define BB_ADD(expr, flag)                      \
  do {                                          \
    INT_ASSERT(expr);                           \
    basicBlock->exprs.push_back(expr);          \
    basicBlock->isEssential.push_back(flag);    \
  } while (0)
...
bool BasicBlock::isOK() {
  // There must be an isEssentialFlag for every expr
  if (exprs.size() != isEssential.size())
    return false;

  // All expressions must be live (non-NULL);
...
void BasicBlock::buildBasicBlocks(FnSymbol* fn, Expr* stmt) {
  if (stmt == 0) {
    ...
    BB_ADD(s, false); // Put the goto at the end of its block.
    BB_RESTART();
    ...
    if (def && toLabelSymbol(def->sym)) {
      ...
      BB_ADD(def, false); // Put the label def at the start of its block.
