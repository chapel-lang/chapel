...
static bool isRefUse(SymExpr* se)
{
  ...
  else // A primitive.
  {
    if (se->typeInfo()->symbol->hasFlag(FLAG_REF))
    {
      ...
      // accept a ref argument in that position, such that may update the
      // referenced value.
      switch(call->primitive->tag)
      {
       default:
...
/// \param useSet A SymExpr appearing in useSet is read (used) by the call in
/// which it appears as determined by isDefAndOrUse.
/// \param defSet A SymExpr appearing in defSet is written (defined) by the call in
/// which it appears as determined by isDefAndOrUse.
static void
localCopyPropagationCore(BasicBlock* bb,
                         AvailableMap& available,
