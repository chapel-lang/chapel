...
static void test1(Parser* parser) {
  ...
  assert(multi->numStmts() == 2);
  auto x = multi->stmt(0)->toVariableDecl();
  auto y = multi->stmt(1)->toVariableDecl();
...
static void test4(Parser* parser) {
  ...
  assert(module->numStmts() == 1);
  auto multi = module->stmt(0)->toMultiVarDeclaration();
  assert(multi->numStmts() == 4);
  auto a = multi->stmt(0)->toVariableDecl();
  auto b = multi->stmt(1)->toVariableDecl();
  auto c = multi->stmt(2)->toVariableDecl();
  auto d = multi->stmt(3)->toVariableDecl();
...
static void test4b(Parser* parser) {
  ...
  assert(module->numStmts() == 1);
  auto multi = module->stmt(0)->toMultiVarDeclaration();
  assert(multi->numStmts() == 5);
  assert(multi->stmt(0)->isComment());
  auto a = multi->stmt(1)->toVariableDecl();
  auto b = multi->stmt(2)->toVariableDecl();
  auto c = multi->stmt(3)->toVariableDecl();
  auto d = multi->stmt(4)->toVariableDecl();
  assert(0 == a->name().compare("a"));
  assert(0 == b->name().compare("b"));
  assert(0 == c->name().compare("c"));
  assert(0 == d->name().compare("d"));
