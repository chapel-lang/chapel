...
bool UintLiteral::contentsMatchInner(const ASTNode* other) const {
  const UintLiteral* lhs = this;
  const UintLiteral* rhs = (const UintLiteral*) other;
  return lhs->literalContentsMatchInner(rhs) &&
         lhs->value_ == rhs->value_ &&
         lhs->base_ == rhs->base_;
}
void UintLiteral::markUniqueStringsInner(Context* context) const {
