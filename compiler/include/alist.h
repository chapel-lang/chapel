...
#define for_alist(node, list)                                           \
  for (Expr *node = (list).head,                                        \
         *_alist_next = node ? node->next : nullptr;                    \
       node;                                                            \
       node = _alist_next,                                              \
         _alist_next = node ? node->next : nullptr)
...
#define for_alist_backward(node, list)                                  \
  for (Expr *node = (list).tail,                                        \
         *_alist_prev = node ? node->prev : nullptr;                    \
       node;                                                            \
       node = _alist_prev,                                              \
         _alist_prev = node ? node->prev : nullptr)
...
#define for_enums(node, et)                                             \
  for (DefExpr *node = toDefExpr((et)->constants.head),                 \
         *_alist_next = node ? toDefExpr(node->next) : nullptr;         \
       node;                                                            \
       node = _alist_next,                                              \
         _alist_next = node ? toDefExpr(node->next) : nullptr)
...
#define for_enums_backward(node, et)                                    \
  for (DefExpr *node = toDefExpr((et)->constants.tail),                 \
         *_alist_prev = node ? toDefExpr(node->prev) : nullptr;         \
       node;                                                            \
       node = _alist_prev,                                              \
         _alist_prev = node ? toDefExpr(node->prev) : nullptr)
...
#define for_formals(formal, fn)                                         \
  for (ArgSymbol *formal = ((fn)->formals.head) ? toArgSymbol(toDefExpr((fn)->formals.head)->sym) : nullptr, \
         *_alist_next = (formal && formal->defPoint->next) ? toArgSymbol(toDefExpr((formal)->defPoint->next)->sym) : nullptr; \
       (formal);                                                        \
       formal = _alist_next,                                            \
         _alist_next = (formal && formal->defPoint->next) ? toArgSymbol(toDefExpr((formal)->defPoint->next)->sym) : nullptr)

#define next_formal(formal)                                             \
  ((formal && formal->defPoint->next) ? toArgSymbol(toDefExpr((formal)->defPoint->next)->sym) : nullptr)

#define for_formals_backward(formal, fn)                                \
  for (ArgSymbol *formal = ((fn)->formals.tail) ? toArgSymbol(toDefExpr((fn)->formals.tail)->sym) : nullptr, \
         *_alist_prev = (formal && formal->defPoint->prev) ? toArgSymbol(toDefExpr((formal)->defPoint->prev)->sym) : nullptr; \
       (formal);                                                        \
       formal = _alist_prev,                                            \
         _alist_prev = (formal && formal->defPoint->prev) ? toArgSymbol(toDefExpr((formal)->defPoint->prev)->sym) : nullptr)
...
#define for_actuals(actual, call)                                       \
  for (Expr *actual = (call)->argList.head,                             \
         *_alist_next = actual ? actual->next : nullptr;                \
       actual;                                                          \
       actual = _alist_next,                                            \
         _alist_next = actual ? actual->next : nullptr)
...
#define for_actuals_backward(actual, call)                              \
  for (Expr *actual = (call)->argList.tail,                             \
         *_alist_prev = actual ? actual->prev : nullptr;                \
       actual;                                                          \
       actual = _alist_prev,                                            \
         _alist_prev = actual ? actual->prev : nullptr)
...
#define for_formals_actuals(formal, actual, call)                             \
  ...
  if (_alist_fn) {                                                            \
    ...
    actual = actual->next->next;                                              \
                                                                              ...
                                                                              \
  Expr* _alist_actual_next = (actual) ? actual->next : nullptr;               \
                                                                              \
  for (ArgSymbol* formal = (_alist_fn->formals.head) ?                        \
         toArgSymbol(toDefExpr(_alist_fn->formals.head)->sym) : nullptr,      \
                                                                              \
       *_alist_formal_next = (formal && formal->defPoint->next) ?             \
         toArgSymbol(toDefExpr((formal)->defPoint->next)->sym) : nullptr;     \
                                                                              \
       (formal);                                                              \
                                                                              \
       ...
       _alist_actual_next = (actual) ? actual->next : nullptr)



#define for_fields(field, ct)                                                 \
  for (Symbol *field = ((ct)->fields.head) ? toDefExpr((ct)->fields.head)->sym : nullptr, \
         *_alist_next = (field && field->defPoint->next) ? toDefExpr(field->defPoint->next)->sym : nullptr; \
       (field);                                                               \
       field = _alist_next,                                                   \
         _alist_next = (field && field->defPoint->next) ? toDefExpr(field->defPoint->next)->sym : nullptr)

#define for_fields_backward(field, ct)                                        \
  for (Symbol *field = ((ct)->fields.tail) ? toDefExpr((ct)->fields.tail)->sym : nullptr, \
         *_alist_prev = (field && field->defPoint->prev) ? toDefExpr(field->defPoint->prev)->sym : nullptr; \
       (field);                                                               \
       field = _alist_prev,                                                   \
         _alist_prev = (field && field->defPoint->prev) ? toDefExpr(field->defPoint->prev)->sym : nullptr)
