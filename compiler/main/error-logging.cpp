...
void setupError(const char* subdir, const char* filename, int lineno,
  ...
  err_lineno        = lineno;
  err_fatal         = tag == ERR_INTERNAL_FATAL ||
                      tag == ERR_USER_FATAL     ||
                      tag == ERR_USER_FATAL_CONTINUE;
  err_user          = tag != ERR_INTERNAL_FATAL;
  err_print         = tag == ERR_USER_PRINT;
  err_ignore        = ignore_warnings && tag == ERR_USER_WARN;

  exit_immediately  = tag == ERR_INTERNAL_FATAL || tag == ERR_USER_FATAL;
  exit_eventually  |= tag == ERR_USER_FATAL_CONTINUE;
}

// No need to set path info because this does not handle internal errors.
...
void dynoPrintSyntaxError(const chpl::ErrorMessage& err) {
  INT_ASSERT(err.kind() == chpl::ErrorMessage::SYNTAX);

  const char* msg = err.message().c_str();
  auto loc = err.computeLocation(gContext);
  UniqueString path = loc.path();
  const int line = loc.line();
  const auto tag = ERR_USER_FATAL_CONTINUE;

  setupError("parser", path.c_str(), line, tag);
