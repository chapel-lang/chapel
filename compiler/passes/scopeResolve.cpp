...
static void scopeResolve(InterfaceSymbol*    ifcSym,
                         const ResolveScope* parent) {
  ResolveScope* scope = new ResolveScope(ifcSym, parent);

  for_alist(def, ifcSym->ifcFormals)
    scope->extend(toDefExpr(def)->sym);

  scopeResolve(ifcSym->ifcBody, scope);
}
  ...
  for_alist(stmt, alist) {
    if (DefExpr* def = toDefExpr(stmt))   {
      ...
      if (sym->hasFlag(FLAG_TEMP) == false &&
        ...
        } else if (InterfaceSymbol* ifcSym = toInterfaceSymbol(sym)) {
          scopeResolve(ifcSym, scope);
        }
      }
...
BaseAST* getScope(BaseAST* ast) {
  if (Expr* expr = toExpr(ast)) {
   if (Expr* parent = expr->parentExpr) {
    ...
    } else if (InterfaceSymbol* ifc = toInterfaceSymbol(expr->parentSymbol)) {
      return ifc;
    }
