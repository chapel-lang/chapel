  ...
  for (TypeSymbol *type : typeSymbols) {
    ...
    if (AggregateType* ct = toAggregateType(type->type)) {
      ...
      if (ct->symbol->hasFlag(FLAG_RESOLVED_EARLY)) {
        // The type is being generated by the 'dyno' frontend. Don't
        // bother generating default functions for it as the frontend
        // will have already generated whatever is needed on demand.
        continue;
      }
