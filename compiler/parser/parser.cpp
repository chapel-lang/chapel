...
static Vec<UseExpr*>    modReqdByInt;  // modules required by internal ones

void addModuleToParseList(const char* name, UseExpr* useExpr) {
...
void parseDependentModules(ModTag modtype) {
  ...
  if (modtype == MOD_USER) {
    do {
      Vec<UseExpr*> modReqdByIntCopy = modReqdByInt;

      modReqdByInt.clear();

      handlingInternalModulesNow = true;

      forv_Vec(UseExpr*, moduse, modReqdByIntCopy) {
