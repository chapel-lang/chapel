//
//    EXPR.CPP -- Code generator (C back-end)
//
//        Expressions
//
...
// interface: Expr variants
  ...
  GenRet GenC::SymExpr_codegen(SymExpr *self) {
      Symbol *var = self->var;

      GenRet ret;
      if (self->getStmtExpr() == self)
          codegenStmt(self);
      ret = Symbol_codegen(var);
      return ret;  
      }
      
  GenRet GenC::UnresolvedSymExpr_codegen(UnresolvedSymExpr *self) {
      const char *unresolved = self->unresolved;
  
      GenRet ret;
      INT_FATAL(self, "UnresolvedSymExpr::codegen called");
      writeC("%s /* unresolved symbol */", unresolved);
      return ret;  
      }
      
  GenRet GenC::DefExpr_codegen(DefExpr *self) {
      Symbol *sym = self->sym;
 ...
 */
  GenRet GenC::CallExpr_codegen(CallExpr *self) {  
      GenRet ret;
  ...
  GenRet GenC::NamedExpr_codegen(NamedExpr *self) {
      GenRet ret;
...
// implementation: evaluation helpers
  ...
  GenRet GenC::evalSym(Symbol *sym) {
